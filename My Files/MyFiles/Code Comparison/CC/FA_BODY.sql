...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body XXMX_FA_MASS_ADDITIONS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_FA_MASS_ADDITIONS_PKG" as 
    ...
    BEGIN
        ...
        */
        ...
        xxmx_utilities_pkg.log_module_message(  
         ...
         --
         FOR i in (Select dist.mass_addition_id,mas.interface_line_num
                        ,count(dist.mass_addition_id) cnt 
                  from xxmx_fa_mass_addition_dist_Stg dist, xxmx_fa_mass_additions_Stg  mas
                  where  mas.mass_addition_id = dist.mass_addition_id
                  group by dist.mass_addition_id,mas.interface_line_num)         
         LOOP
           v_line_num:= i.interface_line_num;

           FOR j in (Select rowid from xxmx_fa_mass_addition_dist_Stg where mass_addition_id = i.mass_addition_id)
           LOOP
            v_line_num:= v_line_num+1;
               DBMS_OUTPUT.PUT_LINE(v_line_num);
               UPDATE xxmx_fa_mass_addition_dist_Stg
               SET INTERFACE_LINE_NUM= v_line_num
               where rowid= j.rowid;
           END LOOP;
         END LOOP;
   ...
   IS
    ...
    IS
        SELECT   fab.asset_id                                                              AS mass_addition_id
                ...
                ,DECODE((SELECT DISTINCT TRANSACTION_SUBTYPE FROM apps.fa_transaction_headers@MXDM_NVIS_EXTRACT
				WHERE 1=1
				AND ASSET_ID=FAB.ASSET_ID 
				AND TRANSACTION_SUBTYPE='AMORTIZED'),'AMORTIZED','YES','NO')               AS amortize_flag
                ,DECODE((SELECT DISTINCT TRANSACTION_SUBTYPE FROM apps.fa_transaction_headers@MXDM_NVIS_EXTRACT
				  ...
				  (select LAST_DAY(TO_CHAR(ADD_MONTHS(PARAMETER_VALUE,1),'DD-MON-YYYY')) 
                      ...
                      AND ENABLED_FLAG='Y'),'')                                            AS amortization_start_date
                ,fb.depreciate_flag                                                        AS depreciate_flag
                ...
                ,(SELECT NVL(SUM(YTD_IMPAIRMENT),0) FROM APPS.FA_DEPRN_DETAIL@MXDM_NVIS_EXTRACT A ,FA_DEPRN_PERIODS@MXDM_NVIS_EXTRACT B
                		...
                		AND A.PERIOD_COUNTER <= GVV_PERIOD_COUNTER)                        AS ytd_impairment
                ,(SELECT SUM(impairment_reserve)--SUM(impairment_amount)
                  FROM   apps.fa_itf_impairments@MXDM_NVIS_EXTRACT a
                  WHERE  a.asset_id            = fab.asset_id
                  AND    a.book_type_code      = fb.book_type_code
                  AND    a.impairment_id       = (SELECT MAX(b.impairment_id)
                                                  FROM   apps.fa_itf_impairments@MXDM_NVIS_EXTRACT b
                ...
                ,xxmx_utilities_pkg.gvv_UserName                                           AS last_updated_by -- constant
                ,SYSDATE                                                                   AS last_update_date -- constant
         FROM apps.fa_additions_v@MXDM_NVIS_EXTRACT               fab
           ,apps.fa_additions_tl@MXDM_NVIS_EXTRACT                fat
           ,apps.fa_asset_keywords@MXDM_NVIS_EXTRACT              fak
           ,apps.fa_books@MXDM_NVIS_EXTRACT                       fb
           ,apps.fa_deprn_summary@MXDM_NVIS_EXTRACT               fds
           ,apps.fa_categories_b@MXDM_NVIS_EXTRACT                fac
       ...
       PRAGMA EXCEPTION_INIT(ex_dml_errors, -24381);
       l_error_count             NUMBER;
       v_line_num                NUMBER; -- Used for INTERFACE_LINE_NUM
        ...
        */
        ...
        THEN
            ...
            */
            ...
            --**                  by a Synonym in the xxmx_core schema to that table.
            --
/*New code added as per citco's recommendation*/
BEGIN
...
END;
          ...
          VALUES (
                  ...
                  ,imp_massadd_src_tb(i).interface_line_num
                  ,'EXTRACTED'
                  ,gvt_MigrationSetName
         ...
         v_line_num:= 100;
         FOR i in (SELECT MASS_ADDITION_ID FROM XXMX_STG.XXMX_FA_MASS_ADDITIONS_STG fmas)
         LOOP
            UPDATE XXMX_STG.XXMX_FA_MASS_ADDITIONS_STG
            SET INTERFACE_LINE_NUM = v_line_num
            WHERE MASS_ADDITION_ID= i.MASS_ADDITION_ID;

            v_line_num:= v_line_num+100;
         END LOOP;
         COMMIT;
         --
         gvv_ProgressIndicator := '0032';
	...
	*****************************************************************/
    ...
    AS 
      ...
      --**********************
      --
        /* Use the below cursor when reporting currency in ledger have cost.*/
    ...
    /* Use the below cursor when reporting currency in ledger doesn't have cost. Therefore cost is being set as NULL and rate is being used instead below.*/
    /* CURSOR imp_massadd_rates_cur
        IS
            --
            SELECT  fmav.mass_addition_id,
                    gsob.to_currency                AS currency_code,
                    NULL                            AS fixed_assets_cost, --fmav.fixed_assets_cost --SHOULD NOT HAVE BOTH COST AND RATE, just need one of them. 
            ...
            FROM    xxmx_fa_mass_additions_stg fmav,
                    apps.gl_daily_rates@MXDM_NVIS_EXTRACT      gsob,
