--------------------------------------------------------
--  DDL for Package Body XXMX_KIT_PERSON_STG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_KIT_PERSON_STG" AS 
...
--**   1.6  22-AUG-22  Vamsi                Removed Applicant type from XXMX_PER_PERSONS_STG insert Query
--**                                        as Applicant Numbers and Employee Numbers are same for few.
--******************************************************************************
	...
	*****************************************************************/
    ...
    BEGIN
        ...
        SELECT  /*+ driving_site(table alias name) */ distinct pt_i_MigrationSetID                               
				...
				,gv_hr_date
				,papf.person_id
        from per_disabilities_f@XXMX_EXTRACT pdf
		...
		-- DG 14/02/2022
		and papf.effective_Start_Date = (select EFFECTIVE_START_DATE FROM XXMX_HCM_CURRENT_PERSON_MV xpmsd
											WHERE xpmsd.person_id = papf.person_id
											)
	...
	*********************************************************/
	...
	BEGIN
       ...
       Select parent_id,
              bg_id,
              image 
        from per_images@XXMX_EXTRACT PI ,
	...
	*********************************************************/
	...
	BEGIN
        ...
        INTO    XXMX_PER_ABSENCE_STG
          ...
          select * FROM(
           with  absences as (  
                  select Y.ABSENCE_ATTENDANCE_TYPE_ID         
                        ,x.Approval_status
                        ,X.SICKNESS_START_DATE
                        ,X.SICKNESS_END_DATE
                        ,X.DATE_START 
                        ,X.DATE_END
                        ,Y.NAME absence_type_name
                        ,Y.ABSENCE_CATEGORY                               
--                        ,xxmx_kit_person_stg.get_target_lookup_code(Y.ABSENCE_CATEGORY,'ABSENCE_CATEGORY') abs_cat_meaning
                        --,Y.ABSENCE_CATEGORY abs_cat_meaning
                        ,NULL abs_cat_meaning
                        ,X.DATE_NOTIFICATION
                        ,ABSR.name absence_reason
--                        ,xxmx_kit_person_stg.get_target_lookup_code(Y.ABSENCE_CATEGORY,'ABSENCE_REASON') abs_reason_meaning
                        --,Y.ABSENCE_CATEGORY abs_reason_meaning
                        ,NULL abs_reason_meaning
                        ,X.ABSENCE_DAYS 
                        ,X.ABSENCE_Hours 
                        ,X.BUSINESS_GROUP_ID
                        ,x.person_id
                        ,X.SSP1_Issued
                   FROM PER_ABSENCE_ATTENDANCES@XXMX_EXTRACT X,
                        PER_ABS_ATTENDANCE_REASONS@XXMX_EXTRACT ABSR,
                        PER_ABSENCE_ATTENDANCE_TYPES@XXMX_EXTRACT Y
                         --fnd_lookup_values@XXMX_EXTRACT flv
                  WHERE X.ABSENCE_ATTENDANCE_TYPE_ID = Y.ABSENCE_ATTENDANCE_TYPE_ID
                    ...
                    AND x.business_group_id = P_BG_ID   
                    and Exists (select 1 
                                   FROM xxmx_per_persons_stg per
                                  WHERE per.person_id = X.person_id
                                    AND x.business_group_id = P_BG_ID )
                   )
            SELECT  distinct pt_i_MigrationSetID  migration_set_id                              
                   ...
                   ,p_bg_name bg_name
                   ,p_bg_id bg_id
                   ,A.EMPLOYEE_NUMBER   PERSONNUMBER
                   ,absences.ABSENCE_ATTENDANCE_TYPE_ID ABSENCETYPEID
                   ,NULL  ABSENCESTATUS
                   ,absences.Approval_status  APPROVALSTATUS
                   ,NVL(absences.SICKNESS_START_DATE,absences.DATE_START ) STARTDATE
                   ,NULL  STARTTIME
                   ,NVL(absences.SICKNESS_END_DATE,absences.DATE_END) ENDDATE
                   ,NULL ENDTIME
                   ,NULL ABSENCETYPE
                   ,absences.absence_type_name  ABSENCENAME
                   ,absences.ABSENCE_CATEGORY ABSENCECATEGORY
                   ,absences.abs_cat_meaning  ABSCAT_MEANING
                   ,absences.DATE_NOTIFICATION  DATE_NOTIFICATION                
                   ,NVL(absences.absence_reason,'DATA MIGRATION') ABSENCEREASON
                   ,absences.abs_reason_meaning ABSREASON_MEANING
                   ,NVL(absences.SICKNESS_START_DATE,absences.DATE_START ) CONFIRMEDDATE
                   ,NULL COMMENTS
                   ,absences.ABSENCE_DAYS  ABSENCE_DAYS
                   ,absences.ABSENCE_Hours ABSENCE_HOURS                
                   ,gvv_leg_code LEGISLATIONCODE
                   ,'NULL'  LEGAL_EMPLOYER_NAME
                   ,NULL INTENDTORETFLAG
                   ,NULL ACTUAL_BIRTH_DATE
                   ,Decode(absences.ABSENCE_CATEGORY, 'M',absences.DATE_START ) DUE_DATE
                   ,absences.SSP1_Issued SSP1_Issued
                   ,NULL EFFECTIVESTARTDATE
                   ,NULL EFFECTIVEENDDATE
                   ,A.ASSIGNMENT_ID  ASSIGNMENT_ID
                   ,A.Assignment_type||A.ASSIGNMENT_NUMBER ASSIGNMENT_NUMBER
                   ,A.PERSON_ID PERSONID
                   ,(SELECT Payroll_name
                       FROM PAY_PAYROLLS_F@XXMX_EXTRACT ppf
                      WHERE PAYROLL_ID = A.payroll_id
                        AND gvd_migration_date_from BETWEEN ppf.effective_start_date and ppf.effective_end_date) ATTRIBUTE1 
                   ,0 STARTDATEDURATION
                   ,0 EndDateDuration 
             FROM  xxmx_hcm_current_asg_mv A,           
                   absences
            WHERE  A.BUSINESS_GROUP_ID = absences.BUSINESS_GROUP_ID
              AND  A.PERSON_ID = absences.PERSON_ID
              AND  a.business_group_id = P_BG_ID
              AND ( decode(a.location_id,361,TO_DATE('01/09/2023','dd/mm/yyyy')
                                        ,362,TO_DATE('01/06/2023','dd/mm/yyyy')
                                        ,399,TO_DATE('01/05/2023','dd/mm/yyyy')
                                        ,    TO_DATE('01/01/2023','dd/mm/yyyy')) BETWEEN absences.DATE_START  AND absences.DATE_END
                    OR 
                  absences.DATE_START  > decode(a.location_id,361,TO_DATE('01/09/2023','dd/mm/yyyy')
                                                             ,362,TO_DATE('01/06/2023','dd/mm/yyyy')
                                                             ,399,TO_DATE('01/05/2023','dd/mm/yyyy')
                                                             ,    TO_DATE('01/01/2023','dd/mm/yyyy'))
                  )
              );
        /*Commented by Bharath 03/05/22 - for maternity due date */
		/*INSERT  
        INTO    XXMX_PER_ABSENCE_STG
               ( migration_set_id 		,
				migration_set_name 		,
  ...
  with matabsences as (  
        select Y.ABSENCE_ATTENDANCE_TYPE_ID			
         ...
         FROM    PER_ABSENCE_ATTENDANCES@XXMX_EXTRACT X,
				PER_ABS_ATTENDANCE_REASONS@XXMX_EXTRACT ABSR,
			   PER_ABSENCE_ATTENDANCE_TYPES@XXMX_EXTRACT Y,
               SSP_MATERNITIES@XXMX_EXTRACT SSPM
        WHERE   X.ABSENCE_ATTENDANCE_TYPE_ID = Y.ABSENCE_ATTENDANCE_TYPE_ID
			AND     X.BUSINESS_GROUP_ID = Y.BUSINESS_GROUP_ID 
            and 	ABSR.ABS_ATTENDANCE_REASON_ID(+) = X.ABS_ATTENDANCE_REASON_ID
            AND      SSPM.Maternity_id = X.Maternity_id
            /* Prasad - 10/03/2022  - Commented below code
			AND		( X.DATE_START  BETWEEN trunc(gvd_migration_date_from) AND TRUNC(sysdate)
			OR 		X.DATE_END BETWEEN trunc(gvd_migration_date_from) AND TRUNC(sysdate))
           and UPPER(Y.name)  like '%MATERNITY%'
		   ...
		   --AND		( X.DATE_START  BETWEEN '01-JAN-21' AND TRUNC(sysdate)
			--OR 		X.DATE_END BETWEEN '01-JAN-21'  AND TRUNC(sysdate))
            */
            /* Start - Prasad - 10/03/2022  - Added new code 
            and UPPER(Y.name)  like '%MATERNITY%'
			/*Commented - 07/04/2022*/
			/*AND 
			(
			TO_DATE('01/01/2022','dd/mm/yyyy') BETWEEN X.DATE_START  AND X.DATE_END
			OR 
			X.DATE_START  > TO_DATE('01/01/2022','dd/mm/yyyy')
			)*/
			/*end -Commented - 07/04/2022*/
			/* end - Prasad - 10/03/2022  - Added new code 
            AND 	x.business_group_id = P_BG_ID
		   and Exists (select 1
							FROM xxmx_per_persons_stg per
							WHERE per.person_id = X.person_id
...
--			OR 		X.DATE_END BETWEEN '01-JAN-21'  AND TRUNC(sysdate))
  ...
  SELECT  distinct pt_i_MigrationSetID  migration_set_id                              
                ...
                ,p_bg_id bg_id
				,A.EMPLOYEE_NUMBER   PERSONNUMBER
				,matabsences.ABSENCE_ATTENDANCE_TYPE_ID ABSENCETYPEID
				,NULL  ABSENCESTATUS
				,matabsences.Approval_status  APPROVALSTATUS
				,NVL(matabsences.SICKNESS_START_DATE,matabsences.DATE_START ) STARTDATE
				,NULL  STARTTIME
				,NVL(matabsences.SICKNESS_END_DATE,matabsences.DATE_END) ENDDATE
				,NULL ENDTIME
				,NULL ABSENCETYPE
				,matabsences.absence_type_name  ABSENCENAME
				,matabsences.ABSENCE_CATEGORY ABSENCECATEGORY
				,matabsences.abs_cat_meaning  ABSCAT_MEANING
				,matabsences.DATE_NOTIFICATION  DATE_NOTIFICATION                
				,NVL(matabsences.absence_reason,'DATA MIGRATION') ABSENCEREASON
				,matabsences.abs_reason_meaning ABSREASON_MEANING
				,NVL(matabsences.SICKNESS_START_DATE,matabsences.DATE_START ) CONFIRMEDDATE
				,NULL COMMENTS
				,matabsences.ABSENCE_DAYS  ABSENCE_DAYS
				,matabsences.ABSENCE_Hours ABSENCE_HOURS                
				,gvv_leg_code LEGISLATIONCODE
				,'NULL'  LEGAL_EMPLOYER_NAME
				,matabsences.INTEND_TO_RETURN_FLAG INTENDTORETFLAG
				,matabsences.ACTUAL_BIRTH_DATE ACTUAL_BIRTH_DATE
				,matabsences.DUE_DATE  DUE_DATE
				,matabsences.SSP1_Issued SSP1_Issued
				,NULL EFFECTIVESTARTDATE
				,NULL EFFECTIVEENDDATE
				,A.ASSIGNMENT_ID  ASSIGNMENT_ID
				,A.Assignment_type||A.ASSIGNMENT_NUMBER ASSIGNMENT_NUMBER
               ...
               ,0 EndDateDuration 
			FROM   
			  xxmx_hcm_current_asg_mv A,			  
			  matabsences
			WHERE   A.BUSINESS_GROUP_ID = matabsences.BUSINESS_GROUP_ID
			AND     A.PERSON_ID = matabsences.PERSON_ID
            AND  a.business_group_id = P_BG_ID
			AND (
                 decode(a.location_id,361,TO_DATE('01/09/2021','dd/mm/yyyy') --DK
                                     ,362,TO_DATE('01/06/2021','dd/mm/yyyy')  --FR
                                     ,399,TO_DATE('01/05/2021','dd/mm/yyyy')  --MC
                                     ,    TO_DATE('01/01/2022','dd/mm/yyyy')) BETWEEN matabsences.DATE_START  AND matabsences.DATE_END
		           	OR 
		           	matabsences.DATE_START  > decode(a.location_id,361,TO_DATE('01/09/2021','dd/mm/yyyy')
                                                                  ,362,TO_DATE('01/06/2021','dd/mm/yyyy')
                                                                  ,399,TO_DATE('01/05/2021','dd/mm/yyyy')
            ...
            SELECT distinct pt_i_MigrationSetID                               
                ...
                ,p_bg_id
				,A.EMPLOYEE_NUMBER 
				,nonmatabsences.ABSENCE_ATTENDANCE_TYPE_ID
				,NULL
				,nonmatabsences.Approval_status
				,NVL(nonmatabsences.SICKNESS_START_DATE,nonmatabsences.DATE_START )
				,NULL
				,NVL(nonmatabsences.SICKNESS_END_DATE,nonmatabsences.DATE_END)
				,NULL
				,NULL
				,nonmatabsences.absence_type_name
				,nonmatabsences.ABSENCE_CATEGORY
				,abs_cat_meaning
				,nonmatabsences.DATE_NOTIFICATION
				,NVL(nonmatabsences.absence_reason,'DATA MIGRATION')
				,abs_reason_meaning
				,NVL(nonmatabsences.SICKNESS_START_DATE,nonmatabsences.DATE_START )
				--,x.COMMENTS
				,NULL
				,nonmatabsences.ABSENCE_DAYS LeaveDuration
				,nonmatabsences.ABSENCE_Hours TotalHours
				,gvv_leg_code
				,'NULL' Employer
				,NULL
				,NULL
				...
				,NULL
				,A.ASSIGNMENT_ID 
				,A.Assignment_type||A.ASSIGNMENT_NUMBER
				,A.PERSON_ID
				,(SELECT Payroll_name
				  FROM PAY_PAYROLLS_F@XXMX_EXTRACT ppf
				  WHERE PAYROLL_ID = A.payroll_id
			...
			FROM   nonmatabsences
                ,xxmx_hcm_current_asg_mv A
			WHERE   A.PERSON_ID = nonmatabsences.PERSON_ID
            AND  a.business_group_id = P_BG_ID
			AND (
                 decode(a.location_id,361,TO_DATE('01/09/2021','dd/mm/yyyy')
                                     ,362,TO_DATE('01/06/2021','dd/mm/yyyy')
                                     ,399,TO_DATE('01/05/2021','dd/mm/yyyy')
                                     ,    TO_DATE('01/01/2022','dd/mm/yyyy')) BETWEEN nonmatabsences.DATE_START  AND nonmatabsences.DATE_END
		         OR 
		          nonmatabsences.DATE_START  > decode(a.location_id,361,TO_DATE('01/09/2021','dd/mm/yyyy')
                                                                   ,362,TO_DATE('01/06/2021','dd/mm/yyyy')
                                                                   ,399,TO_DATE('01/05/2021','dd/mm/yyyy')
                                                                   ,    TO_DATE('01/01/2022','dd/mm/yyyy'))
		        )
       ...
       INSERT  
        ...
        SELECT  distinct pt_i_MigrationSetID                               
			...
			FROM    PER_ABSENCE_ATTENDANCES@XXMX_EXTRACT X,
				PER_ABS_ATTENDANCE_REASONS@XXMX_EXTRACT ABSR,
			  PER_ABSENCE_ATTENDANCE_TYPES@XXMX_EXTRACT Y,
			  --PER_ALL_PEOPLE_F@XXMX_EXTRACT A,
			  --PER_ALL_ASSIGNMENTS_F@XXMX_EXTRACT B,
			  XXMX_HCM_CURRENT_PERSON_MV A,
			  xxmx_hcm_current_asg_mv B,			  
			  SSP_MATERNITIES@XXMX_EXTRACT SSPM
			WHERE   X.ABSENCE_ATTENDANCE_TYPE_ID = Y.ABSENCE_ATTENDANCE_TYPE_ID
			AND     X.BUSINESS_GROUP_ID = Y.BUSINESS_GROUP_ID
			AND 	SSPM.Maternity_id = X.Maternity_id
       ...
       INSERT  
		...
		FROM ( SELECT papf.employee_number			
				...
				,XXMX_KIT_PERSON_STG.EXPORT_DFF_SEGMENT_VAL(person_id
                                                ...
                                                ,'gvd_migration_date_from between effective_start_date and effective_end_date')
												Religion		
				from FND_DESCRIPTIVE_FLEXS@XXMX_EXTRACT  ffv 
					,fnd_descr_flex_contexts@XXMX_EXTRACT ffc
					,fnd_application@XXMX_EXTRACT ap
					,FND_DESCR_FLEX_COL_USAGE_TL@XXMX_EXTRACT att
					--,per_all_people_f@XXMX_EXTRACT papf
				...
				AND  papf.business_group_id = p_bg_id 
                 AND papf.system_person_type = 'EMP'
				/*AND  papf.effective_Start_Date IN    (SELECT max(effective_Start_Date)
																  FROM per_all_people_f@XXMX_EXTRACT papf1
																  WHERE papf1.person_id = papf.person_id
																  AND ((
        ...
        SELECT  distinct pt_i_MigrationSetID                               
				...
				,person_id
		FROM ( 
		SELECT   DECODE(papf.system_person_type,'EMP',papf.employee_number, 'APL',papf.applicant_number,'CWK',papf.npw_number,-1) employee_number
				,NVL(papf.original_date_of_hire , papf.start_date) c1_effective_Start_date
				,'Y'
				,papf.person_id
        ...
        AND (gvd_migration_date_from between  papf.effective_Start_Date  and  papf.effective_END_Date 
		     OR papf.effective_Start_Date > gvd_migration_date_from )  -- For Future Dated Workers
		/*AND  papf.effective_Start_Date IN    (SELECT max(effective_Start_Date)
											  FROM per_all_people_f@XXMX_EXTRACT papf1
											  WHERE papf1.person_id = papf.person_id
											  AND ((
    ...
    BEGIN
      ...
      gvd_migration_date_from := TO_DATE(gvv_migration_date_from,'YYYY-MM-DD');
        ...
        SELECT  distinct pt_i_MigrationSetID                               
				...
				,DECODE(xpmsd.system_person_type,'EMP',xpmsd.employee_number, 'APL',xpmsd.applicant_number,'CWK',xpmsd.npw_number,-1)
        FROM XXMX_HCM_CURRENT_PERSON_MV xpmsd
            -- per_all_people_f@XXMX_EXTRACT per_all_people_f
        WHERE 1=1 /*xpmsd.person_id = per_all_people_f.person_id
          AND xpmsd.EFFECTIVE_START_DATE =  per_all_people_f.effective_start_date */
          AND xpmsd.system_person_type in ( 'EMP','CWK')   --Commented on 02AUG23 to fetch PendingWorkers
...
commit;
       ...
       /* FROM     per_all_people_f@XXMX_EXTRACT per_all_people_f
		...
		AND  per_all_people_f.effective_Start_Date IN    (
                            -- DG 14/02. Created table  XXMX_HCM_CURRENT_PERSON_MV to improve performance
                                select EFFECTIVE_START_DATE 
                        from XXMX_HCM_CURRENT_PERSON_MV xpmax
                        where xpmax.person_id = per_all_people_f.person_id
    ...
    BEGIN
        ...
        SELECT   distinct pt_i_MigrationSetID                               
                ...
                ,DECODE(papf.system_person_type,'EMP',papf.employee_number--, 'APL',papf.applicant_number
                ,'CWK',papf.npw_number,-1)
		FROM --per_all_people_f@XXMX_EXTRACT PAPF,
              XXMX_HCM_CURRENT_PERSON_MV papf,
		...
		and (gvd_migration_date_from between pp.date_from and NVL(PP.date_to, '31-DEC-4712')
		     or pp.date_from > gvd_migration_date_from )
		/*AND   PAPF.effective_Start_Date IN    (SELECT max(effective_Start_Date)
                                                          FROM per_all_people_f@XXMX_EXTRACT papf1
                                                          WHERE papf1.person_id = papf.person_id
														  AND ((
		...
		with per_contact as (
		 ...
		 SELECT   distinct pt_i_MigrationSetID                               
				...
				FROM    per_contact papf,
					...
					WHERE  gvd_migration_date_from between a.date_From and NVL(a.date_to,gvd_migration_date_to)
   ...
   COMMIT;
    ...
    BEGIN  
        ...
        SELECT   pt_i_MigrationSetID                               
				...
				,DECODE(papf.system_person_type,'EMP',papf.employee_number--, 'APL',papf.applicant_number
                ,'CWK',papf.npw_number,-1) person_number
		...
		and (gvd_migration_date_from BETWEEN papf.effective_Start_Date and papf.effective_end_date
		     or papf.effective_Start_Date > gvd_migration_date_from);
       /* AND 
		(
                    (
                    TRUNC(papf.effective_start_Date) BETWEEN 
            ...
            ,
                ...
                SELECT  decode (per_all_people_f.per_information25
					...
					,DECODE(per_all_people_f.system_person_type,'EMP',per_all_people_f.employee_number
															  ...
															  , 'CWK',per_all_people_f.npw_number,-1) employee_number
                FROM  XXMX_HCM_CURRENT_PERSON_MV per_all_people_f
                --per_all_people_f@XXMX_EXTRACT per_all_people_f
                    ,hr_all_organization_units@XXMX_EXTRACT horg
                WHERE  1=1
				AND (gvd_migration_date_from BETWEEN per_all_people_f.effective_Start_Date and per_All_people_f.effective_end_Date
				     OR per_all_people_f.effective_Start_Date > gvd_migration_date_from ) --Future Dated Workers Change
				AND    horg.organization_id = per_all_people_f.business_group_id
                AND     horg.name = p_bg_name
...
COMMIT;
    ...
    BEGIN       
		...
		SELECT distinct  pt_i_MigrationSetID                               
				...
				,DECODE(per_all_people_f.system_person_type,'EMP',per_all_people_f.employee_number
				                                           --,'APL',per_all_people_f.applicant_number
														   ,'CWK',per_all_people_f.npw_number,-1)
		...
		AND (gvd_migration_date_from between per_all_people_f.effective_Start_Date and per_all_people_F.effective_end_Date
		     OR per_all_people_f.effective_Start_Date > gvd_migration_date_from ) -- Added on 25Jun2023 for Future Dated Workers
		aND exists ( Select 1
					FROM xxmx_per_persons_stg xxper
					WHERE xxper.person_id = per_all_people_f.person_id);
  ...
  PROCEDURE export_hrt_rating_levels_tl_1
    ...
    BEGIN
        ...
        SELECT UNIQUE pt_i_MigrationSetID --migration_set_id
              ...
              , (DECODE(HCA.USER_PERSON_TYPE,'Contingent Worker','CT','ET')||CO.PERSONNUMBER) WORKTERMSNUMBER
              , CO.PLANNAME --planname
              , 'COVR' ACCRUALTYPE
              ,Decode(hca.location_id, 361 ,TO_DATE('01/09/2023','dd/mm/yyyy')--DK
                                     , 362 ,TO_DATE('01/06/2023','dd/mm/yyyy') --FR
                                     , 399 ,TO_DATE('01/05/2023','dd/mm/yyyy') --MC
                                     --, NULL, NULL
                                     ,     TO_DATE('01/01/2023','dd/mm/yyyy')) procddate
              , CO.VALUE --VALUE
