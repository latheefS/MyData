--------------------------------------------------------
--  DDL for Package Body XXMX_SUPPLIER_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_SUPPLIER_PKG" 
AS
/*
     ******************************************************************************
     **
     ** xxmx_supplier_pkg.plb HISTORY
     ** ------------------------------------
     **
     **   Vsn  Change Date  Changed By           Change Description
     ** -----  -----------  ------------------   -----------------------------------
     **   1.0  12-04-2022   Meenakshi Rajendran  ap_supp_site_assigns_cur - Fetched the following columns liability_distribution,prepayment_distribution,bills_payable_distribution,distribution_set 
	 **                                          from supplier site level
     **   1.1  28-04-2022   Meenakshi Rajendran  ap_suppliers_cur - Modified tax_organization_type,business_relationship column values                                       
     **
	 **   1.2  16-08-2022   Meenakshi Rajendran  ap_suppliers_cur - Included Supplier Payee details at header level
	 **                                          ap_supplier_sites_cur - Included iby_ext_party_pmt_mthds table
	 **
	 **   1.3  03-10-2022   Meenakshi Rajendran  ap_supplier_pmt_instrs_cur - primary_flag has been obtained from iby_account_owners table
	 **
	 **   1.4  10-10-2022   Meenakshi Rajendran  ap_supplier_sites_cur - Updated invoice_match_option,match_approval_level column value as null as requested by CITCO
	 **
	 **   1.5  11-10-2022   Meenakshi Rajendran  ap_supplier_sites_cur - primary_pay has been obtained from ap_supplier_sites_all table
	 **   
	 **   1.6  07-11-2022   Meenakshi Rajendran  ap_supplier_sites_cur - Hold flag and fields has been obtained from Supplier Site LEVEL
	 **                                          ap_supp_site_assigns_cur - Bug Fix
     **   1.7  20-07-2023   Meenakshi Rajendran  ap_supplier_payees_cur - Removed unnecessary table from the extract query
     **
     **   1.8  21-07-2023   Meenakshi Rajendran  ap_supplier_sites_cur - Pay_Group logic modified for EMPLOYEE 
     **
     **   1.9  11-08-2023   Meenakshi Rajendran  ap_supplier_bank_accts_cur,ap_supplier_pmt_instrs_cur - Added ORDER BY clause in bank account & payment instruments cursor
     **
     ******************************************************************************
     */
     ...
     */
     ...
     IS
          ...
          IS
               --
               --
               WITH payee_details AS --1.2
                 (
                 SELECT iepa.payee_party_id
                       ,iepm.payment_method_code
                       ,iepa.exclusive_payment_flag 
                       ...
                       ,iepa.service_level_code
                 FROM  apps.iby_external_payees_all@xxmx_extract iepa
                      ,apps.iby_ext_party_pmt_mthds@xxmx_extract iepm
                 WHERE iepa.ext_payee_id=iepm.ext_pmt_party_id
                 ...
                 AND iepa.payment_function='PAYABLES_DISB'
                      ...
                      'CREATE'                                                        AS import_action --REMOVCE THIS
                      ...
                      ,DECODE(aps.federal_reportable_flag,'Y','US',NULL) AS taxpayer_country  --CHANGED BY VIJAY GADDAM 09-JAN-2023
					  --,aps.num_1099  AS taxpayer_id --commented by VIJAY GADDAM 16-feb-2023
                      ,NVL(aps.individual_1099,aps.num_1099)  AS taxpayer_id --ADDED BY VIJAY GADDAM 09-FEB-2023--aps.num_1099                        
                      ...
                      --,decode(gct_MigrateCISData,'N','N',aps.allow_awt_flag)          AS use_withholding_tax--commented By Gaurav on 12-12-2022
                      --,aps.allow_awt_flag AS use_withholding_tax --Added By Gaurav on 12-12-2022  COMMENTED BY VIJAY GADDAM 21-02-2023
                      /*,CASE WHEN  WTH_DETAILS.MEANING IS NULL THEN 
					      aps.allow_awt_flag                    
                       ELSE
                       'Y'
					    END  AS use_withholding_tax */
                        ,'Y' AS use_withholding_tax              --Added by Veda Poojitha on 03-05-2023 as allow_awt_flag can be Y only if allow_awt_flag for supplier is 'Y'.
                      /*,decode(gct_MigrateCISData,'N',null,(
                        SELECT name
                        FROM  apps.ap_awt_groups@xxmx_extract aag
                        WHERE  aag.group_id = aps.awt_group_id
                       ))  
                       AS withholding_tax_group*/--commented By Gaurav on 12-12-2022                       
                       ,CASE WHEN  WTH_DETAILS.MEANING IS NULL THEN 
					   (
                        SELECT name
                        FROM  apps.ap_awt_groups@xxmx_extract aag
                       ...
                       ELSE
                       WTH_DETAILS.MEANING
					    END  AS withholding_tax_group --ADDED BY VIJAY GADDAM
                     /*(
                        SELECT name
                        FROM  apps.ap_awt_groups@xxmx_extract aag
                        WHERE  aag.group_id = aps.awt_group_id
                       )                                                             AS withholding_tax_group */--Added By Gaurav on 12-12-2022  
					   --COMMENTED BY VIJAY GADDAM
                      ,aps.vat_code                                                   AS supplier_vat_code
                     -- ,aps.vat_registration_num                                    AS tax_registration_number
					  ,NULL                                   AS tax_registration_number --CHANGE BY VIJAY GADDM
                      ,aps.auto_tax_calc_override                                     AS auto_tax_calc_override
--                      ,aps.payment_method_lookup_code                                 AS supplier_payment_method--1.2
                      ,payee_details.payment_method_code                              AS supplier_payment_method
                      ,payee_details.delivery_channel_code                            AS delivery_channel
                      ,payee_details.bank_instruction1_code                           AS bank_instruction_1
                      ,payee_details.bank_instruction2_code                           AS bank_instruction_2
                      ,payee_details.bank_instruction_details                         AS bank_instruction
                      ,payee_details.settlement_priority                              AS settlement_priority
                      ,payee_details.payment_text_message1                            AS payment_text_message_1
                      ,payee_details.payment_text_message2                            AS payment_text_message_2
                      ,payee_details.payment_text_message3                            AS payment_text_message_3
                      ,payee_details.bank_charge_bearer                               AS bank_charge_bearer
                      ,payee_details.payment_reason_code                              AS payment_reason
                      ,payee_details.payment_reason_comments                          AS payment_reason_comments
                      ,payee_details.payment_reason_comments                          AS payment_format
                      /*,''                                                             AS delivery_channel
                      ...
                      ,''                                                             AS payment_format*/--1.2
                      ,aps.attribute_category                                         AS attribute_category
                      ,to_char(aps.vendor_id)                                         AS attribute1
                      ,aps.attribute2                                                 AS attribute2
                      ,aps.attribute3                                                 AS attribute3
                      ,NULL                                                           AS attribute4
                      ,NULL                                                           AS attribute5
                      ,NULL                                                           AS attribute6
                      ,NULL                                                           AS attribute7
                    ...
                    /*  ,''                                                             AS service_level_code
                      ...
                      ,''                                                             AS remit_advice_fax*/--1.2
                      ,payee_details.service_level_code                               AS service_level_code
                      ,payee_details.exclusive_payment_flag                           AS exclusive_payment_flag
                      ,payee_details.remit_advice_delivery_method                     AS remit_advice_delivery_method
                      ,payee_details.remit_advice_email                               AS remit_advice_email
                      ,payee_details.remit_advice_fax                                 AS remit_advice_fax
               FROM    xxmx_supplier_scope                xssv
                      ,apps.ap_suppliers@xxmx_extract  aps
                      --,apps.hz_parties@xxmx_extract    hp
                      ,payee_details
                      ,(SELECT  DISTINCT  h.party_id,MEANING
                 FROM jai_party_regs@xxmx_extract h,
					...
					AND  nvl(effective_to, sysdate+1) >  trunc(sysdate) 
				 )WTH_DETAILS
               WHERE   1 = 1
               AND     aps.vendor_id = xssv.vendor_id
               --AND     aps.party_id  = hp.party_id
               AND aps.party_id=payee_details.payee_party_id(+)
               AND WTH_DETAILS.PARTY_ID(+)=aps.VENDOR_ID
			   AND WTH_DETAILS.PARTY_ID(+)=xssv.VENDOR_ID
               ;
               --
               --
          --** END CURSOR **
          ...
          THEN
               ...
               LOOP
                    ...
                    IN     1..ap_suppliers_tbl.COUNT
                         --
                         INSERT
                         INTO   xxmx_stg.xxmx_ap_suppliers_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
     ...
     IS
          ...
          IS
              ...
              ,contact_points
                    ...
                    (
                     SELECT  DISTINCT
                           ...
                           ,(
                             ...
                             FROM    XXMX_HZ_CONTACT_POINTS      cp_phone
                             WHERE   1 = 1
							 ...
							 AND     cp_phone.phone_line_type         ='GEN'
                           ...
                           ,(
                             ...
                             FROM    XXMX_HZ_CONTACT_POINTS      cp_email
                             WHERE   1 = 1
                           ...
                           ,(
                             ...
                             FROM    XXMX_HZ_CONTACT_POINTS      cp_fax
                             WHERE   1 = 1
                             AND     cp_fax.owner_table_name     = 'HZ_PARTY_SITES'
                             AND     cp_fax.owner_table_id      = dps.party_site_id
                             AND     cp_fax.contact_point_type   = 'PHONE'
							 AND     cp_fax.phone_line_type         ='FAX'
                             AND     cp_fax.status              = 'A'
                             --AND     decode(cp_fax.primary_flag,'N','Y','Y')        = 'Y'
                             AND     ROWNUM                         = 1
                            )                                                      AS telex_number
							,(SELECT  cp_phone.PHONE_EXTENSION
                             FROM    XXMX_HZ_CONTACT_POINTS      cp_phone
                             WHERE   1 = 1
                             AND     cp_phone.owner_table_name(+)     = 'HZ_PARTY_SITES'
                             AND     cp_phone.owner_table_id       = dps.party_site_id
							...
							,(SELECT  cp_fax.PHONE_COUNTRY_CODE
                             FROM    XXMX_HZ_CONTACT_POINTS      cp_fax
                             WHERE   1 = 1
                             AND     cp_fax.owner_table_name     = 'HZ_PARTY_SITES'
                             AND     cp_fax.owner_table_id       = dps.party_site_id
							...
							AND     cp_fax.phone_line_type         ='FAX'
                             AND     cp_fax.status              = 'A'
                            -- AND     decode(cp_fax.primary_flag,'N','Y','Y')        = 'Y'
                             AND     ROWNUM                         = 1)  AS fax_country_code
							,(SELECT  cp_fax.PHONE_AREA_CODE
                             FROM    XXMX_HZ_CONTACT_POINTS     cp_fax
                             WHERE   1 = 1
                             AND     cp_fax.owner_table_name     = 'HZ_PARTY_SITES'
                             AND     cp_fax.owner_table_id      = dps.party_site_id
							...
							AND     cp_fax.phone_line_type         ='FAX'
                             AND     cp_fax.status               = 'A'
                             --AND     decode(cp_fax.primary_flag,'N','Y','Y')        = 'Y'
                             AND     ROWNUM                         = 1)  AS fax_area_code
							,(SELECT  cp_fax.phone_number
                             FROM    XXMX_HZ_CONTACT_POINTS      cp_fax
                             WHERE   1 = 1
                             AND     cp_fax.owner_table_name     = 'HZ_PARTY_SITES'
                            AND     cp_fax.owner_table_id(+)       = dps.party_site_id
               ...
               SELECT  /*+ DRIVING_SITE(hps) */ 'CREATE'                                                                AS import_action
			   --NULL                                                                    AS import_action -- Remove this??
                       ,hl.location_id                                                          AS location_id
                       ,NULL                                                                    AS supplier_site_id
                       ,NULL                                                                    AS party_site_id
                       ,dps.vendor_id                                                           AS supplier_id
                       ,TRIM(dps.vendor_name)                                                   AS supplier_name
                       ,hps.party_site_name                                                     AS address_name
                       ,''                                                                      AS address_name_new
                     --  ,hl.country                                                            AS country --Commented by Sushma chowdary on 08-May-2023
		       ,UPPER(hl.country)                                                       AS country   -- Added by Sushma chowdary on 08-May-2023  to make country uppercase
                       ...
                       --,hl.city                                                               AS city --Commented by Sushma chowdary on 08-May-2023
					   ,UPPER(hl.city)                                                          AS city -- Added by Sushma chowdary on 08-May-2023  to make city uppercase
                       --,hl.state                                                              AS state  --Commented by Sushma chowdary on 08-May-2023
					   ,decode(hl.country ,'US' ,hl.state, 'IN',hl.state, UPPER(hl.state))      AS state  -- Added by Sushma chowdary on 08-May-2023  to make state uppercase
                       --,hl.province                                                           AS province --Commented by Sushma chowdary on 08-May-2023
					   ,decode(hl.country ,'CA' ,hl.province, upper(hl.province))                AS province -- Added by Sushma chowdary on 08-May-2023  to make province uppercase
                       ,hl.county                                                               AS county
                       --,hl.postal_code                                                        AS postal_code --Commented by Sushma chowdary on 08-May-2023
					   ,UPPER(hl.postal_code)                                                   AS postal_code -- Added by Sushma chowdary on 08-May-2023  to make postal_code uppercase
                       ,hl.postal_plus4_code                                                    AS postal_plus_4_code
                       ,(
                         SELECT hps.addressee
                         FROM   apps.hz_party_sites@xxmx_extract hps
                       ...
                       ,cp.phone_extension                                                                      AS phone_extension
                       ,cp.fax_country_code                                                                      AS fax_country_code
                       ,cp.fax_area_code                                                                      AS fax_area_code
                       ,cp.fax                                                         AS fax
                       ,NVL(
                            (
                             SELECT DISTINCT 'Y'
                       ...
                       ,''                                                                      AS remittance_fax
               FROM    distinct_party_sites                   dps
                      ,apps.hz_party_sites@xxmx_extract  hps   -- Join to HZ_PARTIES to get DISTINCT Addresses
                      ,apps.hz_locations@xxmx_extract    hl
                      ,contact_points                         cp
               WHERE   1 = 1
          ...
          THEN
               ...
               LOOP
                    ...
                    IN     1..ap_supplier_addresses_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_addrs_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
                         ...
                         VALUES    (
                                   ...
                                   ,ap_supplier_addresses_tbl(i).supplier_id                 -- supplier_name
                         ...
                         (
                          SELECT aps.vendor_id                                           AS vendor_id
                             ...
                             /*   ,aps.hold_by                                             AS hold_by
                                ,aps.hold_date                                           AS hold_date*/ -- commented by Laxmikanth
                              ,NULL                                            AS hold_by  -- added by Laxmikanth
                                ,NULL                                           AS hold_date -- added by Laxmikanth								
                                ,CASE
                                ...
                                ,aps.exclusive_payment_flag                              AS exclusive_payment_flag
                          FROM   ap_suppliers@xxmx_extract  aps
                         )
                 ,r12_payee_details
                  AS
                         ( 
                         SELECT  iepa.supplier_site_id                           AS supplier_site_id
                                 ,iepm.payment_method_code                        AS payment_method_code
                                 ...
                                 ,iepa.remit_advice_email                         AS remittance_email
                                 ,iepa.remit_advice_fax                           AS remittance_fax
                                 ,iepa.service_level_code                         AS service_level_code
                          FROM    apps.iby_external_payees_all@xxmx_extract  iepa
                                 ,apps.iby_ext_party_pmt_mthds@xxmx_extract  iepm --1.2
                          WHERE   1 = 1
                          AND     iepa.inactive_date IS NULL
                          AND     iepa.ext_payee_id=iepm.ext_pmt_party_id
                          AND     iepm.primary_flag='Y'
                          AND     iepa.payment_function='PAYABLES_DISB'
                         )
                  SELECT  DISTINCT
                         ...
                         ,assa.primary_pay_site_flag                                               AS primary_pay--1.5
                         /*,DECODE(
                                 ROW_NUMBER() OVER (
                         ...
                         ,(
                           ...
                           FROM   apps.wsh_carriers_v@xxmx_extract wcv
                           WHERE  1 = 1
                         ...
                         ,(
                           SELECT /*+ DRIVING_SITE(wcs) */ wcs.mode_of_transport
                           FROM    apps.wsh_carriers_v@xxmx_extract       wcv
                                  ,apps.wsh_carrier_services@xxmx_extract wcs
                           WHERE   1 = 1
                         ...
                         ,(
                           SELECT segment1
                           FROM   apps.pos_sup_products_services@xxmx_extract psps
                           WHERE  1 = 1
                         ...
                         ,dfs.match_approval_level                                                 AS match_approval_level*/--1.4
                         ,null                                                                     AS invoice_match_option--1.4
                         ,null                                                                     AS match_approval_level--1.4
                         ,assa.payment_currency_code                                               AS payment_currency
                         ,assa.payment_priority                                                    AS payment_priority
--                         ,assa.pay_group_lookup_code                                               AS pay_group --commented for 1.8
                         ,decode(assa.pay_group_lookup_code,'EMPLOYEE','Employee',assa.pay_group_lookup_code) AS pay_group --Added for 1.8
                         ,(
                           SELECT att2.tolerance_name
                           FROM   apps.ap_tolerance_templates@xxmx_extract att2
                           WHERE  1 = 1
...
--                         ,dfs.hold_future_payments_flag                                            AS hold_unvalidated_invoices--1.6
						 ,assa.hold_future_payments_flag                                            AS hold_unvalidated_invoices
						...
						,NULL                                                           AS vat_code --CHANGED BY VIJAY GADDAM 10/02/2023
                         --,assa.vat_registration_num                                                AS tax_registration_number
						 ,NULL                                              AS tax_registration_number --CHANGED BY VIJAY GADDAM 10/02/2023
--                         ,assa.payment_method_lookup_code                                          AS payment_method--1.2
                         ,r12pd.payment_method_code                                                AS payment_method
                         ,r12pd.delivery_channel                                                   AS delivery_channel
                         ,r12pd.bank_instruction_1                                                 AS bank_instruction_1
                         ,r12pd.bank_instruction_2                                                 AS bank_instruction_2
                         ,r12pd.bank_instruction                                                   AS bank_instruction
                         ,r12pd.settlement_priority                                                AS settlement_priority
                         ,r12pd.payment_text_message_1                                             AS payment_text_message_1
                         ,r12pd.payment_text_message_2                                             AS payment_text_message_2
                         ,r12pd.payment_text_message_3                                             AS payment_text_message_3
                         ,r12pd.bank_charge_bearer                                                 AS bank_charge_bearer
                         ,r12pd.payment_reason                                                     AS payment_reason
                         ,r12pd.payment_reason_comments                                            AS payment_reason_comments
                         ,r12pd.delivery_method                                                    AS delivery_method
                         ,r12pd.remittance_email                                                   AS remittance_email
                         ,r12pd.remittance_fax                                                     AS remittance_fax
                         ,NULL                                                                     AS attribute_category
                         ,NULL                                                                     AS attribute1
                         ,NULL                                                                     AS attribute2
                         ,NULL                                                                     AS attribute3
                         ...
                         ,''                                                                       AS invoice_channel
                         ,r12pd.service_level_code                                                 AS payee_service_level_code
--                         ,dfs.exclusive_payment_flag                                               AS exclusive_payment_flag--1.2
                         ,r12pd.exclusive_payment_flag                                             AS exclusive_payment_flag
                  FROM    xxmx_supplier_scope                              xssv
                         ,apps.ap_supplier_sites_all@xxmx_extract       assa
                         ,apps.hz_party_sites@xxmx_extract              hps
                         ,apps.ap_terms_tl@xxmx_extract                 att
                         ,apps.hz_locations@xxmx_extract                hl
                         ,defaults_from_supplier                             dfs
                         ,r12_payee_details                                  r12pd
                  WHERE   1=1
                  ...
                  AND     r12pd.supplier_site_id(+) = assa.vendor_site_id;
          ...
          THEN
               ...
               LOOP
                    ...
                    IN 1..ap_supplier_sites_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supplier_sites_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
                         ...
                         VALUES  (
                                   ...
                                   ,ap_supplier_sites_tbl(i).remittance_email
     ...
     PROCEDURE ap_supp_3rd_party_rels_stg
      ...
      IS
          ...
          IS
               ...
               FROM    xxmx_supplier_scope                               xssv
                      ,apps.ap_supplier_sites_all@xxmx_extract        assa
                      ,apps.iby_ext_payee_relationships@xxmx_extract  iepr
                      ,apps.hz_parties@xxmx_extract                   supplier_party
                      ,apps.hz_parties@xxmx_extract                   remit_to_party
                      ,apps.ap_supplier_sites_all@xxmx_extract        remit_to_site
               WHERE   1 = 1
               AND     assa.org_id                  = xssv.org_id
               AND     assa.vendor_id               = xssv.vendor_id
               AND     assa.vendor_site_id          = xssv.vendor_site_id
          ...
          THEN
               ...
               LOOP
                    ...
                    IN 1..ap_supp_3rd_party_rels_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_3rd_pty_rels_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
...
--                       )                                                                        AS withholding_tax_group
                      --,decode(gct_MigrateCISData,'N','N',aps.allow_awt_flag)                    AS use_withholding_tax --commented by gaurav on 12-12-2022
                      --,assa.allow_awt_flag                   AS use_withholding_tax--added by Gaurav on 12-12-2022 COMMENTED BY VIJAY GADDAM 21-02-2023
                       ,CASE WHEN  WTH_DETAILS_ASSIGN.MEANING IS NULL THEN 
					      assa.allow_awt_flag                    
                       ELSE
                       'Y'
					    END  AS use_withholding_tax
                      /*,decode(gct_MigrateCISData,'N',null,(
                       ...
                       ) COMMENTED BY VIJAY GADDAM                                                                    AS withholding_tax_group */   --added by Gaurav on 12-12-2022					   
					  ,CASE WHEN  WTH_DETAILS_ASSIGN.MEANING IS NULL THEN 
					   (
                        SELECT name
                        FROM  apps.ap_awt_groups@xxmx_extract aag
                       ...
                       ELSE
                       WTH_DETAILS_ASSIGN.MEANING
					    END  AS withholding_tax_group   ,(
                        SELECT concatenated_segments
                        FROM   apps.gl_code_combinations_kfv@xxmx_extract gcc
                        WHERE  1 = 1
                        AND    gcc.code_combination_id = assa.accts_pay_code_combination_id
                       )                                                                        AS liability_distribution --1.0
                      ,(
                        SELECT concatenated_segments
                        FROM   apps.gl_code_combinations_kfv@xxmx_extract gcc
                        WHERE  1 = 1
                        AND    gcc.code_combination_id = assa.prepay_code_combination_id
                       )                                                                        AS prepayment_distribution --1.0
                      ,(
                        SELECT concatenated_segments
                        FROM   apps.gl_code_combinations_kfv@xxmx_extract gcc
                        WHERE  1 = 1
                        AND    gcc.code_combination_id = assa.future_dated_payment_ccid
                       )                                                                        AS bills_payable_distribution--1.0
                      ,(
                        SELECT distribution_set_name
                        FROM   apps.ap_distribution_sets_all@xxmx_extract ads
                        WHERE  1 = 1
                        AND    ads.distribution_set_id = assa.distribution_set_id
                       )                                                                        AS distribution_set--1.0
                      ,assa.inactive_date                                                       AS inactive_date
               FROM    xxmx_supplier_scope                             xssv
                      ,apps.ap_supplier_sites_all@xxmx_extract      assa
                      ,apps.ap_suppliers@xxmx_extract               aps
                    /*,apps.hz_party_sites@xxmx_extract             hps
                      ...
                      ,apps.hz_locations@xxmx_extract               hl*/--1.6
                      ,apps.hr_locations_all@xxmx_extract           bill_loc
                      ,apps.hr_locations_all@xxmx_extract           ship_loc
               ,(SELECT  DISTINCT  h.party_site_id,h.party_id,MEANING
                 FROM jai_party_regs@xxmx_extract h,
					jai_party_reg_lines@xxmx_extract l,
					ja_lookups@xxmx_extract sec_code					
					WHERE 1=1					
               ...
               WHERE   1 = 1
			   AND WTH_DETAILS_ASSIGN.party_site_id(+)=assa.vendor_site_id
			   AND WTH_DETAILS_ASSIGN.party_id(+)=aps.vendor_id
			   AND WTH_DETAILS_ASSIGN.party_id(+)=xssv.vendor_id
			   AND WTH_DETAILS_ASSIGN.party_site_id(+)=xssv.vendor_site_id
               AND     assa.org_id          = xssv.org_id
               AND     assa.vendor_id       = xssv.vendor_id
               AND     assa.vendor_site_id  = xssv.vendor_site_id
              ...
              /*AND     hps.party_site_id    = assa.party_site_id
               ...
               AND     ship_loc.location_id = assa.ship_to_location_id(+);*/--1.6
          ...
          THEN
               ...
               LOOP
                    ...
                    IN 1..ap_supp_site_assigns_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_site_assigns_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
                         ...
                         VALUES
                                   ...
                                   ,ap_supp_site_assigns_tbl(i).operating_unit_name         -- operating_unit_name
     ...
     PROCEDURE ap_supplier_contacts_stg
      ...
      IS
          ...
          IS
                        ...
                        ,CASE
                              WHEN apsc.phone IS NOT NULL
                                   THEN (
                                         ...
                                         FROM   apps.hz_phone_country_codes@xxmx_extract hpc
                                         WHERE  1 = 1
               ...
               FROM      xxmx_supplier_scope                         xssv
                        ,apps.ap_supplier_sites_all@xxmx_extract  assa
                        ,apps.hz_parties@xxmx_extract             person_party
                        ,apps.hz_parties@xxmx_extract             relationship_party
                        ,apps.ap_supplier_contacts@xxmx_extract   apsc
               WHERE     1 = 1
               AND       assa.org_id                 = xssv.org_id
               AND       assa.vendor_id              = xssv.vendor_id
               AND       assa.vendor_site_id         = xssv.vendor_site_id
          ...
          THEN
               ...
               LOOP
                    ...
                    IN     1..ap_supplier_contacts_tbl.COUNT
                         --
                         INSERT
                         INTO   xxmx_stg.xxmx_ap_supp_contacts_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
      ...
      IS
          ...
          --
            ...
            IS
                  ...
                  --
                  SELECT  'CREATE' import_action
				  --NULL     import_action                                            -- REMOVE THIS ??
                         ,xssv.vendor_id                                                    AS supplier_id
                         ,xssv.vendor_name                                                  AS supplier_name
                         ,assa.vendor_site_id                                               AS supplier_site_id
          ...
          THEN
               ...
               LOOP
                    ...
                    IN     1..ap_supplier_contact_addrs_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_cont_addrs_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
     ...
     */
     ...
     IS
          ...
          IS
               --
               SELECT  DISTINCT
                       supplier_payees.SUPPLIER_ID                   AS SUPPLIER_ID
                      ,supplier_payees.SUPPLIER_NAME                 AS SUPPLIER_NAME
                      ,supplier_payees.supplier_site_id              AS supplier_site_id
                      ,supplier_payees.supplier_site                 AS SUPPLIER_site
...
--                      ,supplier_payees.vendor_name                   AS supplier_name
               FROM   (
                       SELECT /*+ RULE */
                             xssv.vendor_id                          AS supplier_id
                            ,TRIM(xssv.vendor_name)                  AS supplier_name
                            ,xssv.vendor_site_id                     AS supplier_site_id
                            ,xssv.vendor_site_code                   AS supplier_site
                            ,iepa.ext_payee_id                  AS temp_ext_payee_id
                              ,xssv.operating_unit_name           AS operating_unit_name
...
--                              ,xssv.vendor_name                   AS vendor_name
                       FROM    xxmx_supplier_scope                           xssv
                              ,apps.iby_external_payees_all@xxmx_extract  iepa
--                              ,apps.iby_ext_bank_accounts@xxmx_extract    ieba    --Commented for 1.7
--                              ,apps.iby_pmt_instr_uses_all@xxmx_extract   ipiua   --Commented for 1.7
--                              ,apps.ce_bank_branches_v@xxmx_extract       cbbv    --Commented for 1.7
                       WHERE   1 = 1
                       AND     iepa.payee_party_id          = xssv.party_id
                       AND     iepa.supplier_site_id        = xssv.vendor_site_id
                       AND     SYSDATE                      < NVL(iepa.inactive_Date, SYSDATE+1)
                       and iepa.party_site_id is not null                             --refer doc ID 2851662.1 in support.oracle.com
                     /*  AND     ipiua.ext_pmt_party_id       = iepa.ext_payee_id
                       ...
                       AND     cbbv.pk_id                   = ieba.branch_id*/--Commented for 1.7
--                       AND     ipiua.order_of_preference    = --<= pn_i_OrderOfPreferenceLimit   -- DH 25/01/2022 amended to cater for accounts with not order of preference=1
--                       (
--                           SELECT MIN(ip.order_of_preference)
--                           FROM   apps.iby_pmt_instr_uses_all@xxmx_extract ip
--                           WHERE  ip.ext_pmt_party_id       = iepa.ext_payee_id
--                           AND    ip.instrument_type        = 'BANKACCOUNT'
--                           AND    ip.payment_function       = 'PAYABLES_DISB'
--                           AND    SYSDATE                   BETWEEN NVL(ip.start_date,SYSDATE-1)
--                                                            AND NVL(ip.end_Date,SYSDATE+1)
--                           AND    ieba.ext_bank_account_id     = ip.instrument_id                       
--                       )                                              
...
/*
                       SELECT  
                             xssv.vendor_id                          AS supplier_id
                            ,TRIM(xssv.vendor_name)                  AS supplier_name
                            ,NULL                                    AS supplier_site_id   --xssv.vendor_site_id    changed 06/05/2022
                            ,NULL                                    AS supplier_site      --xssv.vendor_site_code  changed 06/05/2022
                            ,iepa.ext_payee_id                  AS temp_ext_payee_id
...
--                              ,xssv.vendor_name                   AS vendor_name
                       FROM    xxmx_supplier_scope                          xssv
                              ,apps.iby_ext_bank_accounts@xxmx_extract   ieba
                              ,apps.iby_pmt_instr_uses_all@xxmx_extract  ipiua
                              ,apps.iby_external_payees_all@xxmx_extract iepa
                              ,apps.ce_bank_branches_v@xxmx_extract      cbbv
                              ,apps.ap_suppliers@xxmx_extract            aps
                       WHERE   1 = 1
                       AND     iepa.payee_party_id          = xssv.party_id
                       AND     iepa.supplier_site_id       IS NULL                                /* Supplier Level Bank Account 
                       AND     SYSDATE                      < NVL(iepa.inactive_Date, SYSDATE+1)
                       AND     ipiua.ext_pmt_party_id       = iepa.ext_payee_id
                       AND     ipiua.instrument_type        = 'BANKACCOUNT'
                       AND     ipiua.payment_function       = 'PAYABLES_DISB'
          ...
          THEN
               ...
               */
               ...
               OPEN ap_supplier_payees_cur
                         (
                             vn_OrderOfPreferenceLimit  -- DH amended as  
                         );
               --
               --
               gvv_ProgressIndicator := '0060';
               ...
               LOOP
                    ...
                    IN     1..ap_supplier_payees_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_payees_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
...
--                                   ,supplier_name
                         ...
                         VALUES    (
                                   ...
                                   ,gvt_MigrationSetName                                    -- migration_set_name
                                   ,'EXTRACTED'                                             -- migration_status
                              ,ap_supplier_payees_tbl(i).supplier_id
                              ,ap_supplier_payees_tbl(i).supplier_name
                              ,ap_supplier_payees_tbl(i).supplier_site_id
                              ,ap_supplier_payees_tbl(i).supplier_site
     ...
     */
     ...
     IS
          ...
          IS
               --
               SELECT   DISTINCT
                        bank_accounts.SUPPLIER_ID                   AS SUPPLIER_ID
                       ,bank_accounts.SUPPLIER_NAME                 AS SUPPLIER_NAME
                       ,bank_accounts.supplier_site_id              AS supplier_site_id
                       ,bank_accounts.supplier_site                 AS SUPPLIER_site
                       ,bank_accounts.temp_ext_payee_id             AS temp_ext_party_id
                       ,bank_accounts.temp_ext_bank_acct_id         AS temp_ext_bank_acct_id
                       ,bank_accounts.bank_name                     AS bank_name
                       ,bank_accounts.branch_name                   AS branch_name
                       ,bank_accounts.country_code                  AS country_code
                       ,bank_accounts.bank_account_name             AS bank_account_name
                       ,bank_accounts.bank_account_num              AS bank_account_num
                       ,bank_accounts.currency_code                 AS currency_code
                       ,bank_accounts.foreign_payment_use_flag      AS foreign_payment_use_flag
                       ,bank_accounts.start_date                    AS start_date
                       ,bank_accounts.end_date                      AS end_date
                       ,bank_accounts.iban                          AS iban
                       ,bank_accounts.check_digits                  AS check_digits
                       ,bank_accounts.bank_account_name_alt         AS bank_account_name_alt
                       ,bank_accounts.bank_account_type             AS bank_account_type
                       ,bank_accounts.account_suffix                AS account_suffix
                       ,bank_accounts.description                   AS description
                       ,bank_accounts.agency_location_code          AS agency_location_code
                       ,bank_accounts.exchange_rate_agreement_num   AS exchange_rate_agreement_num
                       ,bank_accounts.exchange_rate_agreement_type  AS exchange_rate_agreement_type
                       ,bank_accounts.exchange_rate                 AS exchange_rate
                       ,bank_accounts.secondary_account_reference   AS secondary_account_reference
                       ,NULL                                        AS attribute_category
                       ,NULL                                        AS attribute1
                       ,NULL                                        AS attribute2
                       ,NULL                                        AS attribute3
                       ...
                       ,NULL                                        AS attribute15
                       ,operating_unit_name
                       ,bank_accounts.order_of_preference --1.9
                FROM  (
                       SELECT /*+ RULE */
                             xssv.vendor_id                                                            AS supplier_id
                            ,xssv.vendor_name                                                  AS supplier_name
                            ...
                            ,iepa.ext_payee_id                                           AS temp_ext_payee_id
                              ,TO_NUMBER(
                                       ...
                                       ||xssv.org_id
                                        )                                                  AS temp_ext_bank_acct_id --TO_NUMBER(ieba.ext_bank_account_id||iepa.supplier_site_id)
...
/*                       SELECT  
                              xssv.vendor_id                                               AS supplier_id
                             ,TRIM(xssv.vendor_name)                                       AS supplier_name
                             ,NULL                                                         AS supplier_site_id --xssv.vendor_site_id
                             ,NULL                                                         AS supplier_site  --xssv.vendor_site_code
                             ,iepa.ext_payee_id                                            AS temp_ext_payee_id
                              ,TO_NUMBER(iepa.ext_payee_id||ieba.ext_bank_account_id||xssv.org_id)   AS temp_ext_bank_acct_id --TO_NUMBER(ieba.ext_bank_account_id||iepa.supplier_site_id)
                              ,cbbv.bank_name                                              AS bank_name
                              ,cbbv.bank_branch_name                                       AS branch_name
                              ,ieba.country_code                                           AS country_code
                              ,ieba.bank_account_name                                      AS bank_account_name
...
*/
                      ...
                      ORDER BY bank_accounts.temp_ext_payee_id,bank_accounts.order_of_preference;--1.9
                -- remove order by as extract does not need to be ordered
               --ORDER BY bank_accounts.temp_ext_payee_id
                       --,bank_accounts.temp_ext_bank_acct_id;
               --
               --
          --** END CURSOR **
          --
               ...
               */
               ...
               LOOP
                    ...
                    IN     1..ap_supplier_bank_accts_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_bank_accts_stg
                              (
                               migration_set_id
                              ,migration_set_name
                              ,migration_status
                         ...
                         VALUES
                              ...
                              ,gvt_MigrationSetName                                        -- migration_set_name
                              ,'EXTRACTED'                                                 -- migration_status
                              ,ap_supplier_bank_accts_tbl(i).supplier_id
                              ,ap_supplier_bank_accts_tbl(i).supplier_name
                              ,ap_supplier_bank_accts_tbl(i).supplier_site_id
                              ,ap_supplier_bank_accts_tbl(i).supplier_site
     ...
     IS
          ...
          IS
               ...
               SELECT  DISTINCT
                       ...
                       ,supplier_payees.supplier_site_id              AS supplier_site_id
                       ,supplier_payees.supplier_site                 AS SUPPLIER_site
                       ,supplier_payees.temp_ext_payee_id                                                  AS temp_ext_party_id
                      ,supplier_payees.temp_ext_bank_acct_id                                              AS temp_ext_bank_acct_id
                      ,supplier_payees.temp_pmt_instr_use_id                                              AS temp_pmt_instr_use_id
/*
                      ,DECODE(
                              ROW_NUMBER() OVER (
                                                 PARTITION BY supplier_payees.temp_ext_payee_id
                                                             ,supplier_payees.temp_ext_bank_acct_id
                                                 ORDER BY     supplier_payees.temp_ext_payee_id     DESC
                                                             ,supplier_payees.temp_ext_bank_acct_id DESC
                                                             ,supplier_payees.temp_pmt_instr_use_id DESC
                                                )
                              ,1 ,'Y'
                                 ,'N'                                                                      /* ISV 06/05/2021 : Replaced NULL with 'N' as primary flag is mandatory in Fusion 
                             )                                                                             AS primary_flag
*/
                      ,supplier_payees.primary_flag                                                        AS primary_flag
                      ,supplier_payees.acct_assig_start_date                                               AS acct_assig_start_date
                      ,supplier_payees.acct_assig_end_date                                                 AS acct_assig_end_date
                      ,operating_unit_name
                      ,supplier_payees.order_of_preference --1.9
               FROM   (
                       SELECT /*+ RULE */ 
                             xssv.vendor_id                          AS supplier_id
                            ,TRIM(xssv.vendor_name)                  AS supplier_name
                            ...
                            ,iepa.ext_payee_id                                           AS temp_ext_payee_id
                              ...
                              ,ieao.primary_flag                                           AS primary_flag--1.3
                              ,ipiua.start_date                                            AS acct_assig_start_date
                              ,ipiua.end_date                                              AS acct_assig_end_date
                              ,xssv.operating_unit_name                                    AS operating_unit_name
                              ,ipiua.order_of_preference --1.9
                       FROM    xxmx_supplier_scope                      xssv
                              ,apps.iby_account_owners@xxmx_extract       ieao
                              ,apps.iby_external_payees_all@xxmx_extract  iepa
                              ,apps.iby_ext_bank_accounts@xxmx_extract    ieba
                              ,apps.iby_pmt_instr_uses_all@xxmx_extract   ipiua
                              ,apps.ce_bank_branches_v@xxmx_extract       cbbv
                       ...
                       AND     ieba.ext_bank_account_id     = ipiua.instrument_id
                       AND     cbbv.pk_id                   = ieba.branch_id
                       AND     ieao.ext_bank_account_id     = ieba.ext_bank_account_id
                       AND     ieao.account_owner_party_id  = iepa.payee_party_id
                       AND     nvl(ieao.end_date,ipiua.start_date) between ipiua.start_date and nvl(ipiua.end_date,sysdate+1)                       
--                       AND     ipiua.order_of_preference    = --<= pn_i_OrderOfPreferenceLimit   -- DH 25/01/2022 amended to cater for accounts with not order of preference=1
--                       (
--                           SELECT MIN(ip.order_of_preference)
--                           FROM   apps.iby_pmt_instr_uses_all@xxmx_extract ip
...
--               ORDER BY  supplier_payees.temp_ext_payee_id
--                        ,supplier_payees.temp_ext_bank_acct_id
--                        ,supplier_payees.temp_pmt_instr_use_id;
               --
               --
          --** END CURSOR **
          --
               ...
               */
               ...
               LOOP
                    ...
                    IN     1..ap_supplier_pmt_instrs_tbl.COUNT
                         --
                         INSERT
                         INTO xxmx_stg.xxmx_ap_supp_pmt_instrs_stg
                                   (
                                    migration_set_id
                                   ,migration_set_name
                                   ,migration_status
                         ...
                         VALUES  (
                                   ...
                                   ,gvt_MigrationSetName                                 -- migration_set_name
                                   ,'EXTRACTED'                                          -- migration_status
                              ,ap_supplier_pmt_instrs_tbl(i).supplier_id
                              ,ap_supplier_pmt_instrs_tbl(i).supplier_name
                              ,ap_supplier_pmt_instrs_tbl(i).supplier_site_id
                              ,ap_supplier_pmt_instrs_tbl(i).supplier_site
