--------------------------------------------------------
--  DDL for Procedure XXMX_IREC_CANDIDATE_XFM_LOAD_PRC_UAT
--------------------------------------------------------
set define off;

  CREATE OR REPLACE EDITIONABLE PROCEDURE "XXMX_CORE"."XXMX_IREC_CANDIDATE_XFM_LOAD_PRC_UAT" 
AS
  CURSOR CANDIDATE_DATA_CUR 
     IS  SELECT UNIQUE case when length(REPLACE(SUBSTR(can.EMAIL,1,INSTR(can.EMAIL,'@',1,1)-1),'.','_'))>30 
             ...
             nvl(can.ADDRESS,hrl.Address_Line_1||','||hrl.address_line_2) address,
             nvl(case when length(can.city)>30
             then substr(can.city,1,instr(can.city,',',1,1)-1)
             else can.city
             end, hrl.town_or_city) city
              ...
              nvl((select g.country_code from XXMX_HCM_GEO_MAP g 
                  where g.country = can.country),hrl.country) COUNTRY
            , nvl(CASE WHEN  UPPER(can.country) IN ('IN','INDIA') 
                ...
                ELSE can.POSTALCODE 
                END ,hrl.postal_code) POSTALCODE
              , can.REQUISITIONID
	          , CASE
              ...
              , CASE 
                ...
                END AREA_CODE
              ,/* CASE 
                WHEN UPPER(can.COUNTRY) ='CANADA'
                THEN 'CA'
                WHEN UPPER(can.COUNTRY) = 'TRINIDAD AND TOBAGO'
                THEN 'TT'
                WHEN UPPER(can.COUNTRY) = 'UNITED STATES'
                THEN 'US'
                WHEN UPPER(can.COUNTRY) = 'UNITED_STATES'
                THEN 'US'
                WHEN UPPER(can.COUNTRY) = 'BERMUDA'
                THEN 'BM'
                WHEN UPPER(can.COUNTRY) = 'CAYMAN ISLANDS'
                THEN 'KY'
				WHEN UPPER(can.COUNTRY) = 'CAYMAN_ISLANDS'
                THEN 'KY'
                WHEN UPPER(can.COUNTRY) = 'BAHAMAS'
                THEN 'BS'
                WHEN UPPER(can.COUNTRY) = 'SAINT LUCIA'
                THEN 'LC'
                WHEN UPPER(can.COUNTRY) = 'DOMINICA'
                THEN 'DM'
                WHEN UPPER(can.COUNTRY) = 'JAMAICA'
                THEN 'JM'
                WHEN UPPER(can.COUNTRY) = 'BARBADOS'
				THEN 'BB'
                WHEN UPPER(can.COUNTRY) = 'UNITED KINGDOM' THEN 'GB'
                WHEN UPPER(can.COUNTRY) = 'ITALY' THEN 'IT'
                WHEN UPPER(can.COUNTRY) = 'AUSTRALIA' THEN 'AU'
                WHEN UPPER(can.COUNTRY) = 'RUSSIAN FEDERATION' THEN 'RU'
                ELSE NULL END */
                (select g.country_code from XXMX_HCM_GEO_MAP g where g.country = can.country) legislation_CODE
              , CASE 
			  ...
			  , CASE
                  ...
                  WHEN can.status = 'Submitted to Hiring Manager'   THEN  'SCREENING'
                  ...
                  WHEN can.status = 'First Interveiw'   THEN  'INTERVIEW_AND_SELECTION'
                ...
                CASE
                  ...
                  WHEN can.status = 'Submitted to Hiring Manager'   THEN  '300000017023892'
                  ...
                  WHEN can.status = 'First Interveiw'   THEN  'INTERVIEW_SCHEDULED'
               ...
               'Agency','Agency Name','Employee Referral',
               'Employee Referral',
               ...
               -- 'Agency' SOURCE_MEDIUM
                ...
                ,can.OFFERLETTER
                ,can.ORIGINALDOCUMENT ,can.coverletterfile
        ...
        AND ((TRUNC(TO_DATE(submitted,'MM/DD/RRRR HH24:MI')) 
          ...
          /*  AND (upper(source) like 'EMP%REFERRAL%' or upper(source) like '%AGENCY%') 
            AND TRUNC(TO_DATE(submitted,'MM/DD/RRRR HH24:MI')) 
            BETWEEN  TRUNC(TO_DATE('03/31/2022','MM/DD/RRRR')) 
            AND TRUNC(TO_DATE('04/01/2023','MM/DD/RRRR'))
            
           */ (upper(can.status) in ('NEW'
...
END XXMX_IREC_CANDIDATE_XFM_LOAD_PRC_UAT;
