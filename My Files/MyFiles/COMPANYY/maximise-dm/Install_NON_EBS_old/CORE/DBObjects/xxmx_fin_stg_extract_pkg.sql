create or replace PACKAGE     "XXMX_FIN_STG_EXTRACT_PKG" AUTHID CURRENT_USER
AS
     --
     /*
     *****************************************************************************
...
--
--
create or replace NONEDITIONABLE PACKAGE BODY             "XXMX_FIN_STG_EXTRACT_PKG" 
AS
     --
     /*
     **********************
     ...
     */
     ...
     IS
          ...
          IS
               --
               --
               SELECT  xmm.sub_entity, STG_TABLE
               FROM    xxmx_migration_metadata  xmm
               WHERE   1 = 1
               AND     xmm.application_suite = pt_ApplicationSuite
               AND     xmm.application       = pt_Application
          ...
          --
           ...
           END IF; --** IF gvt_StgPopulationMethod  = 'DATA_FILE'
              ...
              */
              ...
              LOOP
                    BEGIN 
                        gv_sql:= 'SELECT distinct FILE_SET_ID
                                  FROM XXMX_STG.'||SubEntity_rec.stg_table||
                                ' WHERE migration_set_id ='||gvt_MigrationSetID;
                                 
                        EXECUTE IMMEDIATE gv_sql INTO  gvt_FileSetID;
                        DBMS_OUTPUT.PUT_LINE('TEST '||gvt_FileSetID);
                    EXCEPTION
                        WHEN OTHERS THEN
                            gvv_ProgressIndicator := '0085';     
                            gvt_ModuleMessage := 'ERROR while getting File_set_id';
                    END;
...
/
--
SHOW ERRORS XXMX_FIN_STG_EXTRACT_PKG;
/