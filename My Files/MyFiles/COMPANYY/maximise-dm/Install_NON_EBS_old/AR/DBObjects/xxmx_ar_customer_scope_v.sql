...
/
  ...
  CREATE MATERIALIZED VIEW "XXMX_CORE"."XXMX_CUSTOMER_SCOPE_V" ("ORG_ID", "CUST_ACCOUNT_ID", "ACCOUNT_NUMBER", "ACCOUNT_PARTY_ID", "ACCOUNT_STATUS", "CUST_ACCT_SITE_ID", "CUST_ACCT_SITE_STATUS", "PARTY_SITE_ID", "SITE_PARTY_ID", "LOCATION_ID")
          ...
          (
           ...
           AND     EXISTS (
                           ...
                           FROM   apps.ar_payment_schedules_all@MXDM_NVIS_EXTRACT  apsa
                                 ,xxmx_parameter_conversions                     op_xpc
                           WHERE  1 = 1
                           AND   ( (apsa.status               = 'OP'  AND    apsa.amount_due_remaining > 0)
                                       OR   TRUNC(apsa.last_update_date) >= op_xpc.customer_scope_start_date
                                 )
