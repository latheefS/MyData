...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_ADDRS_STG 
   ( 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_BANK_ACCTS_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_CONT_ADDRS_STG 
   ( 
     file_set_id                    VARCHAR2(30), 
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_CONTACTS_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_PAYEES_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_PMT_INSTRS_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_SITE_ASSIGNS_STG 
   ( 
     file_set_id                    VARCHAR2(30),     
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPPLIER_SITES_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPPLIERS_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--------------------------------------------------------
  ...
  CREATE TABLE XXMX_AP_SUPP_3RD_PTY_RELS_STG 
   ( 
     file_set_id                    VARCHAR2(30),
     migration_set_id NUMBER,
     migration_set_name VARCHAR2(100),                 
...
--
PROMPT
PROMPT Creating View xxmx_supplier_scope_v
PROMPT
--
--------------------------------------------------------
...
/*CREATE OR REPLACE FORCE VIEW XXMX_SUPPLIER_SCOPE_V
     (
      VENDOR_ID
     ,VENDOR_NAME
     )
AS 
     ...
     SELECT  DISTINCT 
             aps.vendor_id
            ,aps.vendor_name
     FROM    apps.ap_suppliers@MXDM_NVIS_EXTRACT              aps
          --,apps.hz_parties@MXDM_NVIS_EXTRACT                 hz
     WHERE   1 = 1
     AND     aps.enabled_flag            = 'Y'
     --AND    aps.party_id                = hz.party_id
     AND     aps.VENDOR_TYPE_LOOKUP_CODE != 'INTERCOMPANY'
     AND     SYSDATE                   < NVL(trunc(aps.end_date_Active), SYSDATE+1)
     AND     NVL(aps.one_time_flag,'N') <> 'Y'
     AND     aps.EMPLOYEE_ID is null
     -- At least one eligible Supplier Site exists
     AND     EXISTS ( 
                     SELECT 'X'
                     FROM   apps.ap_supplier_sites_all@MXDM_NVIS_EXTRACT     apsa
                           ,apps.hz_party_sites@MXDM_NVIS_EXTRACT            hps
                     WHERE  1 = 1
                     AND    aps.vendor_id                = apsa.vendor_id
                     AND    SYSDATE                      < NVL(TRUNC(apsa.inactive_date),SYSDATE+1)
                     AND    hps.party_site_id            = apsa.party_site_id
                     AND   (
                               apsa.rfq_only_site_flag   = 'Y'
                            OR apsa.purchasing_site_flag = 'Y'
                            OR apsa.pay_site_flag        = 'Y'
                           )    
                     AND    apsa.org_id                 IN (
                                                            ...
                                                            AND     haou.name            IN (
                                                                                             ...
                                                                                             FROM   xxmx_migration_params  xmp
                                                                                             WHERE  1 = 1
                                                                                             ...
                                                                                             AND    NOT EXISTS (
                                                                                                                SELECT 'X'
                                                                                                                FROM   xxmx_migration_params  xmp2
                                                                                                                WHERE  1 = 1
                                                                                                               ...
                                                                                                               )  --** This prevents the “general” parameters being used if there are migration specific parameters.
                                                                                             UNION
                                                                                             SELECT xmp3.parameter_value
                                                                                             FROM   xxmx_migration_params  xmp3
                                                                                             WHERE  1 = 1
     ...
     AND     EXISTS (
                     -- Has STANDARD POs created within the previous X Months
                     SELECT 'X'
                     FROM   apps.po_headers_all@MXDM_NVIS_EXTRACT poh
                     WHERE  1 = 1
                     AND    poh.vendor_id             = aps.vendor_id
                     AND    poh.type_lookup_code      = 'STANDARD'
                     AND    TRUNC(poh.creation_date) >= ADD_MONTHS(
                                                                   SYSDATE
                                                                  ,- NVL(TO_NUMBER(
                     ...
                     UNION
                     -- Has BLANKET POs created within the previous X Months
                     SELECT 'X'
                     FROM   apps.po_headers_all@MXDM_NVIS_EXTRACT poh
                     WHERE  1 = 1
                     AND    poh.vendor_id             = aps.vendor_id
                     ...
                     UNION
                     -- Has PO CONTRACTs that are still open
                     SELECT 'X'
                     FROM   apps.po_headers_all@MXDM_NVIS_EXTRACT poh
                     WHERE  1 = 1
                     AND    poh.vendor_id         = aps.vendor_id
                     AND    poh.type_lookup_code IN ('CONTRACT')
                     AND    SYSDATE               < NVL(TRUNC(poh.end_date),SYSDATE+1)
                     ...
                     UNION
                     -- Has AP Invoices created within the previous X Months
                     SELECT 'X'
                     FROM   apps.ap_invoices_all@MXDM_NVIS_EXTRACT            aia,
                            apps.ap_payment_schedules_all@MXDM_NVIS_EXTRACT   aps
                     WHERE  1 = 1
                     AND    aia.vendor_id             = aps.vendor_id
                     AND    aia.cancelled_date       IS NULL
                     AND    aps.invoice_id            = aia.invoice_id
                     AND    aps.amount_remaining     <> 0
                     AND    TRUNC(aia.creation_date) >= ADD_MONTHS(
                                                                   SYSDATE
                                                                  ,- NVL(
                                                                         TO_NUMBER(
                                                                                   xxmx_utilities_pkg.get_single_parameter_value
                                                                                        (
                                                                                         'FIN'               -- application_suite
                                                                                        ,'AP'                -- application
                                                                                        ,'SUPPLIER_INVOICES'          -- business_entity
																						,'ALL'
                                                                                        ,'MONTHS_TO_MIGRATE' -- parameter_code
                                                                                        )
                                                                                  )
                     ...
                     UNION
                     -- Supplier was created within the previous X Months
                     SELECT 'X'
                     FROM   apps.ap_suppliers@MXDM_NVIS_EXTRACT aps2
                     WHERE  1 = 1
                     AND    aps2.vendor_id             = aps.vendor_id
                     AND    TRUNC(aps2.creation_date) >= ADD_MONTHS(SYSDATE
                                                                   ,- NVL(
                                                                          TO_NUMBER(
                                                                                    xxmx_utilities_pkg.get_single_parameter_value
                                                                                         (
