CREATE OR REPLACE PACKAGE xxmx_fa_mass_additions_pkg AS 
     ...
     **
     **
     ** FILENAME  :  xxmx_fa_mass_additions_pkg.sql
     **
...
CREATE OR REPLACE PACKAGE BODY xxmx_fa_mass_additions_pkg as 
    ...
    g_batch_name                              CONSTANT  VARCHAR2(300)                               := 'FA_'||to_char(TO_DATE(SYSDATE, 'DD-MON-RRRR'),'DDMMRRRRHHMISS') ;
   ...
   -- setup
       ...
       gvv_migration_date := xxmx_utilities_pkg.get_single_parameter_value(
        ...
        gvd_migration_date := TO_DATE(gvv_migration_date,'YYYY-MM-DD');
...
--                dbms_output.put_line(' #' ||r_package_name.v_package ||' #'|| l_bg_name || '  #' || l_bg_id || '  #' || vt_MigrationSetID || '  #' || pt_i_MigrationSetName  );
   ...
   IS
        ...
        SELECT distinct fab_dist.asset_id                AS mass_addition_id
              ...
              ,'FIXED_ASSET'||to_char(TO_DATE(SYSDATE, 'DD-MON-RRRR'),'DDMMRRRRHHMISS')      AS  Batch_name
              --,xxmx_utilities_pkg.gvv_UserName                                          AS created_by  -- constant
              ,NULL  AS created_by  -- constant
              ,TO_CHAR(SYSDATE,'YYYY/MM/DD')                                              AS  creation_date -- constant
              --,xxmx_utilities_pkg.gvv_UserName                                          AS last_updated_by -- constant
              ,NULL as last_updated_by
              ,TO_CHAR(SYSDATE,'YYYY/MM/DD')                                              AS last_update_date -- constant
        ...
        --xxsh check subquery below for performance..
        AND    fds.period_counter                   = ( SELECT MAX (fds1.period_counter)
                                                        ...
                                                        AND    trunc(fds1.deprn_run_date) <= TRUNC(TO_DATE('31-AUG-2021', 'DD-MON-YYYY')) )                                                
        --xxsh revisit both lines below - change the date once agreed...OR add to the scope view
        AND  trunc(fb.date_placed_in_service)    <= trunc(to_date('31-AUG-2021', 'DD-MON-YYYY'))
        AND  fab.in_use_flag = 'YES'  --(all except 2 - 104794, 104877)
        --
        --
        UNION
        --
        ...
        SELECT distinct fab_dist.asset_id                AS mass_addition_id
              ...
              ,NULL as last_updated_by
              ,TO_CHAR(SYSDATE,'YYYY/MM/DD')                                              AS last_update_date -- constant
              ,NULL                                                                       AS fds.deprn_source_code  --xxsh non deprn assets
        FROM   apps.fa_distribution_history@MXDM_NVIS_EXTRACT    fdh
              ,apps.fa_additions_b@MXDM_NVIS_EXTRACT             fab_dist
              ,apps.fa_locations@MXDM_NVIS_EXTRACT               fal
              ,apps.gl_code_combinations@MXDM_NVIS_EXTRACT       gcc_exp
        ...
        AND not exists (
           SELECT 1
           FROM apps.fa_deprn_summary@MXDM_NVIS_EXTRACT               fds
           ,    apps.fa_deprn_periods@MXDM_NVIS_EXTRACT               fdp
           WHERE 1=1
           AND fdp.period_counter                   = fds.period_counter
           AND fds.asset_id                         = fb.asset_id
           AND fds.book_type_code                   = fb.book_type_code
           AND fdp.period_counter                   = 24262)  --xxsh attention hardcode testing   ************************    
        --xxsh revisit both lines below - change the date once agreed...OR add to the scope view
        AND  trunc(fb.date_placed_in_service)    <= trunc(to_date('31-AUG-2021', 'DD-MON-YYYY'))
        AND  fab.in_use_flag = 'YES'  --(all except 2 - 104794, 104877)
        ;
        ...
        THEN
           ...
           --
            ...
            xxmx_utilities_pkg.log_module_message(  
                   ...
                   ,pt_i_ProcOrFuncName      => cv_ProcOrFuncName
                   ,pt_i_ProgressIndicator   => gvv_ProgressIndicator
                   ,pt_i_ModuleMessage       => 'CURSOR imp_massadd_src_cur Fetch into imp_massadd_src_tb'
                   ,pt_i_OracleError         => gvt_ReturnMessage  );
           --
   ...
   IS
    ...
    IS
        ...
        --DEPRECIATING assets
        --
        SELECT   fab.asset_id                                                              AS mass_addition_id
                ,fb.book_type_code /* Reference data from EDQ*/                              AS book_type_code
                ,NULL                                                                      AS transaction_name
                ,fab.asset_number                                                          AS asset_number

                ,SUBSTR(replace(replace(TRIM(replace(Replace(replace((regexp_replace(fat.description, '(^[[:space:]]+)|([[:space:]]+$)',null)),chr(10)),chr(13)),chr(14849697))), '"', '"'), CHR(9)),1,10)
                                                                                           AS description
                ...
                ,NULL                                                                      AS commitment   --xxsh not in 11i
                ,NULL                                                                      AS investment_law  --xxsh not in 11i
                ,NULL                                                                      AS amortize_flag
                ,NULL                                                                      AS amortization_start_date
                ,NULL                                                                      AS depreciate_flag
                ...
                ,NULL                                                                      AS ytd_impairment  --xxsh not reqd at BS, and r12 table not in 11i
                ,NULL                                                                      AS impairment_reserve  --xxsh not reqd at BS, and r12 table not in 11i
                ...
                ,NULL                                                                      AS cash_generating_unit  --xxsh not in 11i
                ...
                ,NULL                                                                      AS nbv_at_switch  --xxsh not in 11i
                ,NULL                                                                      AS period_name_fully_reserved
                ,NULL                                                                      AS period_name_extended
                ,NULL                                                                      AS prior_deprn_limit_type  --xxsh not in 11i
                ,NULL                                                                      AS prior_deprn_limit  --xxsh not in 11i
                ,NULL                                                                      AS prior_deprn_limit_amount  --xxsh not in 11i
                ,NULL                                                                      AS prior_method_code
                ,NULL                                                                      AS prior_life_in_months  --xxsh not in 11i
                ,NULL                                                                      AS prior_basic_rate  --xxsh not in 11i
                ,NULL                                                                      AS prior_adjusted_rate  --xxsh not in 11i
                ,NULL                                                                      AS asset_schedule_num
                ,NULL                                                                      AS lease_number  --xxsh no data in fa_leases at BS. --NB this column is in fa_additions_v but now swapped to use fa_additions_b for performance
                ,fds.reval_reserve                                                         AS reval_reserve
                ,NULL                                                                      AS reval_loss_balance  --xxsh not in 11i
                ...
                ,xxmx_utilities_pkg.gvv_UserName                                          AS last_updated_by -- constant
                ,TO_CHAR(SYSDATE,'YYYY/MM/DD')                                              AS last_update_date -- constant
        FROM     apps.fa_categories_b@MXDM_NVIS_EXTRACT                fac
                ,apps.fa_asset_keywords@MXDM_NVIS_EXTRACT              fak
                ,apps.fa_books@MXDM_NVIS_EXTRACT                       fb
                ,apps.fa_additions_b@MXDM_NVIS_EXTRACT                 fab     --xxsh using table rather than view for performance
                ,apps.fa_deprn_summary@MXDM_NVIS_EXTRACT               fds
                ,apps.fa_additions_tl@MXDM_NVIS_EXTRACT                fat
                ,apps.fa_category_books@MXDM_NVIS_EXTRACT              fcb
                ,apps.fa_deprn_periods@MXDM_NVIS_EXTRACT               fdp
                ,apps.fa_book_controls@MXDM_NVIS_EXTRACT               fbc
        WHERE    1                                    = 1
        ...
        --xxsh need to revisit line below for performance reasons
        ...
        --xxsh need to revisit query below for performance reasons
        AND      fds.period_counter                   = (SELECT MAX (fds1.period_counter)
                                                         FROM   apps.fa_deprn_summary@MXDM_NVIS_EXTRACT fds1
                                                         WHERE  fds1.asset_id = fds.asset_id
                                                         AND    fds1.book_type_code = fds.book_type_code
        ...
        --
        SELECT   fab.asset_id                                                              AS mass_addition_id
                ,fb.book_type_code                                                         AS book_type_code  --Reference data from EDQ
                ,NULL                                                                      AS transaction_name
                ,fab.asset_number                                                          AS asset_number
                ...
                ,fb.salvage_value                                                          AS salvage_value
                ,fb.percent_salvage_value                                                  AS percent_salvage_value
                ,NULL                                                                      AS ytd_deprn  --xxsh non-deprn asset
                ,NULL                                                                      AS deprn_reserve  --xxsh non-deprn asset
                ,NULL                                                                      AS bonus_ytd_deprn  --xxsh non-deprn asset
                ,NULL                                                                      AS bonus_deprn_reserve  --xxsh non-deprn asset
                ,NULL                                                                      AS ytd_impairment  --xxsh not reqd at BS, and r12 table not in 11i
                ,NULL                                                                      AS impairment_reserve  --xxsh not reqd at BS, and r12 table not in 11i
                ,fb.deprn_method_code                                                      AS method_code
                ,fb.life_in_months                                                         AS life_in_months
                ...
                ,NULL                                                                      AS asset_schedule_num
                ,NULL                                                                      AS lease_number  --xxsh no data in fa_leases at BS. --NB this column is in fa_additions_v but now swapped to use fa_additions_b for performance
                ,NULL                                                                      AS reval_reserve  --xxsh non-deprn asset
                ,NULL                                                                      AS reval_loss_balance  --xxsh not in 11i
                ,NULL                                                                      AS reval_amortization_basis  --xxsh non-deprn asset
                ,NULL                                                                      AS impair_loss_balance
                ,fb.reval_ceiling                                                          AS reval_ceiling
                ,NULL                                                                      AS fair_market_value
                ,NULL                                                                      AS last_price_index_value
        ...
        AND not exists (
           ...
           AND fds.asset_id                         = fb.asset_id
           AND fds.book_type_code                   = fb.book_type_code
           AND fdp.period_counter                   = 24262)  --xxsh attention  ****change this...****************************
        --xxsh revisit both lines below - change the date once agreed...OR add to the scope view
        AND  trunc(fb.date_placed_in_service)    <= trunc(to_date('31-AUG-2021', 'DD-MON-YYYY'))
        AND  fab.in_use_flag = 'YES'  --(all except 2 - 104794, 104877)
        ;
        ...
        THEN
         ...
         SET    (fmas.POSTING_STATUS, fmas.QUEUE_NAME) = 
                              (SELECT fma.POSTING_STATUS, fma.QUEUE_NAME
                               FROM   apps.fa_mass_additions@MXDM_NVIS_EXTRACT fma
                               WHERE  1=1
    ...
    AS 
      ...
      --
        ...
        IS
            ...
            FROM     apps.fa_mc_mass_rates@MXDM_NVIS_EXTRACT           fmr
                    ,apps.fa_mass_additions_v@MXDM_NVIS_EXTRACT        fmav
                    ,apps.gl_sets_of_books@MXDM_NVIS_EXTRACT           gsob
           WHERE    fmr.mass_addition_id              = fmav.mass_addition_id
           AND      gsob.set_of_books_id              = fmr.set_of_books_id
           AND      EXISTS                              ( SELECT 1
                                                          FROM  xxmx_fa_mass_additions_stg pfsv
                                                          WHERE pfsv.mass_addition_id = fmav.asset_id
                                                         );
