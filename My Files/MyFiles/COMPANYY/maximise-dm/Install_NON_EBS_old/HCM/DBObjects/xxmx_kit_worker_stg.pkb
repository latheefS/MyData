...
--**
--**
--** FILENAME  :  xxmx_kit_worker_stg.sql
--**
...
--** AUTHORS   :  Ian
--**
--** PURPOSE   :  This package contains procedures for extracting HCM data from EBS 
--**              for migration
--**
--** NOTES     :
--**
...
--**   Vsn  Change Date  Changed By          Change Description
--** -----  -----------  ------------------  -----------------------------------
--** 1.0 	11/11/2020 	 Ian                 Initial Build
--** 1.1    11-NOV-20    Pallavi			 Add Extracts for PaymentMethod, Salary , Work Measure, GradeStep
--** 1.2    26/03/2021   Pallavi Kanajar	 Added People Group Name, Supervisor and all Other attributes (location
--**   	 					 				  Department, Position to Assignment File, Added Person_type Parameter		
--** 1.3    10/05/2021   Pallavi Kanajar     Parameter for Payroll
--** 1.4    27/05/2021   Pallavi Kanajar     Added Sort_code in the File
--** 1.5    17/06/2021   Pallavi Kanajar     PositionOverride Flag reset to 'Y' in the File
--**
--******************************************************************************
--**
...
/*                AND     (
    ...
    BEGIN
		...
		SELECT  distinct  pt_i_MigrationSetID                               
                ...
                ,c21_s_person_id|| '_PERSON' person_id
	...
	--	INSERT INTO XXMX_TEST VALUES (lv_person_id);
		...
		SELECT  distinct pt_i_MigrationSetID                               
                ...
                ,c8_effective_start_date effective_start_date
                , to_date ('31-12-4712','DD-MM-YYYY') effective_end_date
                ,c25_s_person_id|| '_PERSON' person_id
    ...
    PROCEDURE export_all_assignments_m_3
        ...
        ct_Phase                            CONSTANT    xxmx_module_messages.phase%TYPE  := 'EXTRACT';
        cv_StagingTable                     CONSTANT    VARCHAR2(100) DEFAULT 'XXMX_PER_ASSIGNMENTS_M_STG';
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'TERM_ASSIGN';
    ...
    BEGIN
		...
		SELECT   pt_i_MigrationSetID                               
                ...
                ,to_date ('31-12-4712','DD-MM-YYYY') effective_end_date
                ,c80_s_person_id|| '_PERSON' person_id
                ,s0_period_of_placement_id|| '_PERIOD_OF_PLACEMENT' period_of_service_id
                ,'C' assignment_type
...
;
	...
	PROCEDURE export_all_assignments_m_5
        ...
        ct_Phase                            CONSTANT    xxmx_module_messages.phase%TYPE  := 'EXTRACT';
        cv_StagingTable                     CONSTANT    VARCHAR2(100) DEFAULT 'XXMX_PER_ASSIGNMENTS_M_STG';
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'ADD_ASSIGN';
    ...
    BEGIN
        ...
        /*AND sup.effective_start_Date IN( SELECT max(effective_Start_date) 
										FROM  per_all_Assignments_f@MXDM_NVIS_EXTRACT paaf2
    ...
    BEGIN
		...
		*/
        ...
        Set CONTRACTTYPE = ( Select CONTRACT_TYPE frFom PQP_ASSIGNMENT_ATTRIBUTES_F@MXDM_NVIS_EXTRACT casg
	...
	PROCEDURE export_asg_pay_method
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'PERSON_PAYMENT_METHOD';
	...
	PROCEDURE export_ext_Bank_Account
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'EXTERNAL_BANK_ACCOUNTS';
	...
	PROCEDURE export_asg_payroll
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'ASSGINMENT_PAYROLL';
	...
	PROCEDURE export_asg_salary
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'ASG_SALARY';
...
*/
	...
	PROCEDURE export_asg_gradestep
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'ASSIGNMENT_GRADESTEP';
...
;
	...
	SET effective_start_date = NVL((SELECT a.effective_start_date 
				  ...
				  AND a.person_id = ap.person_id||'_PERSON'
				  AND a.Action_code IN('ADD_ASSIGN', 'CURRENT')),SYSDATE);
	COMMIT;
        --
        gvv_ProgressIndicator := '0030';
	...
	PROCEDURE export_asg_wrkmsr
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'ASSIGNMENT_WRKMSURE';
	...
	*****************************************************************/
    ...
    PROCEDURE export_Senioritydt
        ...
        cv_i_BusinessEntityLevel            CONSTANT    VARCHAR2(100) DEFAULT 'PERSON_SENIORITYDT';
    ...
    BEGIN
        ...
        SELECT  distinct pt_i_MigrationSetID                               
			    ...
			    ,(SELECT PAYROLLNAME
				FROM xxmx_per_assignments_m_stg asg
				WHERE asg.PERSON_ID = PAPF.PERSON_ID||'_PERSON'
...
/
SHOW ERRORS PACKAGE BODY xxmx_kit_worker_stg;
/