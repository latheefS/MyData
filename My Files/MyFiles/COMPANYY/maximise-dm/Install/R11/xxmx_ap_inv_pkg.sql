CREATE OR REPLACE PACKAGE xxmx_ap_inv_pkg AUTHID CURRENT_USER
AS
     --
     --
     /*
...
AS
     ...
     */
     ...
     gct_BusinessEntity                        CONSTANT xxmx_migration_metadata.business_entity%TYPE := 'AP_INVOICES'; -- changes for Maximise 2.0
     --
     /*
      ...
      IS
          ...
          IS
               ...
               SELECT     aia.invoice_id                                             AS invoice_id
                         ,TRIM(haou.name)                                            AS operating_unit  --xxsh map in XFM
--xxsh manual mapping if reqd
--                         ,decode(TRIM(haou.name),                                           
--                                 'OUBS LTD','Boylesports 2 Unlimited',
--                                 'OUBS NI','Bolyesports (N.I.) Limited',
--                                 'OUBS CC','Bolyesports (Call Centre)',
--                                 'OUBS CC MKTH','Bolyesports (Call Centre)- Markethill',
--                                 'OUBS GIB','Bolyesports (Gibraltar) Limited',
--                                 'OUBS UK','Bolyesports (UK) Limited',
--                                 'NotMapped')                                        AS operating_unit   --xxsh setup mapping in XFM
                         ,(
                           SELECT gl.name
                           FROM   apps.gl_sets_of_books@mxdm_nvis_extract  gl   --xxsh 11i table
                           WHERE  1 = 1
                           AND    gl.set_of_books_id = aia.set_of_books_id   --xxsh 11i join
                          )                                                          AS ledger_name
                         ,TRIM(aia.invoice_num)                                      AS invoice_num
                         --xxsh NB GSTT use + round(ps.amount_remaining,2) - should be the same as below....
                         ...
                         ,TRIM(haou.name)                                            AS legal_entity_name   --xxsh map in XFM based on OU
                         ,NULL                                                       AS cust_registration_number  --xxsh not in 11i 
                         ,NULL                                                       AS cust_registration_code  --xxsh not in 11i 
                         ...
                         ,TRIM(aia.gl_date)                                          AS gl_date    --xxsh GSTT use fin_utilities_pkg.gc_accounting_date - do we need to consider an override also ??  ************
                         ,TRIM(aia.payment_method_lookup_code)                       AS payment_method_code   --xxsh 11i column name
                         ...
                         ,NULL                                                       AS delivery_channel_code  --xxsh not in 11i
                         ,NULL                                                       AS bank_charge_bearer  --xxsh not in 11i
                         ,NULL                                                       AS remit_to_supplier_name  --xxsh not in 11i
                         ,NULL                                                       AS remit_to_supplier_num
                         ,NULL                                                       AS remit_to_address_name
                         ,NULL                                                       AS payment_priority
                         ,NULL                                                       AS settlement_priority   --xxsh not in 11i
                         ,NULL                                                       AS unique_remittance_identifier   --xxsh not in 11i
                         ,NULL                                                       AS uri_check_digit   --xxsh not in 11i
                         ,NULL                                                       AS payment_reason_code   --xxsh not in 11i
                         ,NULL                                                       AS payment_reason_comments   --xxsh not in 11i
                         ,NULL                                                       AS remittance_message1   --xxsh not in 11i
                         ,NULL                                                       AS remittance_message2   --xxsh not in 11i
                         ,NULL                                                       AS remittance_message3   --xxsh not in 11i
                         ,NULL                                                       AS awt_group_name
                         ,NULL                                                       AS ship_to_location
                         ,NULL                                                       AS taxation_country  --xxsh not in 11i
                         ,NULL                                                       AS document_sub_type  --xxsh not in 11i
                         ,NULL                                                       AS tax_invoice_internal_seq  --xxsh not in 11i
                         ,NULL                                                       AS supplier_tax_invoice_number  --xxsh not in 11i
                         ,NULL                                                       AS tax_invoice_recording_date  --xxsh not in 11i
                         ,NULL                                                       AS supplier_tax_invoice_date  --xxsh not in 11i
                         ,TRIM(aia.supplier_tax_exchange_rate)                       AS supplier_tax_exchange_rate
                         ,NULL                                                       AS port_of_entry_code  --xxsh not in 11i
                         ,NULL                                                       AS correction_year
                         ,NULL                                                       AS correction_period
                         ,NULL                                                       AS import_document_number
                         ,NULL                                                       AS import_document_date
                         ,TRIM(aia.tax_amount)                                       AS control_amount  --xxsh 11i diff column name.  NB may have to reduce by prorate amount remaining ??
                         ,NULL                                                       AS calc_tax_during_import_flag -- EDQ MApping 'N'  --xxsh GSTT = N (they have tax lines to import)
                         ,NULL                                                       AS add_tax_to_inv_amt_flag -- EDQ MApping 'N'  --xxsh GSTT = N (they have tax lines to import)
...
--xxsh not in 11i               AND        hp.party_id                        = aps.party_id
         ...
         -- Cursor to get the detail AP Invoices Lines
           ...
           IS
                  ...
                  SELECT DISTINCT
                          ...
                          , TRIM(aida.distribution_line_number)                                                   AS  line_number  --xxsh get from distn
                          , TRIM(aida.line_type_lookup_code)                                                      AS  line_type_lookup_code  --xxsh get from distn
                          , round(aida.amount,2)                                                                  AS  amount  --xxsh 11i - basic swap
                          --xxsh but GSTT use this:
                          --round((aid.amount/ai.invoice_amount)*(ps.amount_remaining),2)  --so for partially paid - prorata for all distn lines... 
                          , round(aida.quantity_invoiced,2)                                                       AS  quantity_invoiced  --xxsh get from distn
                          , NULL                                                                                  AS  unit_price
                          , TRIM(aida.unit_meas_lookup_code)                                                      AS  unit_of_meas_lookup_code  --xxsh get from distn
                          , xxmx_utilities_pkg.convert_string
                                 (
                                  pv_i_StringToConvert     => SUBSTRB(TRIM(aida.description), 1, 240)
                                 ,pv_i_ConvertCommaToSpace => 'N'
                                 )                                                                                AS  description  --xxsh get from distn
                          , NULL                                                                                  AS  po_number
                          ...
                          , TRIM(aida.final_match_flag)                                                           AS  final_match_flag  --xxsh get from distn
                          , (
                             SELECT concatenated_segments
                             FROM   apps.gl_code_combinations_kfv@MXDM_NVIS_EXTRACT       gcck   --xxsh reinstate dblink
                             WHERE  gcck.code_combination_id = aida.dist_code_combination_id                          
                            )                                                                                     AS  dist_code_concatenated ---xxsh get direct from distn
                          , NULL                                                                                  AS  distribution_set_name
                          , aida.accounting_date                                                                  AS  accounting_date --xxsh get from distn
                          , NULL                                                                                  AS  account_segment
                          , NULL                                                                                  AS  balancing_segment
                          , NULL                                                                                  AS  cost_center_segment
                          , NULL                                                                                  AS  tax_classification_code
                          , NULL                                                                                  AS  ship_to_location_code --xxsh set to NULL (same as GSTT
                          , NULL                                                                                  AS  ship_from_location_code
                          , NULL                                                                                  AS  final_discharge_location_code
                          , NULL                                                                                  AS  trx_business_category  --xxsh not in 11i
                          , NULL                                                                                  AS  product_fisc_classification  --xxsh not in 11i
                          , NULL                                                                                  AS  primary_intended_use  --xxsh not in 11i
                          , NULL                                                                                  AS  user_defined_fisc_class  --xxsh not in 11i
                          , NULL                                                                                  AS  product_type
                          , NULL                                                                                  AS  assessable_value
                          , NULL                                                                                  AS  product_category
                          , NULL                                                                                  AS  control_amount  --xxsh not in 11i NB tax_amount at header level
                          ...
                          , NULL                                                                                  AS  tax_rate
                          , NULL                                                                                  AS  awt_group_name
                          , TRIM(aida.type_1099)                                                                  AS  type_1099   --xxsh get from distn
                          , NULL                                                                                  AS  income_tax_region
                          , NULL                                                                                  AS  prorate_across_flag
                          , NULL                                                                                  AS  line_group_number
                          , NULL                                                                                  AS  cost_factor_name
                          , TRIM(aida.stat_amount)                                                                AS  stat_amount  --xxsh get from distn
                          , NULL                                                                                  AS  assets_tracking_flag
                          , NULL                                                                                  AS  asset_book_type_code
                          , NULL                                                                                  AS  asset_category_id
                          , NULL                                                                                  AS  serial_number  --xxsh not in 11i
                          , NULL                                                                                  AS  manufacturer  --xxsh not in 11i
                          , NULL                                                                                  AS  model_number  --xxsh not in 11i
                          , NULL                                                                                  AS  warranty_number  --xxsh not in 11i
                  ...
                  FROM    xxmx_ap_inv_scope_v                                    selection
                          ...
                          , apps.ap_invoice_distributions_all@MXDM_NVIS_EXTRACT         aida  --xxsh no Lines in 11i
                          , apps.hr_all_organization_units@MXDM_NVIS_EXTRACT    haou
                   WHERE   1=1
                  AND aida.line_type_lookup_code                 IN ('ITEM')
                  AND apsa.invoice_id                             = selection.invoice_id
                  AND aida.invoice_id                             = selection.invoice_id
                  AND haou.organization_id                         = aida.org_id
                  ;
          --** END CURSOR ** 
          --
          --
