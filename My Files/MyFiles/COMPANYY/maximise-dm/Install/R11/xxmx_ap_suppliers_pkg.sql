...
/
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_SUPPLIER_PKG" AUTHID CURRENT_USER
AS
     --
     --**********************
     --** Global Declarations
     ...
     IS
          ...
          IS
               ...
               SELECT DISTINCT
                      ...
                      ,TRIM(REPLACE(aps.vendor_name, '"', '""'))                      AS supplier_name   --xxsh GSTT has this replace
                      ,''                                                             AS supplier_name_new
                      ,aps.segment1                                                   AS supplier_number
                      ,TRIM (REPLACE (aps.vendor_name_alt, ',', ''))                  AS alternate_name  --xxsh GSTT has this replace
                      ,NULL                                                           AS tax_organization_type
                      ,aps.vendor_type_lookup_code                                    AS supplier_type   --xxsh NB GSTT defaultsCORPORATION
                      ,aps.end_date_active                                            AS inactive_date
                      ,NULL                                                           AS business_relationship  --xxsh GSTT defaults to 'SPEND_AUTHORIZED'
                      ,(
                        SELECT aps2.vendor_name
                        FROM   apps.po_vendors@MXDM_NVIS_EXTRACT aps2   --xxsh 11i table
                        WHERE  aps2.vendor_id = aps.parent_vendor_id
                       )                                                              AS parent_supplier
                      ,''                                                             AS alias
                      ,(
                        SELECT RPAD(MAX(assa.ece_tp_location_code),9,'0')
                        FROM   apps.po_vendor_sites_all@MXDM_NVIS_EXTRACT assa    --xxsh 11i table
...
--xxsh not in 11i                      ,(
--xxsh                        SELECT country_code
--xxsh                        FROM   apps.zx_party_tax_profile@MXDM_NVIS_EXTRACT zptp
--xxsh                        WHERE  zptp.party_id   = aps.party_id
--xxsh                        AND    party_type_code = 'THIRD_PARTY'
--xxsh                       )                                                              AS taxpayer_country
                      ,''                                                             AS taxpayer_country    --xxsh as per GSTT
               ...
               FROM    xxmx_supplier_scope_v                xssv
                      ,apps.po_vendors@MXDM_NVIS_EXTRACT    aps  --xxsh 11i table
--xxsh not in 11i                      ,apps.hz_parties@MXDM_NVIS_EXTRACT    hp
               WHERE   1 = 1
               AND     aps.vendor_id = xssv.vendor_id
--xxsh not in 11i               AND     aps.party_id  = hp.party_id
               ;
               --
               --
          --** END CURSOR **
     ...
     */
     ...
     IS
          ...
          IS
               --
               WITH
               distinct_vendor_sites    --xxsh no party records in 11i
               AS
                    (
                     SELECT  DISTINCT
                             xssv.vendor_name
                            ,xssv.vendor_site_id     --xxsh use vendor_site_id instead
                     FROM    xxmx_supplier_scope_v  xssv
                    )
--xxsh removed contact_points - now taking values direct from po_vendor_sites_all table
--xxsh will revisit as part of testing
--xxsh              ,contact_points
               SELECT   'CREATE'                                                                AS import_action
--xxsh no hz_locations in 11i                       ,hl.location_id                                                          AS location_id
                       ,pvs.vendor_site_id                                                      AS supplier_site_id  --xxsh added id - may be useful for pk/fk or debugging ?
--xxsh no party site connection in 11i                       ,NULL                                                                    AS party_site_id
                       ,TRIM(REPLACE(dvs.vendor_name, '"', '""'))                               AS supplier_name  --xxsh GSTT use replace
                       ,TRIM(pvs.vendor_site_code)                                              AS address_name   --xxsh use vendor site code  , GSTT use this if party_site_name is null
                       ,''                                                                      AS address_name_new
                       ,pvs.country                                                             AS country   --xxsh  use pvs table
                       ,REPLACE(pvs.address_line1, '"', '""')                                   AS address1  --xxsh  --xxsh GSTT use replace
                       ,REPLACE(pvs.address_line2, '"', '""')                                   AS address2  --xxsh  --xxsh GSTT use replace
                       ,REPLACE(pvs.address_line3, '"', '""')                                   AS address3  --xxsh  --xxsh GSTT use replace
                       ,REPLACE(pvs.address_line4, '"', '""')                                   AS address4  --xxsh  --xxsh GSTT use replace
                       ,NULL                                                                    AS phonetic_address_line  --xxsh not in 11i
                       ...
                       ,NULL                                                                    AS building  --xxsh not in 11i
                       ,NULL                                                                    AS floor_number  --xxsh not in 11i
                       ,REPLACE(pvs.city, '"', '""')                                            AS city  --xxsh GSTT use replace
                       ,REPLACE(pvs.state, '"', '""')                                           AS state  --xxsh GSTT use replace
                       ,REPLACE(pvs.province, '"', '""')                                        AS province  --xxsh GSTT use replace
                       ,REPLACE(pvs.county, '"', '""')                                          AS county  --xxsh GSTT use replace
                       ,pvs.zip                                                                 AS postal_code  --xxsh us pvs table
                       ,NULL                                                                    AS postal_plus_4_code  --xxsh not in 11i
                       ,NULL                                                                    AS addressee  --xxsh not in 11i - same code as GSTT
                       ,NULL                                                                    AS global_location_number  --xxsh not in 11i same as GSTT code
                       ,pvs.language                                                            AS language  --xxsh use pvs table
--xxsh use GSTT code, to cater for sites that will expire soon.. and use pvs table
                       ,CASE WHEN pvs.inactive_date IS NOT NULL AND TRUNC(pvs.inactive_date) <= TRUNC(SYSDATE+15)
                        THEN (SYSDATE + 15) 
                        ELSE pvs.inactive_date
                        END                                                                      AS inactive_date,                
--xxsh use GSTT code:
                       ,CASE
                        WHEN pvs.phone IS NOT NULL
                          THEN
                           (SELECT hpc.phone_country_code
                            FROM hz_phone_country_codes hpc
                            WHERE     hpc.territory_code = pvs.country
                            AND hpc.phone_country_code IS NOT NULL
                            AND ROWNUM = 1)
                        ELSE
                          NULL
...
--xxsh use GSTT code:
                       ,CASE
                        WHEN pvs.phone IS NOT NULL THEN pvs.area_code
                        ELSE NULL
                        END                                                                     AS phone_area_code
                       ,pvs.phone                                                               AS phone            --xxsh use pvs table, as per GSTT code
                       ,''                                                                      AS phone_extension
                       ,''                                                                      AS fax_country_code
                       ,CASE
                        WHEN pvs.fax IS NOT NULL THEN pvs.fax_area_code
                        ELSE NULL
                        END                                                                     AS fax_area_code --xxsh as per GSTT code
                       ,pvs.fax                                                                 AS fax   --xxsh use pvs table
                       ,NVL(pvs.rfq_only_site_flag,'N')                                         AS rfq_or_bidding   --xxsh use pvs table
                       ,NVL(pvs.purchasing_site_flag,'N')                                       AS ordering  --xxsh use pvs table
                       ,NVL(pvs.pay_site_flag,'N')                                              AS pay  --xxsh use pvs table
                       ,NULL                                                                    AS attribute_category
                       ,NULL                                                                    AS attribute1
                       ,NULL                                                                    AS attribute2
                       ,NULL                                                                    AS attribute3
                       ...
                       ,REPLACE(REPLACE(pvs.email_address, Chr(10), ''), Chr(13), '')           AS email_address  --xxsh use pvs table, as per GSTT code
                       ,''                                                                      AS delivery_channel
                       ,''                                                                      AS bank_instruction_1
                       ,''                                                                      AS bank_instruction_2
                       ,''                                                                      AS bank_instruction
                       ...
                       ,''                                                                      AS remittance_email
                       ,''                                                                      AS remittance_fax
               FROM    distinct_vendor_sites                  dvs   --xxsh vendor_sites rtahre than party_sites
                      ,apps.po_vendor_sites_all@MXDM_NVIS_EXTRACT    pvs
               WHERE   1 = 1
               AND     pvs.vendor_site_id             = dvs.vendor_site_id
               ;
               --
               --
          --** END CURSOR **
     ...
     */
     ...
     IS
          ...
          IS
               ...
               AS
                      (
                       SELECT aps.vendor_id                                           AS vendor_id
                             ...
                             ,aps.exclusive_payment_flag                              AS exclusive_payment_flag
                       FROM   po_vendors@MXDM_NVIS_EXTRACT  aps  --xxsh 11i table
                      )
               SELECT  DISTINCT
                       'CREATE'                                                                 AS import_action
                      ,xssv.vendor_id                                                           AS supplier_id
                      ,TRIM(REPLACE(xssv.vendor_name, '"', '""'))                               AS supplier_name  --xxsh use replace also
                      ,xssv.operating_unit_name                                                 AS operating_unit_name
                      ,NULL                                                                     AS party_site_id  --xxsh not in 11i
                      ,assa.vendor_site_code                                                    AS address_name   --party_site_name not in 11i
                      ,assa.vendor_site_id                                                      AS supplier_site_id
                      ,assa.vendor_site_code                                                    AS supplier_site                      
                      ,''                                                                       AS supplier_site_new
                      ,CASE WHEN assa.inactive_date IS NOT NULL AND TRUNC(assa.inactive_date) <= TRUNC(SYSDATE+15)  --xxsh use GSTT code 
                       THEN (SYSDATE + 15) 
                       ELSE assa.inactive_date
                       END                                                                      AS  inactive_date,                                      
                      ...
                      ,CASE
                            ...
                            ELSE NULL
                       END                                                                      AS fax
                      ,NVL(dfs.hold_flag,'N')                                                   AS hold_purchasing_documents   --xxsh NVL as per GSTT
                      ...
                      ,NVL(dfs.hold_future_payments_flag,'N')                                   AS hold_unvalidated_invoices  --xxsh NVL as per GSTT
                      ,dfs.hold_by                                                              AS payment_hold_by
                      ,dfs.hold_date                                                            AS payment_hold_date
                      ,CASE             
                       --xxsh used GSTT logic, as got Import error:
                       --The HOLD_REASON is required when any of the following is set to Y.
                       --HOLD_ALL_PAYMENTS_FLAG,HOLD_UNMATCHED_INVOICES_FLAG,HOLD_FUTURE_PAYMENTS_FLAG
                       WHEN     TRIM(assa.hold_reason) IS NULL
                                AND NVL(assa.hold_all_payments_flag, 'N') = 'Y'
                       THEN
                         'No Hold Reason Provided'
                       WHEN     TRIM(assa.hold_reason) IS NULL
                                AND NVL(dfs.hold_future_payments_flag, 'N') = 'Y'   
                       THEN
                         'No Hold Reason Provided'
                       WHEN (NVL(dfs.hold_future_payments_flag, 'N') = 'Y' OR 
                         NVL(assa.hold_unmatched_invoices_flag, 'N') = 'Y' OR   --xxsh added 
                         NVL(assa.hold_all_payments_flag, 'N') = 'Y') 
                         AND TRIM(assa.hold_reason) IS NULL
                       THEN
                         'No Hold Reason Provided'
                       WHEN (NVL(dfs.hold_future_payments_flag, 'N') = 'Y' OR NVL(assa.hold_all_payments_flag, 'N') = 'Y') 
                            AND TRIM(assa.hold_reason) IS NOT NULL
                       THEN
                         assa.hold_reason
                       ELSE ''
                       END                                                                      AS payment_hold_reason
                      ...
                      ,assa.supplier_notif_method                                               AS delivery_method  --xxsh changed from NULL to column
                      ,NULL                                                                     AS remittance_e_mail  --xxsh keep at NULL, but test this out..
               ...
               FROM    xxmx_supplier_scope_v                              xssv
                      ,apps.po_vendor_sites_all@MXDM_NVIS_EXTRACT         assa  --xxsh 11i table
--xxsh not used in 11i                      ,apps.hz_party_sites@MXDM_NVIS_EXTRACT              hps
                      ,apps.ap_terms_tl@MXDM_NVIS_EXTRACT                 att
--xxsh not used in 11i                      ,apps.hz_locations@MXDM_NVIS_EXTRACT                hl
...
--xxsh not used in 11i               AND     hps.party_site_id         = assa.party_site_id
               AND     att.term_id(+)            = assa.terms_id
               AND     att.LANGUAGE(+)           = 'US'
--xxsh not used in 11i               AND     hl.location_id            = hps.location_id
          ...
          IS
               ...
               FROM    xxmx_stg.xxmx_ap_supplier_sites_stg               xasss
          ...
          THEN
               ...
               */
               ...
               LOOP
                    ...
                    --
                    UPDATE  xxmx_stg.xxmx_ap_supplier_sites_stg  xasss
      ...
      IS
          ...
          IS
               ...
               SELECT  DISTINCT
                       'CREATE'                                                                 AS import_action
                      ,aps.vendor_id                                                            AS supplier_id
                      ,TRIM(REPLACE(aps.vendor_name, '"', '""'))                                AS supplier_name  --xxsh added replace
                      ,assa.vendor_site_id                                                      AS supplier_site_id
                      ,assa.vendor_site_code                                                    AS supplier_site
--xxsh SMBC specific                      ,decode(assa.org_id,142,substr(assa.vendor_site_id||'-'||assa.vendor_site_code,1,15),assa.vendor_site_code) AS supplier_site
                      ,xssv.operating_unit_name                                                 AS procurement_bu
                      ,xssv.operating_unit_name                                                 AS client_bu
                      ,xssv.operating_unit_name                                                 AS bill_to_bu
                      ,TRIM(REPLACE (REPLACE (REPLACE (ship_loc.location_code, CHR (13)), CHR (10), ''), CHR(9), ''))  AS ship_to_location  --xxsh GSTT replace
                      ,TRIM(REPLACE (REPLACE (REPLACE (bill_loc.location_code, CHR (13)), CHR (10), ''), CHR(9), ''))  AS bill_to_location  --xxsh GSTT replace
                      ,DECODE(
                              gct_MigrateCISData
                                   ,'N' ,'N'
                                        ,aps.allow_awt_flag
      ...
      IS
          ...
          IS
               ...
               SELECT   DISTINCT
                        ...
                        ,apsc.first_name                                                            AS first_name  --xxsh party not in 11i
                        ,''                                                                         AS first_name_new
                        ,apsc.middle_name                                                           AS middle_name
                        ,apsc.last_name                                                             AS last_name  --xxsh party not in 11i
                        ,''                                                                         AS last_name_new
                        ,apsc.title                                                                 AS job_title
                        ,''                                                                         AS primary_admin_contact
                        ,REPLACE(REPLACE(apsc.email_address, Chr(10), ''), Chr(13), '')             AS email_address  --xxsh use replace as per GSTT
               ...
               FROM      xxmx_supplier_scope_v                         xssv
                        ,apps.po_vendor_sites_all@MXDM_NVIS_EXTRACT    assa    --xxsh 11i table
--xxsh not used 11i                        ,apps.hz_parties@MXDM_NVIS_EXTRACT             person_party
--xxsh not used 11i                        ,apps.hz_parties@MXDM_NVIS_EXTRACT             relationship_party
                        ,apps.po_vendor_contacts@MXDM_NVIS_EXTRACT     apsc    --xxsh 11i table
               WHERE     1 = 1
               AND       assa.org_id                                        = xssv.org_id
               AND       assa.vendor_id                                     = xssv.vendor_id
               AND       assa.vendor_site_id                                = xssv.vendor_site_id
               AND       apsc.vendor_site_id                                = assa.vendor_site_id  --xxsh removed outer join on apsc
--xxsh not used 11i               AND       apsc.org_party_site_id(+)                          = assa.party_site_id
               AND       NVL(TRUNC(apsc.inactive_date), TRUNC(SYSDATE) + 1) > TRUNC(SYSDATE)
--xxsh not used 11i               AND       person_party.party_id                              = apsc.per_party_id
--xxsh not used 11i               AND       relationship_party.party_id                        = apsc.rel_party_id
               ;
               --
          --** END CURSOR **
          --
...
--xxsh 11i code below:
               SELECT  DISTINCT
                       TO_NUMBER(
                               ...
                               ||pvs.vendor_site_id
                               ||aba.org_id
                                )                                                  AS temp_ext_payee_id
                      ,TO_NUMBER(
                                 decode(pvs.vendor_site_id,NULL,pv.vendor_id,pvs.vendor_site_id)
                               ||aba.bank_account_id
                               ||aba.org_id
                                )                                                  AS temp_ext_bank_acct_id 
                      ,abb.bank_name                                               AS bank_name
                      ,abb.bank_branch_name                                        AS branch_name
                      ,CASE
                            WHEN INSTR(cbbv.branch_number, '-') > 0
                                 THEN REPLACE(cbbv.branch_number, '-', '')
                            ELSE cbbv.branch_number
                       END                                                         AS xxmx_branch_number
                      ,abb.country                                                 AS country_code
                      ,aba.bank_account_name                                       AS bank_account_name
                      ,aba.bank_account_num                                        AS bank_account_num
                      ,aba.currency_code                                           AS currency_code
                      ,aba.multi_currency_flag                                     AS foreign_payment_use_flag  --NB typically set for INT banks only
                      ,abau.start_date                                             AS start_date
                      ,abau.end_date                                               AS end_date
                      ,aba.iban_number                                             AS iban
                      ,aba.check_digits                                            AS check_digits
                      ,aba.bank_account_name_alt                                   AS bank_account_name_alt
                      ,aba.bank_account_type                                       AS bank_account_type
                      ,NULL                                                        AS account_suffix
                      ,aba.description                                             AS description
                      ,aba.agency_location_code                                    AS agency_location_code
                      ,NULL                                                        AS exchange_rate_agreement_num
                      ,NULL                                                        AS exchange_rate_agreement_type
                      ,NULL                                                        AS exchange_rate
                      ,NULL                                                        AS secondary_account_reference
                      ,aba.attribute_category                                      AS attribute_category
                      ,aba.attribute1                                              AS attribute1
                      ,aba.attribute2                                              AS attribute2
                      ,aba.attribute3                                              AS attribute3
                      ,aba.attribute4                                              AS attribute4
                      ,aba.attribute5                                              AS attribute5
                      ,aba.attribute6                                              AS attribute6
                      ,aba.attribute7                                              AS attribute7
                      ,aba.attribute8                                              AS attribute8
                      ,aba.attribute9                                              AS attribute9
                      ,aba.attribute10                                             AS attribute10
                      ,aba.attribute11                                             AS attribute11
                      ,aba.attribute12                                             AS attribute12
                      ,aba.attribute13                                             AS attribute13
                      ,aba.attribute14                                             AS attribute14
                      ,aba.attribute15                                             AS attribute15
                      ,NULL                                                        AS operating_unit_name
                      --xxsh,hou.name                        AS operating_unit_name
               FROM
                   ...
                   --xxsh xxmx_supplier_scope_v                             xssv
                   --xxsh - just a temp replacement for the xssv scope view...take out when convenient
                       (SELECT  DISTINCT
                             pov.vendor_name
                            ,pov.vendor_id
                            ,pvs.vendor_site_id
                        ...
                        AND pov.vendor_name like 'A%')                    xssv
               WHERE
               ...
               AND aba.org_id = hou.organization_id
               AND pvs.vendor_id = xssv.vendor_id
--xxsh not in 11i        AND     xipbav.order_of_preference <= pn_i_OrderOfPreferenceLimit  --BLAKE no equiv in 11i
               AND NVL(aba.inactive_date,SYSDATE+1) > SYSDATE 
               AND SYSDATE BETWEEN abau.start_date AND NVL(abau.end_Date,SYSDATE+1)  --active Use
