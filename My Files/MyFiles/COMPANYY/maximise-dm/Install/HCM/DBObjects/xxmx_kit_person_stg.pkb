...
--** 1.5    09-AUG-22    Pallavi             Add materialized view to separate scope.
--******************************************************************************
--**
...
--
	...
	*****************************************************************/
    ...
    BEGIN
        ...
        SELECT  distinct pt_i_MigrationSetID                               
                     ...
                     ,gvv_leg_code				
                     --,pdf.registration_id
                     ,NVL(xxhcm.original_date_of_hire , xxhcm.start_date)
                     ,pdf.effective_end_Date
                     ,xxhcm.person_id
         from  per_disabilities_f@MXDM_NVIS_EXTRACT pdf
             ,fnd_lookup_values@MXDM_NVIS_EXTRACT flv
             ,xxmx_hcm_person_scope_mv xxhcm
         where pdf.person_id = xxhcm.person_id
         and xxhcm.effective_Start_Date BETWEEN pdf.effective_Start_Date and pdf.effective_end_Date
         and flv.lookup_type = 'DISABILITY_CATEGORY'
         and lookup_code = pdf.category
         and xxhcm.business_group_id = p_bg_id
         and Exists (select 1
                    FROM xxmx_per_persons_stg per
                    WHERE per.person_id = xxhcm.person_id)
		   ;		
        --
        gvv_ProgressIndicator := '0030';
        xxmx_utilities_pkg.log_module_message(  
       ...
       INSERT  
        INTO    XXMX_PER_MAT_ABSENCE_STG
               ( migration_set_id 		,
				...
				bg_name   				, 
				bg_id            		,
                PER_ABS_MAT_ID  ,       -- Added by Ayush Rathore for Task 1838
				PERSONNUMBER 	,
				ABSENCETYPEID	,
				ABSENCESTATUS	,
				APPROVALSTATUS	,
        ...
        SELECT  distinct pt_i_MigrationSetID                               
               ...
               ,p_bg_name
               ,p_bg_id
               ,SSPM.MATERNITY_ID       -- Added by Ayush Rathore for Task 1838
               ,A.EMPLOYEE_NUMBER 
               ,Y.ABSENCE_ATTENDANCE_TYPE_ID
               ,NULL
               ,x.Approval_status
    ...
    BEGIN
      ...
      SELECT  distinct pt_i_MigrationSetID                               
		...
		FROM ( SELECT papf.employee_number			
				...
				,'Y'
				,papf.person_id
				,XXMX_KIT_PERSON_STG.EXPORT_DFF_SEGMENT_VAL
                                 (person_id
	...
	*****************************************************************/
    ...
    BEGIN
        ...
        SELECT  distinct pt_i_MigrationSetID                               
                ...
                ,NVL(per_all_people_f.original_date_of_hire , per_all_people_f.start_date)
                ,NVL(per_all_people_f.effective_end_Date,'31-DEC-4712')
        ...
        FROM     xxmx_hcm_person_scope_mv per_all_people_f
    ...
    BEGIN
        ...
        SELECT   pt_i_MigrationSetID                               
               ...
               ,'CNT_'||papf1.employee_number||'_'||pcr.contact_person_id
               ,NVL(papf1.employee_number,-1)
        from  per_All_people_f@MXDM_NVIS_EXTRACT per_all_people_f --Contact
             ,per_person_type_usages_f@MXDM_NVIS_EXTRACT  pptu
             ,per_person_types@MXDM_NVIS_EXTRACT  ppt
             ,xxmx_hcm_person_scope_mv papf1 --Employee
             ,per_contact_relationships@MXDM_NVIS_EXTRACT PCR --Relation
        where PCR.contact_person_id= per_all_people_f.person_id
        and papf1.person_id= PCR.person_id
        and upper(ppt.system_person_type) = 'OTHER'
        AND per_all_people_f.person_id = pptu.person_id
        and ppt.person_type_id  = pptu.person_type_id
        AND per_all_people_f.effective_start_date BETWEEN  pptu.effective_Start_Date and pptu.effective_end_date
    ...
    BEGIN
		...
		SELECT distinct  -- Added Distinct to remove multiple Date Track Records V1.1
            ...
            ,gvv_leg_code
				...
				,per_all_people_f.employee_number
      FROM XXMX_HCM_PERSON_SCOPE_MV per_all_people_f
		WHERE   per_all_people_f.national_identifier IS NOT NULL
		AND     per_all_people_f.business_group_id = p_bg_id
		AND exists ( Select 1
					 FROM xxmx_per_persons_stg xxper
					 WHERE xxper.person_id = per_all_people_f.person_id)
	...
	**************************************************/
       ...
       SELECT * FROM 
        (
		   ...
		   SELECT   distinct pt_i_MigrationSetID                               
				...
				FROM  per_contact papf,
                  per_All_people_f@MXDM_NVIS_EXTRACT papf1,
                	per_phones@MXDM_NVIS_EXTRACT a
            WHERE  Trunc(papf.effective_start_Date) between a.date_From and NVL(a.date_to,'31-DEC-4712')
    ...
    BEGIN
		...
		AND   papf.effective_Start_Date BETWEEN pa.date_from AND NVL(pa.date_to,'31-DEC-4712')
		AND EXISTS ( SELECT 1
                  FROM xxmx_per_persons_stg xxper
                  WHERE xxper.person_id = PAPF.person_id)
    ...
    BEGIN 
        ...
        INTO    xxmx_per_addr_usg_f_stg            
         ...
         SELECT  pt_i_MigrationSetID                               
               ...
               ,per_addresses.address_id  ||'_ADDRESS_USAGE'
               ,'NOTUSED'
        FROM    per_addresses@MXDM_NVIS_EXTRACT per_addresses
        WHERE   per_addresses.primary_flag = 'Y'
        AND     trunc (sysdate) BETWEEN per_addresses.date_from  AND     nvl (per_addresses.date_to,to_date ('4712-31-12','YYYY-DD-MM'))
        AND     per_addresses.business_group_id = p_bg_id
		  AND EXISTS ( SELECT 1
                    FROM xxmx_per_persons_stg xxper
    ...
    BEGIN     
        ...
        SELECT   pt_i_MigrationSetID                               
                ...
                ,per_all_people_f.person_id person_id
                ,gvv_leg_code
                ,nvl(per_all_people_f.per_information1,'US_ETHNIC_GROUP') ethnicity
                ,'Y' primary_flag
                ,per_all_people_f.per_information14 declarer_id
                ,per_all_people_f.person_id --|| '_ETHNICITY'
				    ,per_all_people_f.employee_number
        FROM xxmx_hcm_person_scope_mv per_all_people_f
    ...
    BEGIN        
        ...
        SELECT  pt_i_MigrationSetID                               
               ...
               ,p_bg_name
               ,p_bg_id
               ,person_id  --|| '_LEGISLATIVE' person_legislative_id
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg,
         ...
         WHERE xxmx_per_persons_stg.person_id = c19_person_id
        ;
        --
        gvv_ProgressIndicator := '0030';
        xxmx_utilities_pkg.log_module_message(  
    ...
    BEGIN       
		...
		SELECT distinct  pt_i_MigrationSetID                               
                ...
                ,p_bg_name
                ,p_bg_id
                ,person_id --|| '_NAME'                             -- Commented by Ayush Rathore for Task 1838
                          person_name_id
                ,per_all_people_f.effective_start_date
                ...
                ,NVL(per_all_people_f.title,'NOTITLE') title
                ,per_all_people_f.pre_name_adjunct 
                ,per_all_people_f.suffix
                ,per_all_people_f.known_as
                ,per_all_people_f.previous_last_name
...
PROCEDURE export_per_Passport
    ...
    BEGIN       
		...
		SELECT   pt_i_MigrationSetID                               
                ...
                ,to_number(person_id)
				,information_type
				,NVL(to_char(PASSPORT_NUMBER),'NOPASSPORT')
				,Issue_Date
				,Expiry_Date
				,''
				,ISSUING_COUNTRY
                ...
                ,name
        FROM (Select 
					 NVL(NVL(papf.employee_number,papf.npw_number),'NoPersonNum') employee_number
					,pei.Person_id
    ...
    BEGIN       
		...
		WHERE per_all_people_f.nationality IS NOT NULL
