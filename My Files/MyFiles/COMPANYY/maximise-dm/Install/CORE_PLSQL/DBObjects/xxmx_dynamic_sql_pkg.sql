...
IS
     ...
     **   Vsn  Change Date  Changed By          Change Description
     ** -----  -----------  ------------------  -----------------------------------
     **   1.0  09-JUL-2020  Ian S. Vickerstaff  Created for Cloudbridge.
     **   1.1  22-SEP-2022  Pallavi Kanajar     Added changes for Fusion Null Target value,
     **                                         Handle single quote values in Transformation
     **   1.2  15-NOV-2023  Pallavi Kanajar     Changes to Transfer_stg_data for non EBS
     ******************************************************************************
     **
     ...
     ** of Seeded Extensions written by the Cloudbridge Development Team.
     **
...
AS
     ...
     */
     ...
     BEGIN
          ...
          --
          /*
          ** For Client Data being loaded from Data File, this is the first Cloudbridge
          ...
          LOOP
               ...
               THEN
                    ...
                    vv_IDLabel := 'File Set ID "'
                                ...
                                ||'"';
                    --
		--  vv_IDCondition := 'file_set_id = :1 '; --Pallavi V1.2
...
--                                USING pt_i_FileSetID; V1.2
          ...
          LOOP
               ...
               */
               ...
               ELSE
			     ...
			     IF( INSTR(vt_SourceValue ,'''',1) >0) 
                             THEN

                                vt_SourceValue := REPLACE(vt_SourceValue ,'''',''''''); 
     ...
     **
     ** Seeded extensions are those written by the Cloudbridge Development
     ** Team.
     **
     ...
     ** tables.
     **
     ** The Cloudbridge Team can provide templates for Extension Procedures.     
     ***********************************************************************
     */
     --
     PROCEDURE execute_extension
          ...
          ELSE
               ...
               */
               ...
               THEN
                    ...
                    **
                    ** Cloudbridge procedures which process the Client Data will log messages using the File Set ID.
                    **
               ...
               ELSE /* STG_POPULATION_METHOD_PARAMETER is "DB_LINK" */
                    --
                    /*
                    ** Data extracted by Cloudbridge PL/SQL procedures via DB Link will have the Migration Set ID
   ...
   IS
          ...
          */
          ...
          --  Commit;
            ...
            FROM   (
    		    ...
    		    and       alltc.OWNER  = xxt.schema_name
		   /* AND not exists ( select 1 from xxmx_xfm_Tables xt, xxmx_xfm_Table_columns xtc
                                    where xt.xfm_table_id = xtc.xfm_Table_id 
                                    and xtc.column_name = alltc.column_name
                                    and xt.table_name = alltc.table_name) */
     ...
     *********************/
     ...
     BEGIN
		...
		/*
		 ,(select xfm_Table_id from xxmx_xfm_Tables where table_name = REPLACE(alltc.table_name,'STG','XFM')) xfm_Table_id,
                    (Select xfm_table_column_id 
                    from xxmx_xfm_table_columns 
                    where xfm_table_id IN(select xfm_Table_id 
                                            from xxmx_xfm_Tables
                                            where table_name = REPLACE(alltc.table_name,'STG','XFM'))
                    and column_name =  alltc.column_name) xfm_table_column_id  */
                 FROM      all_tab_columns  alltc
                          ,xxmx_stg_tables  xst
                 WHERE     1 = 1
                 AND       alltc.table_name = UPPER(xst.table_name)
                 AND       xst.schema_name  = 'XXMX_STG'
		 and       alltc.OWNER  = xst.schema_name
	      /* AND not exists ( select 1 from xxmx_stg_Tables xt, xxmx_stg_Table_columns xtc
                                    where xt.stg_table_id = xtc.stg_Table_id 
                                    and xtc.column_name = alltc.column_name
                                    and xt.table_name = alltc.table_name) */
