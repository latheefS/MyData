create or replace PACKAGE     "XXMX_FIN_STG_EXTRACT_PKG" 
AS
     --
     /*
     *****************************************************************************
     ...
     **
     **
     ** FILENAME  :  xxmx_fin_stg_extract_pkg.sql
     **
     ...
     **
     ** PURPOSE   :  This script installs the package for the Cloudbridge AR Transactions
     **              Data Migration.
     **
     ** NOTES     :
     **
     ...
     **   Vsn  Change Date  Changed By          Change Description
     ** -----  -----------  ------------------  -----------------------------------
     **   1.0  09-Nov-2021  Pallavi Kanajar  Created for Cloudbridge.
     **
     **   1.1  01-JAN-2023  Pallavi Kanajar  Batching Process for large file
	 **   1.2  13-June-2023 Shireesha TR     Batching Process changes 
	 **   1.3  09-Oct-2023  Shireesha TR     Added Batch_flag condition for stg main procedure
	 **   1.4  18-Oct-2023  Shireesha TR     Added pt_batch_count in stg_main procedure 
     ******************************************************************************
     **
     ...
     */
     ...
     PROCEDURE stg_main
                 (
                    ...
                    ,pt_iteration                    IN      VARCHAR2 DEFAULT NULL
					,pt_batch_count                  IN      NUMBER DEFAULT NULL
                    --,pt_instance_id                  IN      VARCHAR2 DEFAULT NULL
                    );
     --
     --
     /*
...
END XXMX_FIN_STG_EXTRACT_PKG;
/

create or replace PACKAGE BODY             "XXMX_FIN_STG_EXTRACT_PKG" 
AS
     --
     /*
     **********************
     ...
     */
     --
     /*
     ** Cloudbridge Integration Globals
     */
     --
     /*
     ...
     */
     --
     gvt_PackageName                           CONSTANT xxmx_module_messages.package_name%TYPE       := 'XXMX_FIN_STG_EXTRACT_PKG';
 ...
 CURSOR batch_cur (
          ...
          IS 
              ...
              SELECT  sub_entity_seq,
                      sub_entity
              FROM xxmx_migration_metadata xmm
              WHERE     xmm.application_suite      = pt_ApplicationSuite
              AND       xmm.application            = pt_Application
              AND       xmm.business_entity        = pt_BusinessEntity
             -- AND SEQ_IN_FBDI_DATA  IS NOT NULL
              AND ENABLED_FLAG= 'Y'
		  ...
		  vbatchload                      VARCHAR2(5);
          vbatchcolumn                    VARCHAR2(200);
          vsubentity                      xxmx_migration_metadata.sub_entity%TYPE;
          vsubentityseq                   xxmx_migration_metadata.sub_entity_seq%TYPE; 	
          pt_start_time_extract            timestamp;  
          pt_end_time_extract              timestamp;
		  batch_flag                       varchar2(5);	
     ...
     BEGIN
	  ...
	  pt_start_time_extract:=sysdate;

          dbms_output.put_line('START'||pt_start_time_extract);
...
--                    AND  pt_i_FileSetID      IS NULL
                    ...
                    THEN
                         ...
                         RAISE e_ModuleError;
                         --
                    ELSIF pt_i_FileSetID IS NULL AND vt_FileSetID is NULL
                    THEN
                         --
               ...
               ELSE
                    ...
                    THEN
                         ...
                         */
                         ...
                         --
			  SELECT  SUBSTR(vt_BusinessEntity||'_'||to_char(SYSDATE,'DDMMRRHHMISS'),1,30)
                          INTO vt_FileSetID
                          FROM DUAL;
                         --1.1

                         xxmx_utilities_pkg.external_file_exists ( pt_i_ApplicationSuite => vt_ApplicationSuite
                                                                   ,pt_i_Application               => vt_Application
                                                                   ,pt_i_BusinessEntity            => vt_BusinessEntity
                                                                   ,pt_i_FileSetID                 => vt_FileSetID
                                                                   ,pv_o_ReturnStatus              => gvv_ReturnStatus 
                         ...
                         THEN
                              ...
                              gvt_ModuleMessage      := '- External File Check failed for Business_entity ';
                        ...
                        vt_MigrationSetID:=xxmx_utilities_pkg.get_migration_set_id(vt_MigrationSetName);
                        --1.1
                        ...
                        xxmx_utilities_pkg.log_module_message
                         ...
                         LOOP
                              ...
                              xxmx_utilities_pkg.log_module_message
                                   ...
                                   ,pt_i_ProcOrFuncName      => ct_ProcOrFuncName
                                   ,pt_i_ProgressIndicator   => gvv_ProgressIndicator
                                   ,pt_i_ModuleMessage       => '    - Calling Procedure xxmx_read_datafile '
                                                                ||' for Sub-Entity "'
                              ...
                              --1.1
                             xxmx_utilities_pkg.xxmx_read_datafile(pt_i_ApplicationSuite  => vt_ApplicationSuite
                                                                  ,pt_i_Application     => vt_Application
                                                                  ,pt_i_BusinessEntity  => vt_BusinessEntity
                             ...
                             xxmx_utilities_pkg.init_file_migration_details
                              (
                                      ...
                                      ,pt_i_SubEntity        => SubEntityMetadata_rec.sub_entity
                                      ,pt_i_FileSetID        => vt_FileSetID
                                      ,pt_i_ValidateStartTimestamp    => SYSTIMESTAMP
                              ); 
                              ...
                              END IF;
                              --
                              /* CONTROL TABLE INSERTION FOR DASHBOARD*/
                              /* Version 1.1*/
                            xxmx_utilities_pkg.INSERT_EXTRACT_CONTROL(p_Business_entity    => vt_BusinessEntity
                                                     ,p_Migration_set_id  => vt_MigrationSetID
                                                     ,p_sub_entity        => SubEntityMetadata_rec.sub_entity
                                                     ,p_iteration         => pt_iteration
                                                     ,p_extract_start_time=> pt_start_time_extract
                                                     ,p_extract_end_time  =>pt_end_time_extract);
                         ...
                         ELSE
                              ...
                              ** any Cloudbridge provided extensions (if any).
                              ** 
                              ...
                              */
                              ...
                              --
                              /*
                              ** After Cloudbridge seeded Extensions have been executed, any custom extensions developed
                    ...
                    ELSE /* STG_POPULATION_METHOD_PARAMETER is "DB_LINK" */
                         --
                         /*
                         ** For Client Data Extract using Cloudbridge PL/SQL packaged Extract 
                              ...
                              */
                              ...
                              LOOP
                                   ...
                                   */
                                   ...
                                   EXECUTE IMMEDIATE gvv_SQLStatement;
                                   --
  								   pt_end_time_extract:= sysdate;
                                   dbms_output.put_line('END'||pt_end_time_extract); 	
                                   /*
           cALL cONTROL TABLE iNSERT pROCEDURE*/
                                 xxmx_utilities_pkg.INSERT_EXTRACT_CONTROL(p_Business_entity    => vt_BusinessEntity
                                                     ,p_Migration_set_id  => vt_MigrationSetID
                                                     ,p_sub_entity        => SubEntityMetadata_rec.sub_entity
                                                     ,p_iteration         => pt_iteration
                              ...
                              ** we can now call any Cloudbridge provided extensions (if any).
                              ** 
                              ** Some extensions may only operate specifically on a single Sub-Entity table
                              ** and therefore we call them "Sub-Extensions".  However some extensions may
                              ** need to update several Sub-Entity STG tables at the same time so we call
                              ...
                              */
                              ...
                              /*
                              ** After Cloudbridge provided have been executed, any custom extensions developed
                              ** by V 1 Implementation Teams or Client Development Teams are executed (if any).
                              */
                              --
                              xxmx_utilities_pkg.log_module_message
	...
	Begin	
	  /* Batch_Process */
          SELECT
            batch_load INTO batch_flag
          FROM
            xxmx_migration_metadata
          WHERE
	    business_entity = vt_BusinessEntity
            and  application_suite = vt_ApplicationSuite
	...
	END;
          ...
          xxmx_utilities_pkg.log_module_message
                         ...
                         ,pt_i_PackageName       => gvt_PackageName
                         ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                         ,pt_i_ProgressIndicator => '0059'
                         ,pt_i_ModuleMessage     => 'Batching Process for Extract is enabled and initiated'                           
                         ,pt_i_OracleError       => NULL
                         );

          FOR batch_rec IN batch_cur(vt_ApplicationSuite,vt_Application,vt_BusinessEntity) LOOP
                  xxmx_utilities_pkg.batch_extract_data 
                        (vt_ApplicationSuite,vt_BusinessEntity,batch_rec.sub_entity,pt_batch_count,'EXTRACT');
          END LOOP;

          dbms_output.put_line('batch_process');  
 		  end if;
     ...
     */
     ...
     IS
	  ...
	  vt_filesetid			  VARCHAR2(30);
	  	  pt_start_time_transform         TIMESTAMP;
          pt_end_time_transform           TIMESTAMP; 
          --
          --*************************
          --** Exception Declarations
          --*************************
     ...
     BEGIN
          ...
          --
          gvt_Phase := 'TRANSFORM';
	  pt_start_time_transform:= sysdate; 
          dbms_output.put_line(pt_start_time_transform);
          --
          gvv_ProgressIndicator := '0010';
          --
          IF   NOT xxmx_utilities_pkg.valid_install
              ...
              LOOP
                   ...
                   */
                   ...
                   ELSE
                        ...
                        ** Call for Simple Tranformation only 
                        ** Table xxmx_simple_transforms
                        ********************************************************
                        */
                        --
                      ...
                      --
                      --
 					pt_end_time_transform:=sysdate;
                    xxmx_utilities_pkg.insert_transform_control (
                       p_business_entity       => pt_i_BusinessEntity,
                        p_migration_set_id      => gvt_MigrationSetID,
                        p_sub_entity            => Subentity_rec.sub_entity,
                        p_iteration             => pt_iteration,
                        p_transform_start_time  => pt_start_time_transform,
                        p_transform_end_time    => pt_end_time_transform
                    );
            END LOOP; --** SubEntity_cur LOOP

                      --
                      /*
                      ** We now call any Cloudbridge provided extensions (if any) for the Transform Phase
                      ...
                      */
                      ...
                      END; -- Extension transformaiton 
                         ...
                         xxmx_utilities_pkg.log_module_message
                              ...
                              ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                              ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                              ,pt_i_ModuleMessage     => '    - Transfer/Simple Transform processing for Sub-Entity  "'
                                                       ||'" complete.'
                              ,pt_i_OracleError       => NULL
                              );
                         --
...
END XXMX_FIN_STG_EXTRACT_PKG;
/
Show errors package body XXMX_FIN_STG_EXTRACT_PKG;
/