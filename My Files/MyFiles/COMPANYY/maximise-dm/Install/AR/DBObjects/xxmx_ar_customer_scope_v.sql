...
BEGIN
    DropView ('XXMX_CUSTOMER_SCOPE_V') ;
    DropView ('XXMX_CUSTOMER_SCOPE_TEMP_STG') ;
END;
/

  CREATE MATERIALIZED VIEW "XXMX_CORE"."XXMX_CUSTOMER_SCOPE_V" ("ORG_ID", "CUST_ACCOUNT_ID", "ACCOUNT_NUMBER", "ACCOUNT_PARTY_ID", "ACCOUNT_STATUS", "CUST_ACCT_SITE_ID", "CUST_ACCT_SITE_STATUS", "PARTY_SITE_ID", "PARTY_SITE_NUMBER", "PARTY_NAME", "SITE_PARTY_ID", "LOCATION_ID")
  ...
  AS WITH
    ...
    AS
      ( 
       ...
       FROM   apps.ar_payment_schedules_all@MXDM_NVIS_EXTRACT  apsa,
            ...
            xxmx_parameter_conversions xpc
       WHERE  1 = 1
       and apsa.org_id = eoil.org_id
       AND  (  (apsa.status               = 'OP'  AND    apsa.amount_due_remaining > 0)
       OR   TRUNC(apsa.last_update_date) >= xpc.customer_scope_start_date
       ))
    ,open_balance_trxs
     AS
        ...
        (  
           ...
           WHERE   1 = 1
           and  op.customer_trx_id = rcta.customer_trx_id
    ...
    ,customers_with_no_activity
     AS
          ...
          (
           ...
           --AND     TRUNC(cwna_hca.last_update_date)   >= cwna_xpc.no_activity_threshold_date --Milind changed to created in last 2 months below
           AND     TRUNC(cwna_hca.creation_date)   >= cwna_xpc.no_activity_threshold_date
    ...
    AS 
        (
            select
            hzcasa.org_id,
            hzca.cust_account_id  AS customer_id,
            hzcasa.cust_acct_site_id
        from
            eligible_org_id_list                            orgs,
            apps.hz_cust_accounts@MXDM_NVIS_EXTRACT              hzca,
            apps.hz_cust_acct_sites_all@MXDM_NVIS_EXTRACT        hzcasa,
            apps.hz_party_sites@MXDM_NVIS_EXTRACT                hzps
        where   hzca.status                             = 'A'
        ...
        (
            select 1
            from
                apps.pa_projects_all@MXDM_NVIS_EXTRACT       papa,
                apps.pa_proj_statuses_v@MXDM_NVIS_EXTRACT    paps,
                apps.pa_project_customers@MXDM_NVIS_EXTRACT  papc
...
AND     hcasa.org_id              = ccl.org_id
AND     hcasa.cust_account_id     = hca.cust_account_id
AND     hcasa.cust_acct_site_id	  = ccl.cust_acct_site_id
...
AND     NOT EXISTS (
   ...
   COMMENT ON MATERIALIZED VIEW "XXMX_CORE"."XXMX_CUSTOMER_SCOPE_V"  IS 'snapshot table for snapshot XXMX_CORE.XXMX_CUSTOMER_SCOPE_V';
/



 --
CREATE MATERIALIZED VIEW XXMX_CUSTOMER_SCOPE_TEMP_STG AS
 SELECT       org_id
             ,cust_account_id
             ,account_number
             ,account_party_id
             ,account_status as HCA_STATUS
             ,cust_acct_site_id
             ,cust_acct_site_status as HCASA_STATUS
             ,party_site_id
			 ,party_site_number
			 ,party_name
             ,site_party_id
             ,location_id
FROM         xxmx_core.xxmx_customer_scope_v;
