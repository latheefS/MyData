...
def generate_data_summary(csv_file_path):
    try:
        summary = "Based on the provided CSV data and focusing on count generation for each module here's a comprehensive summary:\n"
        dataframe = pd.read_csv(csv_file_path)
        #1
        summary += f"\n1. Total Number of Tests:\n * Total Tests Conducted:{dataframe.shape[0]}"
        #2
        pf_v = dataframe['result'].value_counts()
        summary += f"\n\n2. Number of Tests Passed and Failed:\n * Passed:{pf_v.values.tolist()[0]}\n * Failed:{pf_v.values.tolist()[1]}"
        #3
        test_area_counts = dataframe['test_area'].value_counts().to_dict()
        result_counts = dataframe.groupby(['test_area', 'result']).size().unstack(fill_value=0)

        summary += f"\n\n3. Breakdown by Test Area with Pass Rates:"
        for test_area, total_tests in test_area_counts.items():
            ...
            summary += f"\n{test_area}: {total_tests} tests ({passed} Passed, {failed} Failed) - Pass Rate: {pass_rate:.2%}"
        return summary
    except:
        print(f"Exception Occured:\n{traceback.format_exc()}") 
...
try:
    ...
    csv_file_path = r"C:\Users\LatheefS\Pictures\PythonAI\input\x_ve1_maximise_test_results.csv"
    ...
    doc = Document(r"C:\Users\LatheefS\Pictures\PythonAI\input\AT_Report_Template.docx")
    ...
    colors = {'Passed': '#7bdcb5', 'Failed': '#f78da7',
              'Others': '#fcb900'}  # Add more colors for other values if needed
    ...
    test_s = generate_data_summary(csv_file_path)
    add_todoc_test_s = doc.add_paragraph(test_s)
    add_todoc_test_s.paragraph_format.space_before = Pt(10)
    add_todoc_test_s.paragraph_format.space_after = Pt(10)
    print("Test summary is saved in the documents")

    promptreply = CallOpenAi.output
    ...
    table3 = doc.add_table(rows=3, cols=3, style="Table Grid")
    column_headers1 = ["Criteria", "Met / not met", "Mitigation factors (if not met)"]
    header_row1 = table3.rows[0]
    for col_idx, header_text in enumerate(column_headers1):
        cell = header_row1.cells[col_idx]
        cell.text = header_text
        cell.paragraphs[0].runs[0].bold = True
    ...
    table4 = doc.add_table(rows=3, cols=5, style="Table Grid")
    column_headers1 = ["ID", "Title", "Module / Area", "Status", "Comment"]
    header_row1 = table4.rows[0]
    for col_idx, header_text in enumerate(column_headers1):
        cell = header_row1.cells[col_idx]
        cell.text = header_text
        cell.paragraphs[0].runs[0].bold = True
    ...
    table5 = doc.add_table(rows=4, cols=4, style="Table Grid")
    column_headers2 = ["Azure DevOps Bug Number", "Title", "Defect Status", "Resolution"]
    header_row2 = table5.rows[0]
    for col_idx, header_text in enumerate(column_headers2):
        cell = header_row2.cells[col_idx]
        cell.text = header_text
        cell.paragraphs[0].runs[0].bold = True
    ...
    column_headers1 = ["Issue", "Proposed Action", "Owner"]
    header_row1 = table6.rows[0]
    for col_idx, header_text in enumerate(column_headers1):
        cell = header_row1.cells[col_idx]
        cell.text = header_text
        cell.paragraphs[0].runs[0].bold = True
    ...
    doc.save(r"C:\Users\LatheefS\Pictures\PythonAI\output\AT_Audit_Report.docx")
