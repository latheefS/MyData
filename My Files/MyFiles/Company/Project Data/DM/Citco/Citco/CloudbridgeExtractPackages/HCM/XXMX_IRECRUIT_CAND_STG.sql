CREATE OR REPLACE PACKAGE xxmx_irecruit_cand_stg AS
...
CREATE OR REPLACE PACKAGE BODY  xxmx_irecruit_cand_stg AS
--*****************************************************************************
--**
--**                 Copyright (c) 2020 Version 1
--**
...
--                dbms_output.put_line(' #' ||r_package_name.v_package ||' #'|| l_bg_name || '  #' || l_bg_id || '  #' || vt_MigrationSetID || '  #' || pt_i_MigrationSetName  );
    ...
    BEGIN
        ...
        SELECT 
				...
				,ppf.person_id
				,CASE
                     WHEN ppf.applicant_number IS NOT NULL THEN
                    'CAN' || ppf.applicant_number
                 ELSE
                     ppf.applicant_number
                 END AS CANDIDATE_NUMBER
				,'ACTIVE'
				,NULL
				--,TO_DATE(to_CHAR(ppf.effective_start_date,'YYYY/MM/DD')) AS Availability_Date
				--,TO_DATE(to_CHAR(ppf.effective_start_date,'YYYY/MM/DD')) AS SEARCH_DATE
...
PROCEDURE export_irecruit_cand_attmt
    ...
    BEGIN
        ...
        --  
        DELETE 
        FROM    XXMX_STG.XXMX_HCM_IREC_CAN_ATTMT_STG    
        WHERE   bg_id    = p_bg_id    ;
	    ...
	    Insert into XXMX_STG.XXMX_HCM_IREC_CAN_ATTMT_STG
		   (
				FILE_SET_ID			      ,				
				MIGRATION_SET_ID 		  , 			
				MIGRATION_SET_NAME 		  ,			
...
--EXEC xxmx_irecruit_cand_stg.stg_main('max','MAR01');
----
--select * from xxmx_hcm_irec_can_NAME_stg WHERE migration_set_id=1092;
--
--delete from xxmx_hcm_irec_can_NAME_stg where migration_set_id=1058;
--
--
