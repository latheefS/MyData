--------------------------------------------------------
--  DDL for Package Body XXMX_KIT_WORKER_STG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_KIT_WORKER_STG" AS
	 --
     --//================================================================================
     --// Version1
     --// $Id:$
	 ...
	 --// 1.6          Dhanu Gundala      17/02/2022         Updated for Citco + Performance improvements
     --// 1.6          Vamsi              23/08/2023         Removed Hardcoded End Date for Employee Assignments
     --//================================================================================
     --
    ...
    BEGIN 
        ...
        WHERE parameter_code = 'PEOPLE_GROUP';
             ...
             lv_sql:= 'UPDATE XXMX_PER_ASSIGNMENTS_M_STG ASG '
                        ...
                        ||' (SELECT '||lv_fusion|| ' FROM PAY_PEOPLE_GROUPS@XXMX_EXTRACT '
	...
	*************************************************/	
    ...
    BEGIN 
         BEGIN
				...
				FROM XXMX_HCM_ORG_LEGALEMP_MV xep
				WHERE xep.organization_id = p_organization_id
					And rownum=1
					;

				/*FROM
					hr_all_organization_units@XXMX_EXTRACT      o,
					hr_all_organization_units_tl@XXMX_EXTRACT   otl,
					hr_organization_information@XXMX_EXTRACT    o2,
					hr_organization_information@XXMX_EXTRACT    o3,
					xle_entity_profiles@XXMX_EXTRACT            xep
				WHERE
          ...
          /*  SELECT distinct 
			...
			FROM
					xle_entity_profiles@XXMX_EXTRACT           xep,
					xle_registrations@XXMX_EXTRACT             reg,
					hr_all_organization_units@XXMX_EXTRACT      hr_ou,
					hr_locations_all@XXMX_EXTRACT               hr_loc     	 
...
*/
    ...
    PROCEDURE load_usr_wrk_lkp
        ...
        (c_lkp_type IN VARCHAR2) IS
            ...
            --fnd_lookup_values_vl@XXMX_EXTRACT flv
    ...
    PROCEDURE export_period_of_service
        (p_bg_name                          IN          VARCHAR2
        ,p_bg_id                            IN          NUMBER
        ,pt_i_MigrationSetID                IN          xxmx_migration_headers.migration_set_id%TYPE
        ,pt_i_MigrationSetName              IN          xxmx_migration_headers.migration_set_name%TYPE ) IS

        cv_ProcOrFuncName                   CONSTANT    VARCHAR2(30) := 'export_period_of_service'; 
        ct_Phase                            CONSTANT    xxmx_module_messages.phase%TYPE  := 'EXTRACT';
    ...
    BEGIN
      ...
      /*  IF  NVL( lvd_migration_date_from, NULL ) is null or  NVL( lvd_prev_tax_year_date, NULL ) is null
        ...
        INTO    xxmx_per_pos_wr_stg
				...
				,attribute1
				,personnumber)
        SELECT  /*+ driving_site(table alias name) */ pt_i_MigrationSetID                               
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg
                ...
                SELECT  per_all_people_f.rehire_authorizor c29_rehire_authorizor
					...
					,(SELECT distinct Organization_id FROM per_all_assignments_f@XXMX_EXTRACT paaf
					WHERE period_of_service_id = per_periods_of_service.period_of_service_id
					AND person_id = per_all_people_f.person_id
					AND rownum=1
					AND gvd_migration_date_from BETWEEN paaf.effective_start_date   AND     paaf.effective_end_date) s_organization_id
                    ,per_periods_of_service.period_of_service_id s0_period_of_service_id
					,(SELECT Payroll_name
					  FROM PAY_PAYROLLS_F@XXMX_EXTRACT ppf,Per_all_assignments_f@XXMX_EXTRACT paaf
					  WHERE PPF.PAYROLL_ID = paaf.payroll_id
					  and paaf.person_id = per_all_people_f.person_id
					  and paaf.primary_flag = 'Y'
					  and rownum= 1
					  and gvd_migration_date_from BETWEEN paaf.effective_start_date and paaf.effective_end_date
					  AND gvd_migration_date_from BETWEEN ppf.effective_start_date and ppf.effective_end_date)
					  Payroll_name 
					  --,per_all_people_f.employee_number personnumber
					  ,DECODE(per_all_people_f.system_person_type,'EMP',per_all_people_f.employee_number
					                                             ,'APL',per_all_people_f.applicant_number
																 ,'CWK',per_all_people_f.npw_number,-1) personnumber
                FROM per_periods_of_service@XXMX_EXTRACT per_periods_of_service
                ...
                AND     per_periods_of_service.period_of_service_id =  (SELECT MAX(period_of_service_id) FROM per_periods_of_service@XXMX_EXTRACT ppos1
                                                                      WHERE per_periods_of_service.person_id = ppos1.person_id 
                                                                                            )           
                --DG 06/06/22  Updated for Citco. Get the latest POS
                       --  AND    ( (per_periods_of_service.date_start BETWEEN per_all_people_f.effective_start_date
                         --                                        AND     per_all_people_f.effective_end_date)
                                 --   OR (per_periods_of_service.date_start =  per_all_people_f.original_date_of_hire)
                                                   --              )
/*                AND     (
    ...
    BEGIN
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg
                ...
                FROM    per_periods_of_placement@XXMX_EXTRACT per_periods_of_placement
                    --,per_all_people_f@XXMX_EXTRACT per_all_people_f
					,XXMX_HCM_CURRENT_PERSON_MV per_all_people_f
                    ,hr_all_organization_units@XXMX_EXTRACT horg
                WHERE 
                (
                                per_periods_of_placement.person_id = per_all_people_f.person_id
                                                AND     per_periods_of_placement.period_of_placement_id =  (SELECT MAX(period_of_placement_id) FROM per_periods_of_placement@XXMX_EXTRACT ppos1
                                                                      WHERE per_periods_of_placement.person_id = ppos1.person_id 
                                                                                            )

                        -- AND     per_periods_of_placement.date_start BETWEEN per_all_people_f.effective_start_date
                        --                                             AND     per_all_people_f.effective_end_date
                      --  AND     (
                            --            lvd_migration_date_from <  per_all_people_f.effective_end_date  
                          --      OR      lvd_prev_tax_year_date  <  per_all_people_f.effective_end_date                       
                ...
                AND     (
                                horg.organization_id = per_all_people_f.business_group_id
...
and xxmx_per_persons_stg.person_type='CWK'
                )
        ;
        --
        -- 
    ...
    BEGIN
     ...
     gvd_migration_date_from := TO_DATE(gvv_migration_date_from,'YYYY-MM-DD');
        ...
        gvd_prev_tax_year_date := TO_DATE(gvv_prev_tax_year_date,'YYYY-MM-DD');      
         --
        gvv_ProgressIndicator := '0005';
   /* IF  NVL( lvd_migration_date_from, NULL ) is null or NVL( lvd_migration_date_to, NULL ) is null or NVL( lvd_prev_tax_year_date, NULL ) is null
        THEN 
                 --
                 xxmx_utilities_pkg.log_module_message
                         (
		...
		SELECT  /*+ driving_site(table alias name) */ distinct  pt_i_MigrationSetID                               
                ,pt_i_MigrationSetName                               
                ,'EXTRACTED'  
                ,p_bg_name
                ,p_bg_id
				,c72_bargaining_unit_code bargaining_unit_code
				,date_probation_end
				,c75_employee_category employee_category
				,c76_employment_category employment_category
				,Org_name
				,EXPENSE_CHECK_ADDRESS
				,Grade_Code
				,position_code
                ...
                ,'ACTIVE_PROCESS' assignment_status_type
                ,'Y' auto_end_flag	
                ,effective_sequence
                ,Case When business_group_id is NOT NULL THEN 
					xxmx_kit_worker_stg.get_legal_employer_name(business_group_id)
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg
            ,(
                SELECT distinct  per_all_assignments_f.change_reason
                    ...
                    , per_all_assignments_f.effective_start_date c8_effective_start_date---to_date ('01-09-2019','DD-MM-YYYY') 
                    ...
                    ,per_all_assignments_f.person_type_id c24_person_type_id					
                    ...
                    ,per_all_assignments_f.pay_system_status c19_pay_system_status
                    ,per_all_assignments_f.per_system_status c20_per_system_status					
                    ...
                    ,per_all_Assignments_f.ppos_id s0_period_of_placement_id
                    ,per_all_Assignments_f.final_process_date final_process_date					
                    ...
                    ,per_all_assignments_f.employee_number
					...
					,(SELECT NAME 
					  ...
					  AND gvd_migration_date_from BETWEEN DATE_FROM AND NVL(DATE_TO, '31-DEC-4712'))
					  business_unit_name
					,(SELECT Reference 
					  FROM per_contracts_f@XXMX_EXTRACT 
					  WHERE contract_id = per_all_assignments_f.contract_id
					  AND gvd_migration_date_from BETWEEN effective_Start_date AND NVL(effective_end_date, '31-DEC-4712'))
					  contract_name
					,(SELECT NAME 
					  FROM HR_ALL_ORGANIZATION_UNITS@XXMX_EXTRACT 
					  ...
					  Org_name
					,(SELECT Name 
					 FROM per_grades_tl@XXMX_EXTRACT 
                    ...
                    ,per_all_assignments_f.EXPENSE_CHECK_SEND_TO_ADDRESS "EXPENSE_CHECK_ADDRESS"
					,(Select Name from per_jobs_tl@XXMX_EXTRACT where job_id  =  per_All_assignments_f.job_id) JOB_CODE
					,(Select location_code 
					  from hr_locations_all@XXMX_EXTRACT
					  where location_id  =  per_All_assignments_f.location_id)
					location_code
					--,per_periods_of_placement.Date_start
                    ,per_all_Assignments_f.Date_start
					,(Select name from HR_ALL_POSITIONS_F@XXMX_EXTRACT  HAPF
					  WHERE position_id = per_All_assignments_f.position_id
					  AND  gvd_migration_date_from BETWEEN HAPF.effective_Start_date AND HAPF.effective_end_date)
					  POSITION_CODE
					--,per_person_types.SYSTEM_PERSON_TYPE
					--,per_person_types.USER_PERSON_TYPE
                    ,PER_ALL_ASSIGNMENTS_F.SYSTEM_PERSON_TYPE
					,PER_ALL_ASSIGNMENTS_F.USER_PERSON_TYPE
				--	,PER_ALL_ASSIGNMENTS_F.RETIREMENT_DATE
					,(SELECT psp.spinal_point
					from  PER_SPINAL_POINT_STEPS_F@XXMX_EXTRACT	PSPSF
						, per_spinal_points@XXMX_EXTRACT PSP
						,per_grade_spines_f@XXMX_EXTRACT pgsf 
					where Step_id  = PER_ALL_ASSIGNMENTS_F.SPECIAL_CEILING_STEP_ID
					and PSP.spinal_point_id =PSPSF.spinal_point_id
					and  gvd_migration_date_from BETWEEN pgsf.effective_Start_date AND pgsf.effective_end_date				
					AND psp.parent_spine_id = pgsf.parent_spine_id
					and pgsf.grade_id = PER_ALL_ASSIGNMENTS_F.grade_id
					and gvd_migration_date_from between PSPSF.effective_Start_date and PSPSF.effective_end_date)
					SPECIAL_CEILING_STEP_NAME
					,PER_ALL_ASSIGNMENTS_F.TIME_NORMAL_FINISH
					,PER_ALL_ASSIGNMENTS_F.TIME_NORMAL_START
					,PER_ALL_ASSIGNMENTS_F.PEOPLE_GROUP_ID
					,(SELECT NAME FROM per_collective_agreements@XXMX_EXTRACT PCA 
					  WHERE  collective_agreement_id = per_All_Assignments_f.COLLECTIVE_AGREEMENT_ID 
					  AND gvd_migration_date_from BETWEEN PCA.Start_Date AND PCA.end_date)
					  COLLECTIVE_AGREEMENT_NAME
					,(SELECT Name 
					  FROM BEN_PGM_F@XXMX_EXTRACT BPF
					  WHERE PGM_ID = per_all_assignments_f.GRADE_LADDER_PGM_ID
					  AND gvd_migration_date_from BETWEEN BPF.effective_start_Date AND BPF.effective_end_date)
					GRADE_LADDER_PGM_NAME
					,(SELECT Payroll_name
					  FROM PAY_PAYROLLS_F@XXMX_EXTRACT ppf
					  WHERE PAYROLL_ID = per_all_assignments_f.payroll_id
					  AND gvd_migration_date_from BETWEEN ppf.effective_start_date and ppf.effective_end_date)
					  Payroll_name 
                 ,per_all_assignments_f.default_code_comb_id
				 ,'NA' DEFAULT_EXPENSE_ACCOUNT
				 --,NVL(per_all_Assignments_f.employee_number,nvl(per_all_Assignments_f.npw_number,-1)) personnumber
				 ,DECODE(per_all_Assignments_f.system_person_type,'EMP',per_all_Assignments_f.employee_number
				                                                 ,'CWK',per_all_Assignments_f.npw_number,-1) personnumber
                 ,per_all_assignments_f.assignment_type
                 ,(select NVL(decode(attribute11,'AI','F',attribute11),'A') from per_all_people_f@xxmx_extract papf
                   where papf.person_id =  per_all_assignments_f.person_id
                   and gvd_migration_date_from BETWEEN papf.effective_start_date and papf.effective_end_date) head_class_count
                 ,(SELECT NAME 
					  FROM HR_ALL_ORGANIZATION_UNITS@XXMX_EXTRACT 
					  WHERE organization_id = per_all_assignments_f.ass_attribute4
					  AND gvd_migration_date_from BETWEEN DATE_FROM AND NVL(DATE_TO, '31-DEC-4712')) company_of_recharge
                 ,LTRIM(per_all_assignments_f.ass_attribute8,'0')    payroll_providerid
                FROM  
					XXMX_HCM_CURRENT_ASG_MV per_all_assignments_f
					) c_per_all_assignment
                WHERE c_per_all_assignment.c21_s_person_id = xxmx_per_persons_stg.person_id
                 AND c_per_all_assignment.assignment_type = 'C'
                 AND   c_per_all_assignment.business_group_id = p_bg_id
                 ;
				/*	per_person_type_usages_f@XXMX_EXTRACT per_person_type_usages_f
                    ...
                    ,per_all_people_f@XXMX_EXTRACT per_all_people_f
                    ,per_person_types@XXMX_EXTRACT per_person_types
                    ,per_periods_of_placement@XXMX_EXTRACT per_periods_of_placement
                    --,hr_all_organization_units@XXMX_EXTRACT horg
                ...
                and   per_all_assignments_f.effective_start_date IN(SELECT MAX ( paaf.effective_start_date)
                                                                    FROM per_all_assignments_f@XXMX_EXTRACT paaf
                                                                    WHERE paaf.person_id = per_all_assignments_f.person_id
																	and paaf.assignment_id =per_All_assignments_f.assignment_id
                                                                    AND ((
																			TRUNC(paaf.effective_start_date) BETWEEN TRUNC(gvd_migration_date_from) AND TRUNC(gvd_migration_date_to)
																		   )
																			OR  
																		  (
																			...
																			OR
																		  (
																			TRUNC(paaf.effective_start_date)    < TRUNC(gvd_migration_date_from)
																			AND TRUNC(paaf.EFFECTIVE_END_DATE)  >  TRUNC(gvd_migration_date_to)
																		  ))
                                                              )
				UNION 
				SELECT  distinct per_all_assignments_f.change_reason
					...
					,per_all_people_f.EXPENSE_CHECK_SEND_TO_ADDRESS "EXPENSE_CHECK_ADDRESS"
					,(Select Name from per_jobs_tl@XXMX_EXTRACT where job_id  =  per_All_assignments_f.job_id) JOB_CODE
					,(Select location_code from hr_locations_all@XXMX_EXTRACT where location_id  =  per_All_assignments_f.location_id)
					location_code
					,per_periods_of_placement.Date_start
					,(Select name from HR_ALL_POSITIONS_F@XXMX_EXTRACT  HAPF
					  WHERE position_id = per_All_assignments_f.position_id
				...
				--	,PER_ALL_ASSIGNMENTS_F.RETIREMENT_DATE
                 ...
                 ,per_all_assignments_f.default_code_comb_id
				 ,per_all_people_f.npw_number personnumber
                FROM    per_person_type_usages_f@XXMX_EXTRACT per_person_type_usages_f
                    ,per_all_assignments_f@XXMX_EXTRACT per_all_assignments_f
                    ,per_assignment_status_types@XXMX_EXTRACT per_assignment_status_types
                    ,per_all_people_f@XXMX_EXTRACT per_all_people_f
                    ,per_person_types@XXMX_EXTRACT per_person_types
                ...
                AND   per_all_assignments_f.effective_start_date IN(SELECT MAX ( paaf.effective_start_date)
                                                                    ...
                                                                    AND ((
																--			TRUNC(paaf.effective_start_date) BETWEEN TRUNC(gvd_migration_date_from) AND TRUNC(gvd_migration_date_to)
																			TRUNC(paaf.effective_start_date) BETWEEN TRUNC(gvd_migration_date_from) AND '31-DEC-4712'
																		   )
																			OR  
																		  (
																-- 			TRUNC(paaf.effective_end_date) BETWEEN  TRUNC(gvd_migration_date_from) AND TRUNC(gvd_migration_date_to)
																			TRUNC(paaf.effective_end_date) BETWEEN  TRUNC(gvd_migration_date_from) AND '31-DEC-4712'
																		   )
																			OR
																		  (
																--			TRUNC(paaf.effective_start_date)    < TRUNC(gvd_migration_date_from)
																--			AND TRUNC(paaf.EFFECTIVE_END_DATE)  >  TRUNC(gvd_migration_date_to)
																			TRUNC(paaf.effective_start_date)    < TRUNC(gvd_migration_date_from)
																			AND TRUNC(paaf.EFFECTIVE_END_DATE)  >  '31-DEC-4712'
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg
            ,(
                SELECT  per_all_assignments_f.assignment_sequence c2_assignment_sequence
					...
					,(Select Name from per_grades_tl@XXMX_EXTRACT where grade_id = per_All_assignments_f.grade_id)
					,per_all_assignments_f.GRADE_LADDER_PGM_ID
					,per_all_people_f.EXPENSE_CHECK_SEND_TO_ADDRESS "EXPENSE_CHECK_ADDRESS"
					,(Select Name from per_jobs_tl@XXMX_EXTRACT where job_id  =  per_All_assignments_f.job_id)
					,(Select location_code from hr_locations_all@XXMX_EXTRACT where location_id  =  per_All_assignments_f.location_id)
					,per_periods_of_placement.Date_start
					,(Select name from HR_ALL_POSITIONS_F@XXMX_EXTRACT  HAPF
					  WHERE position_id = per_All_assignments_f.position_id
					...
					,per_person_types.USER_PERSON_TYPE
				--	,PER_ALL_ASSIGNMENTS_F.RETIREMENT_DATE
					,(SELECT Name from  PER_SPINAL_POINT_STEPS_F@XXMX_EXTRACT	PSPSF, per_spinal_points@XXMX_EXTRACT PSP
						where Step_id  = PER_ALL_ASSIGNMENTS_F.SPECIAL_CEILING_STEP_ID
						and PSP.spinal_point_id =PSPSF.spinal_point_id
						and Trunc(sysdate) between PSPSF.effective_Start_date and PSPSF.effective_end_date)
					,PER_ALL_ASSIGNMENTS_F.TIME_NORMAL_FINISH
    ...
    BEGIN
        ...
        INTO    xxmx_per_assignments_m_stg
                ...
                ,people_group_id
                ,default_code_comb_id
                ,DEFAULT_EXPENSE_ACCOUNT     --Added on 20Oct2022
				,PERSONNUMBER
                , ass_attribute2
                , ass_attribute3
                , ass_attribute4)
		SELECT  /*+ driving_site(table alias name) */ distinct pt_i_MigrationSetID                               
                ...
                ,c8_effective_start_date effective_start_date
                ,c12_effective_end_date effective_end_date --to_date ('31-12-4712','DD-MM-YYYY') effective_end_date
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg		
			...
			SELECT  distinct  per_all_assignments_f.assignment_sequence c2_assignment_sequence
                    ...
                    ,per_all_assignments_f.person_type_id c24_person_type_id
                    ,per_all_assignments_f.effective_end_date c12_effective_end_date
                    ,per_all_assignments_f.pay_system_status c22_pay_system_status
                    ,per_all_assignments_f.per_system_status c21_per_system_status
                    ,per_all_assignments_f.organization_id c17_organization_id
                    ...
                    ,per_all_assignments_f.assignment_status_type_id s0_asg_status_type_id
                    ,per_all_assignments_f.final_process_date final_process_date
                    ,per_all_assignments_f.period_of_service_id s0_period_of_service_id
					,per_all_assignments_f.sal_review_period c2_sal_review_period
                    ,per_all_assignments_f.time_normal_start c3_time_normal_start
                    --NULL c10_internal_location
                    ,per_all_assignments_f.internal_location c10_internal_location
                   ...
                   --, NULL c18_projected_start_date
                    ,per_all_assignments_f.projected_start_date c18_projected_start_date
                    ...
                    ,per_all_assignments_f.notice_period_uom c74_notice_period_uom
                    ,per_all_assignments_f.employee_category c75_employee_category
                    ,per_all_assignments_f.system_person_type c91_system_person_type
					,per_All_assignments_f.change_reason
					,'CURRENT' action_code
                    ,per_all_assignments_f.first_name || '.' ||  per_all_assignments_f.last_name  c110_billing_title
                    ,(SELECT Name 
					  FROM per_grades_tl@XXMX_EXTRACT 
					  WHERE grade_id = per_All_assignments_f.grade_id) 
					  GRADE_CODE
					...
					,per_All_assignments_f.Date_start
					,(SELECT name 
					  FROM HR_ALL_POSITIONS_F@XXMX_EXTRACT  HAPF
					  WHERE position_id = per_All_assignments_f.position_id
					  AND  gvd_migration_date_from BETWEEN HAPF.effective_Start_date AND HAPF.effective_end_date) 
					 POSITION_CODE
					,PER_ALL_ASSIGNMENTS_F.PROJECTED_ASSIGNMENT_END
                 ...
                 ,NVL((select concatenated_segments 
                    ...
                    where default_code_comb_id = CODE_COMBINATION_ID),'NA') DEFAULT_EXPENSE_ACCOUNT     --Added on 20Oct2022
				 ,per_All_assignments_f.employee_number personnumber
                 ,assignment_type
                 ,(select NVL(decode(attribute11,'AI','F',attribute11),'A') from per_all_people_f@xxmx_extract papf
                   where papf.person_id =  per_all_assignments_f.person_id
                   and gvd_migration_date_from BETWEEN papf.effective_start_date and papf.effective_end_date) head_class_count
                 , (SELECT NAME 
					  ...
					  AND gvd_migration_date_from BETWEEN DATE_FROM AND NVL(DATE_TO, '31-DEC-4712')) company_of_recharge
                 ,LTRIM(per_all_assignments_f.ass_attribute8,'0')    payroll_providerid
                FROM  XXMX_HCM_CURRENT_ASG_MV per_all_assignments_f
				) c_per_all_assignment
				WHERE c_per_all_assignment.c25_s_person_id = xxmx_per_persons_stg.person_id
				  AND c_per_all_assignment.assignment_type = 'E'
                   AND   c_per_all_assignment.business_group_id = p_bg_id
                  ;  
			...
			/*	per_person_type_usages_f@XXMX_EXTRACT per_person_type_usages_f
                    ...
                    ,per_all_people_f@XXMX_EXTRACT per_all_people_f
                    ,per_person_types@XXMX_EXTRACT per_person_types
                    ,per_periods_of_service@XXMX_EXTRACT per_periods_of_service
                   -- ,hr_all_organization_units@XXMX_EXTRACT horg
    ...
    BEGIN
        ...
        lvd_migration_date_from := TO_DATE(lvv_migration_date_from,'YYYY-MM-DD');
         --
        lvv_migration_date_to := xxmx_utilities_pkg.get_single_parameter_value(
                        pt_i_ApplicationSuite           =>     gct_ApplicationSuite
                        ,pt_i_Application                =>     gct_Application
                        ,pt_i_BusinessEntity             =>     'HCMEMPLOYEE'
                        ,pt_i_SubEntity                  =>     'ALL'
        ...
        lvd_migration_date_to := TO_DATE(lvv_migration_date_to,'YYYY-MM-DD');
         --         
        lvv_prev_tax_year_date := xxmx_utilities_pkg.get_single_parameter_value(
                             pt_i_ApplicationSuite           =>     gct_ApplicationSuite
                             ,pt_i_Application                =>     gct_Application
   ...
   /* IF  NVL( lvd_migration_date_from, NULL ) is null or NVL( lvd_migration_date_to, NULL ) is null or NVL( lvd_prev_tax_year_date, NULL ) is null
        ...
        END IF;*/
        --
        IF TRUNC(lvd_prev_tax_year_date) < TRUNC(lvd_migration_date_from)
        THEN
            lvd_migration_date_from := lvd_migration_date_from;
        END IF; 
       -- INSERT INTO XXMX_TEST VALUES('export_all_assignments_m_3'||lvd_migration_date_from||' '||lvd_migration_date_to); 
	  --
	    gvv_ProgressIndicator := '0010';
        xxmx_utilities_pkg.log_module_message(  
                        pt_i_ApplicationSuite    => gct_ApplicationSuite
                        ,pt_i_Application         => gct_Application
                        ,pt_i_BusinessEntity      => gv_i_BusinessEntity                        ,pt_i_SubEntity           => cv_i_BusinessEntityLevel
                        ,pt_i_Phase               => ct_Phase
                        ,pt_i_Severity            => 'NOTIFICATION'
                        ,pt_i_PackageName         => gcv_PackageName
                        ,pt_i_ProcOrFuncName      => cv_ProcOrFuncName
	  ...
	  -- Leavers , EMP
		...
		SELECT   /*+ driving_site(table alias name) */ pt_i_MigrationSetID                               
				...
				,position_code
				,GRADE_LADDER_PGM_NAME
				,c77_hourly_salaried_code hourly_salaried_code
				,c2_sal_review_period sal_review_period
                ,c78_sal_review_period_frequenc sal_review_period_frequency
				,c10_internal_location
				,c10_internal_location
				,c10_internal_location
				,c10_internal_location
				...
				,c73_frequency frequency	
				,c59_notice_period 
				,c74_notice_period_uom notice_period_uom
				,USER_PERSON_TYPE
                ...
                ,s0_period_of_service_id|| '_PERIOD_OF_SERVICE' period_of_service_id
                ,'E' assignment_type
    ...
    BEGIN
      ...
      /*  IF  NVL( lvd_migration_date_from, NULL ) is null or  NVL( lvd_prev_tax_year_date, NULL ) is null
        THEN 
                 ...
                 --
        END IF;*/
		--INSERT INTO XXMX_TEST VALUES('export_all_assignments_m_4'||lvd_migration_date_from||' '||lvd_migration_date_to); 
        --
        set_parameters(p_bg_id);
        gvv_ProgressIndicator := '0020';
        xxmx_utilities_pkg.log_module_message(  
...
;
    ...
    set_parameters(p_bg_id);
		...
		SELECT  /*+ driving_site(table alias name) */ pt_i_MigrationSetID                               
                ...
                ,'E'||c14_assignment_number assignment_number
                ,c2_assignment_sequence assignment_sequence
                ,c22_pay_system_status||'_'||c21_per_system_status assignment_status_type_id
                ,c4_primary_flag
                ,'ADD_ASSIGN'
                ,c11_assignment_name assignment_name
                ,'ACTIVE_PROCESS' assignment_status_type
    ...
    BEGIN
      ...
      /*  IF  NVL( lvd_migration_date_from, NULL ) is null or NVL( lvd_migration_date_to, NULL ) is null or NVL( lvd_prev_tax_year_date, NULL ) is null
        ...
        SELECT  /*+ driving_site(table alias name) */ DISTINCT pt_i_MigrationSetID                               
				...
				,employee.assignment_id || decode (employee.assignment_type
											,'E'
											,'_EMP'
											,'_CWK')
				...
				,sup.assignment_id|| decode (sup.assignment_type
											...
											,'_EMP'
											,'_CWK')
								  || '_ASSIGNMENT' manager_assignment_id
				,sup.person_id manager_id
				,'LINE_MANAGER' manager_type				,NULL action_occurrence_id
				,to_date ('01-01-1901','dd-mm-yyyy') freeze_stat_date
				,to_date ('31-12-4712','dd-mm-yyyy') freeze_until_date
				,(SELECT distinct EMPLOYEE_NUMBER||'_SUP_EMP_NUMBER'
                    FROM XXMX_HCM_CURRENT_PERSON_MV papf --PER_ALL_PEOPLE_F@XXMX_EXTRACT papf
                    WHERE ( gvd_migration_date_from BETWEEN papf.effective_start_Date and papf.effective_end_date
                           OR papf.effective_start_Date > gvd_migration_date_from ) -- Added on 25Jun2023 for Future Dated Workers
                    and person_id = sup.person_id)
				,NULL
				--,NVL(employee.employee_NUMBER ,employee.npw_number)
				,DECODE(Employee.system_person_type,'EMP',Employee.employee_number,'CWK',Employee.npw_number,-1)
				,employee.assignment_number
				,(select distinct NVL(ppf.employee_number,ppf.npw_number) from XXMX_HCM_CURRENT_PERSON_MV ppf --per_all_people_f@XXMX_EXTRACT ppf
				where person_id = sup.person_id
				--and trunc(SYSDATE) between ppf.effective_start_date and ppf.effective_End_date
				--and rownum=1
                )
				,sup.assignment_type||sup.assignment_number
        FROM per_all_Assignments_f@XXMX_EXTRACT sup, 
        xxmx_hcm_current_asg_mv  Employee
        WHERE sup.person_id = employee.supervisor_id
        AND ( gvd_migration_date_from between sup.effective_start_Date and sup.effective_end_date
             OR sup.effective_start_Date > gvd_migration_date_from ) -- Added on 25Jun2023 for Future Dated Workers
      ...
      /*  (  
				...
				WHERE   asg.person_id=   paaf.person_id||'_PERSON'
                AND asg.action_code IN ('CURRENT','ADD_ASSIGN')
                and Trunc(sysdate) BETWEEN paaf.effective_start_Date and paaf.effective_end_Date
                /*AND paaf.effective_start_Date IN( SELECT max(effective_Start_date) 
                                                    FROM  per_all_Assignments_f@XXMX_EXTRACT paaf1
                                                 WHERE paaf1.assignment_id = paaf.assignment_id
                                                -- AND paaf1.supervisor_id= paaf.supervisor_id
                                                 AND paaf1.person_id = paaf.person_id
                                                 AND ((
                                                        TRUNC(paaf1.effective_start_date) BETWEEN TRUNC(lvd_migration_date_from) AND TRUNC(lvd_migration_date_to)
                                                       )
                                                        OR  
                                                      (
                                                        TRUNC(paaf1.effective_end_date) BETWEEN  TRUNC(lvd_migration_date_from) AND TRUNC(lvd_migration_date_to)
                                                       )
                                                        OR
                                                      (
                                                        TRUNC(paaf1.effective_start_date)    <TRUNC(lvd_migration_date_from)
                                                        AND TRUNC(paaf1.EFFECTIVE_END_DATE)  > TRUNC(lvd_migration_date_to)
                                                      ))
                                                      )
      ...
      --  AND trunc(sysdate) between sup.effective_start_Date and sup.effective_end_date
        /*AND sup.effective_start_Date IN( SELECT max(effective_Start_date) 
										FROM  per_all_Assignments_f@XXMX_EXTRACT paaf2
										WHERE paaf2.assignment_id = sup.assignment_id
										AND paaf2.person_id = sup.person_id
										 AND ((
                                                        TRUNC(paaf2.effective_start_date) BETWEEN TRUNC(lvd_migration_date_from) AND TRUNC(lvd_migration_date_to)
                                                       )
                                                        OR  
                                                      (
                                                        TRUNC(paaf2.effective_end_date) BETWEEN  TRUNC(lvd_migration_date_from) AND TRUNC(lvd_migration_date_to)
                                                       )
                                                )
                                        )*/
        ...
        FROM    xxmx_per_persons_stg xxmx_per_persons_stg
            ...
            ,per_all_assignments_f@XXMX_EXTRACT per_all_assignments_f1
            ,
                (
                SELECT  per_all_assignments_f.effective_start_date c1_effective_start_date
                    ,
                        ...
                        FROM    per_all_assignments_f@XXMX_EXTRACT max
                        START WITH (
                ...
                FROM    per_all_assignments_f@XXMX_EXTRACT per_all_assignments_f
                        ...
                        OR      (
                                        (
                                                NOT EXISTS
                                                    (
                                                    SELECT  1
                                                    FROM    per_all_assignments_f@XXMX_EXTRACT inn
    ...
    BEGIN
       ...
       /* INSERT  
        ...
        FROM    xxmx_per_assignments_m_stg xxmx_per_assignments_m_stg
			   ,per_periods_of_service@XXMX_EXTRACT ppos
        WHERE  bg_id =p_bg_id
		...
		*/
		...
		SELECT /*+ driving_site(table alias name) */ distinct pt_i_MigrationSetID                               
				...
				,DECODE( xxmx_per_assignments_m_stg.assignment_type ,
						...
						,'HIRE')action_code
				,xxmx_per_assignments_m_stg.primary_flag primary_flag
				,xxmx_per_assignments_m_stg.primary_flag 
        ...
        SELECT  distinct pt_i_MigrationSetID                               
                ...
                ,'NA'
        FROM    xxmx_per_assignments_m_stg xxmx_per_assignments_m_stg
			   ,per_periods_of_placement@XXMX_EXTRACT ppos
        ...
        --Update ContractType in assignments table. Review with HCM requirements.
/*         UPDATE xxmx_per_assignments_m_stg asg
        Set CONTRACTTYPE = ( Select CONTRACT_TYPE from PQP_ASSIGNMENT_ATTRIBUTES_F@XXMX_EXTRACT casg
    ...
    BEGIN
        ...
        SELECT   pt_i_MigrationSetID                               
				...
				/*,NVL((SELECT a.effective_start_date 
				  ...
				  AND a.person_id = paaf.person_id||'_PERSON'
				...
				,(Select XXMX_KIT_PERSON_STG.EXPORT_DFF_SEGMENT_VAL 
                    ...
                    FROM FND_ID_FLEXS@XXMX_EXTRACT FIF    ,
                         FND_ID_FLEX_SEGMENTS@XXMX_EXTRACT FIFSE
		...
		FROM 
            ...
            xxmx_hcm_current_asg_mv paaf,
			PAY_PERSONAL_PAYMENT_METHODS_F@XXMX_EXTRACT PPPM,
			PAY_ORG_PAYMENT_METHODS_F@XXMX_EXTRACT POPM,
			pay_external_accounts@XXMX_EXTRACT PEA,
			PAY_PAYMENT_TYPES@XXMX_EXTRACT PPT,
			(SELECT distinct substr(a.assignment_id,1,instr(a.assignment_id,'_')-1) assignment_id
					  FROM xxmx_per_assignments_m_stg a												 
					) asg
		...
		and asg.assignment_id =  paaf.assignment_id		
        AND exists ( Select 1
							 From xxmx_per_persons_stg xxper
							 where xxper.person_id = paaf.person_id);		
        /*AND EXISTS ( SELECT 1
    ...
    BEGIN
		...
		FROM XXMX_PER_PAY_METHOD_STG ppm
            ...
            , XXMX_HCM_CURRENT_PERSON_MV ppf
		WHERE bg_id = p_bg_id
		AND ppf.person_id = ppm.person_id
		AND ( gvd_migration_date_from between ppf.effective_start_date and ppf.effective_end_date
             OR ppf.effective_start_date > gvd_migration_date_from ) -- Added on 25Jun2023 for Future Dated Workers
    ...
    BEGIN
        ...
        SELECT * FROM (
						WITH payrolls AS (
							...
							FROM
								pay_payrolls_f@xxmx_extract ppf
							WHERE
								upper(ppf.payroll_name) IN (
									SELECT
										upper(parameter_value)
									FROM
										xxmx_migration_parameters
									WHERE
										parameter_code LIKE 'PAYROLL_NAME'
										AND enabled_flag = 'Y'
									)
								AND gvd_migration_date_from BETWEEN ppf.effective_start_date AND ppf.effective_end_date
								)
			SELECT DISTINCT
				...
				(
					...
					FROM
						payrolls
					WHERE
							business_group_id = paaf.business_group_id
						AND ROWNUM = 1
				) payroll_name,
				paaf.effective_start_date,
				gcd_maxoracledate,
				paaf.assignment_id,
				gvv_leg_code,
				--nvl(paaf.employee_number,
				stg.personnumber
			FROM
				xxmx_hcm_current_asg_mv paaf,
				xxmx_per_persons_stg stg
				WHERE paaf.person_id = stg.person_id				
				);
		...
		/*and paaf.effective_start_date IN( Select MAX(effective_start_date) 
                                             ...
                                             and paaf1.assignment_id = paaf.assignment_id
                                              AND ((
													TRUNC(paaf1.effective_start_date) BETWEEN TRUNC(gvd_migration_date_from) AND TRUNC(gvd_migration_date_to)
												   )
													OR  
												  (
													TRUNC(paaf1.effective_end_date) BETWEEN  TRUNC(gvd_migration_date_from) AND TRUNC(gvd_migration_date_to)
												   )
													OR
												  (
													TRUNC(paaf1.effective_start_date)    < TRUNC(gvd_migration_date_from)
													AND TRUNC(paaf1.effective_end_date)  >  TRUNC(gvd_migration_date_to)
												  ))
										)*/
    ...
    BEGIN
		...
		BEGIN 
			SELECT COUNT(*)
			INTO lv_sal_count
			FROM Per_Pay_Bases@XXMX_EXTRACT;
		END;
		...
		IF(lv_sal_count <> 0) THEN 
				...
				select * from (
					...
					select /*+ driving_site(table alias name) */ distinct  pt_i_MigrationSetID                               
									...
									,Next_sal_review_date
									,paaf1.employee_number
                                    ,paaf1.person_id       -- Added on 14Oct2022
							from 	PER_ALL_ASSIGNMENTS_F@XXMX_EXTRACT PAAF
							        , xxmx_hcm_current_asg_mv paaf1
										,per_pay_proposals@XXMX_EXTRACT ppp
										,Per_Pay_Bases@XXMX_EXTRACT PB 
									--	, asg 10-MAY-22
									WHERE  ppp.change_date between paaf.effective_start_date and paaf.effective_end_date
                                   ...
                                   and ppp.change_date >= ADD_MONTHS(gvd_migration_date_from,-36)                                   
									And PB.Pay_Basis_Id = paaf.Pay_Basis_Id
									--and asg.assignment_id = paaf.assignment_id
                                   AND paaf.business_group_id = p_bg_id  
                                    and ppp.business_group_id = paaf.business_group_id 
                                    AND PB.business_group_id = paaf.business_group_id 
					);
      COMMIT;
           /* Query to Fetch Employees who has no Chnage in Salary for past 3years (30 Records) */
                INSERT  
				INTO    XXMX_PER_ASG_SALARY_STG
						(migration_set_id
						,migration_set_name                   
					...
					select /*+ driving_site(table alias name) */ distinct  pt_i_MigrationSetID                               
							...
							from 	PER_ALL_ASSIGNMENTS_F@XXMX_EXTRACT PAAF
                                   ...
                                   AND ppp.change_date = (SELECT MAX(CHANGE_DATE) 
								                            FROM per_pay_proposals@XXMX_EXTRACT 
														   WHERE ASSIGNMENT_ID = PAAF.ASSIGNMENT_ID )
                                   AND NOT EXISTS (SELECT 1 FROM XXMX_PER_ASG_SALARY_STG S WHERE S.assignment_ID = PAAF.assignment_ID)
									And PB.Pay_Basis_Id = paaf.Pay_Basis_Id
									--and asg.assignment_id = paaf.assignment_id
                                    AND paaf.business_group_id = p_bg_id 
                                    and ppp.business_group_id = paaf.business_group_id 
    ...
    COMMIT;
				...
				SELECT distinct  pt_i_MigrationSetID                               
						...
						,ppp.proposed_salary_N
						,Next_sal_review_date
						,(select employee_number from per_all_people_f@XXMX_EXTRACT ppf
		...
		ELSE
				...
				SELECT  /*+ driving_site(table alias name) */ distinct pt_i_MigrationSetID                               
						...
						,NVL((SELECT a.effective_start_date 
						  ...
						  AND a.Action_code IN('ADD_ASSIGN', 'CURRENT')),gvd_migration_date_from)
						...
						,pgrf.value
						,paaf.employee_number 
                        ,paaf.person_id       -- Added on 14Oct2022
			from 
					--per_all_people_f@XXMX_EXTRACT                papf,
					--per_all_assignments_f@XXMX_EXTRACT    		   paaf,
                    xxmx_hcm_current_asg_mv paaf,
					per_spinal_point_placements_f@XXMX_EXTRACT    psppf,
					per_grade_spines_f@XXMX_EXTRACT pgsf,
					per_spinal_points@XXMX_EXTRACT  PSP,
					per_spinal_point_steps_f@XXMX_EXTRACT  PSPSF,
					pay_grade_rules_f@XXMX_EXTRACT pgrf,
					pay_rates@XXMX_EXTRACT pr,
			...
			--AND 
            pabvf.assignment_id  = paaf.assignment_id
			AND pabvf.unit= 'FTE'
			...
			AND gvd_migration_date_from  BETWEEN psppf.effective_start_date AND psppf.effective_end_date
			...
			AND gvd_migration_date_from   BETWEEN pspsf.effective_start_date AND pspsf.effective_end_date
			AND gvd_migration_date_from  BETWEEN pspsf.effective_start_date AND pspsf.effective_end_date
			AND gvd_migration_date_from BETWEEN pgrf.effective_start_date AND pgrf.effective_end_date
            AND paaf.effective_start_date BETWEEN paaf.effective_start_date AND paaf.effective_end_date
           /*  and paaf.effective_start_date IN( Select MAX(effective_start_date) 
                                             from per_All_Assignments_f@XXMX_EXTRACT paaf1
                                             where paaf1.person_id = paaf.person_id 
	               ...
	               /* and pgrf.effective_start_date IN( Select MAX(effective_start_date) 
                                                        from pay_grade_rules_f@XXMX_EXTRACT pgrf1
		...
		/*INSERT  
			...
			from 
					...
					pay_grade_rules_f@XXMX_EXTRACT pgrf,
					pay_rates@XXMX_EXTRACT pr,
					PER_ASSIGNMENT_BUDGET_VALUES_F@XXMX_EXTRACT pabvf
    ...
    BEGIN
        ...
        SELECT distinct  pt_i_MigrationSetID                               
                ...
                ,(SELECT distinct psp1.spinal_point
                    FROM per_spinal_points@XXMX_EXTRACT                psp1,
                         per_spinal_point_steps_f@XXMX_EXTRACT         pspsf1,
                         pay_grade_rules_f@XXMX_EXTRACT                pgrf1
                    ...
                    FROM
                          per_spinal_points@XXMX_EXTRACT           psp2,
                          per_spinal_point_steps_f@XXMX_EXTRACT    pspsf2
                    WHERE    pspsf2.grade_spine_id    = pspsf.grade_spine_id
                    AND   pspsf2.spinal_point_id  = psp2.spinal_point_id
                    AND   gvd_migration_date_from  BETWEEN pspsf2.effective_start_date AND pspsf2.effective_end_date
    ...
    BEGIN
		...
		with asg as (
				...
				select  distinct  pt_i_MigrationSetID                               
								...
								,pabvf.value 
								,paaf.employee_number
						from per_assignment_budget_values_f@XXMX_EXTRACT pabvf, 
							 xxmx_hcm_current_asg_mv paaf,
							 per_assignment_Status_types@XXMX_EXTRACT pat 
							 , asg
		...
		/*and pabvf.effective_start_date IN( Select MAX(effective_start_date) 
                                             from per_assignment_budget_values_f@XXMX_EXTRACT pabvf1
                                             where 
	...
	*****************************************************************/
    ...
    BEGIN
        ...
        SELECT  /*+ driving_site(table alias name) */ distinct pt_i_MigrationSetID                               
                ...
                ,p_bg_id
				--,employee_number
				,DECODE(xxper.person_type,'EMP',paaf.employee_number, 'CWK',paaf.npw_number,-1) employee_number
				,gvv_leg_code
				,'SMBC_ENTERPRISE'
				,paaf.Date_start
				,paaf.Date_start
				,'31-DEC-4712'
				,NULL
				,trunc(gvd_migration_date_from - paaf.date_Start)
				,'Date Correction'
				--,NULL
				,paaf.Date_start
				,'E' 
				,paaf.person_id
                ,ASG.PAYROLLNAME   /*Modified on 7JUN23 */
			    /*,(SELECT PAYROLLNAME
				FROM xxmx_per_assignments_m_stg asg
		...
		--AND 
        paaf.business_Group_id= p_bg_id
		...
		AND xxper.person_id = paaf.person_id
        AND asg.PERSON_ID(+) = paaf.PERSON_ID
