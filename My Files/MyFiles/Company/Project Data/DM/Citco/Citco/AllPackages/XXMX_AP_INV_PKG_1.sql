--------------------------------------------------------
--  DDL for Package Body XXMX_AP_INV_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_AP_INV_PKG" 
AS
     --
     --**********************
     --** Global Declarations
           ...
           IS
                  ...
                  SELECT DISTINCT
                          ...
                          , round(aila.amount,2) +NVL((SELECT round(sum(unrounded_tax_amt),2)
                                                     FROM apps.zx_lines@XXMX_EXTRACT
                          ...
                          , (
                             ...
                             FROM   (
                                     ...
                                     FROM  apps.ap_invoice_distributions_all@XXMX_EXTRACT   aid
                                     WHERE aid.invoice_id                  = aila.invoice_id
                                     AND   aid.line_type_lookup_code       = 'ITEM'
                                    ) x
                                   ,apps.gl_code_combinations_kfv@XXMX_EXTRACT       gcck
                             WHERE  gcck.code_combination_id = x.dist_code_combination_id
                            )                                                                                     AS  dist_code_concatenated --Distribution code combination value - AP invoice 05/04/2022 
                          /*,(SELECT PARAMETER_VALUE  
                            FROM XXMX_MIGRATION_PARAMETERS
                            WHERE
                            application = 'AP'  
                            AND business_entity = 'INVOICES'
                            AND PARAMETER_CODE = 'DEFAULT_DIST_CCID_'||haou.name
                            AND SUB_ENTITY = 'INVOICE_LINE')                                                     AS  dist_code_concatenated*/
                          , NULL                                                                                  AS  distribution_set_name
--                          , aila.accounting_date                                                                    AS  accounting_date -- EDQ MApping  2019/12/31
                          , xxmx_utilities_pkg.get_single_parameter_value('FIN','GL','BALANCES','ALL','GL_DATE')  AS  accounting_date  -- Updates 24/03/2022 requested by Elvis
                          , NULL                                                                                  AS  account_segment
                          , NULL                                                                                  AS  balancing_segment
                          , NULL                                                                                  AS  cost_center_segment
--                          , NULL                                                                                  AS  tax_classification_code    -- aila.tax_classification_code
,(select nvl(aila.tax_classification_code,il.tax_classification_code)
  from  ap_invoice_lines_all@xxmx_extract il
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
  and   rownum = 1) AS tax_classification_code
                          , (SELECT TRIM(hl.location_code)
                             FROM   apps.hr_locations_all@XXMX_EXTRACT hl
...
,(select tax_regime_code
  ...
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
  and   rownum = 1) AS tax_regime_code
,(select tax
  from  ap_invoice_lines_all@xxmx_extract il
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
...
,(select tax_status_code
  ...
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
  and   rownum = 1) AS  tax_status_code
,(select tax_jurisdiction_code
  from  ap_invoice_lines_all@xxmx_extract il
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
  and   rownum = 1) AS tax_jurisdiction_code
,(select tax_rate_code
  from  ap_invoice_lines_all@xxmx_extract il
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
  and   rownum = 1) AS tax_rate_code
,(select tax_rate
  from  ap_invoice_lines_all@xxmx_extract il
  where il.invoice_id = aila.invoice_id
  and   line_type_lookup_code = 'TAX'
...
*/
                          ...
                          , ppa.segment1                                                                          AS pjc_project_number
                          , pt.task_number                                                                        AS pjc_task_number                          
                          ...
                          , ppa.name                                                                              AS pjc_project_name
                          , pt.task_name                                                                          AS pjc_task_name                          
                  FROM    xxmx_ap_inv_scope_v                                    selection
--                          ,apps.ap_invoices_all@XXMX_EXTRACT              aia
                          , apps.ap_payment_schedules_all@XXMX_EXTRACT     apsa
--                          , apps.ap_supplier_sites_all@XXMX_EXTRACT        assa
--                          , apps.ap_suppliers@XXMX_EXTRACT                 aps
--                          , apps.hz_parties@XXMX_EXTRACT                   hp
--                          , apps.ap_terms_tl@XXMX_EXTRACT                  apt
                          , apps.ap_invoice_lines_all@XXMX_EXTRACT         aila
--                          , apps.po_headers_all@XXMX_EXTRACT               ph
                          , apps.hr_all_organization_units@XXMX_EXTRACT    haou
     ...
     */
     ...
     IS
          ...
          */
          ...
          CURSOR StgTable_cur
                      (
                       pt_MigrationSetID               xxmx_ap_invoice_lines_stg.migration_set_id%TYPE
                      )
          IS
               --
               SELECT  migration_set_id
                              ...
                              */
                              ...
                              LOOP
                                   ...
                                   */
                                   --
/*
 * Replaced with parameters in XXMX_MIGRATION_PARAMETERS table
 *
                                   IF   gvb_DataEnrichmentRequired
                                   THEN
                                        --
