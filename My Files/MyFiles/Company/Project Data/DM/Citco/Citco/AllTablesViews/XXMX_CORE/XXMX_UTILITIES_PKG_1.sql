--------------------------------------------------------
--  DDL for Package Body XXMX_UTILITIES_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_UTILITIES_PKG" 
/*
  ** [previous_filename] HISTORY
     ...
     **  1.7   12-05-2022   Pallavi Kanajar     Removed Dhanu's code on load and Archive procedures
     **  08/12/2022 Michal Arrowsmith  Added generate_table_data_func for OIC to 
     **                                be able to check if this process completed or not.
     **  30/05/2023 Michal Arrowsmith  call_fusion_load_gen changed from proc to func and added code to check that
     **                                the db process completed or still running and return error back to OIC process.
     ******************************************************************************
     */
--
-- Amended xxmx_simple_transforms queries to take account of application value of 'ALL'
--
IS
     --
     --
    ...
    return varchar2 is
    begin
      return replace(pv_i_StringToConvert,'"','""');
    end handle_double_quotes;
--
     PROCEDURE log_module_message
                    (
     ...
     */
     ...
     BEGIN
          ...
          THEN
               --
               DELETE
               FROM   xxmx_core.xxmx_module_messages
               WHERE  1 = 1
          ...
          THEN
               --
               DELETE
               FROM   xxmx_core.xxmx_data_messages
               WHERE  1 = 1
               AND    application_suite = pt_i_ApplicationSuite
               AND    application       = pt_i_Application
               AND    phase             = pt_i_Phase
     ...
     */
     ...
     BEGIN
          ...
          AND  ((xst.application_suite  = pt_i_ApplicationSuite AND xst.application = pt_i_Application) OR
                (xst.application_suite  = 'FIN' AND xst.application = 'ALL'))
...
BEGIN
    log_module_message
         ...
         ,pt_i_Severity          => 'NOTIFICATION'
         ,pt_i_PackageName       => gct_PackageName
         ,pt_i_ProcOrFuncName    => 'p_extract_data'
         ,pt_i_ProgressIndicator => gvv_ProgressIndicator
         ,pt_i_ModuleMessage     => 'PROCEURE: p_extract_data start'
         ,pt_i_OracleError       => NULL
         );
...
-- ----------------------------------------------------------------------------
	...
	RETURN NUMBER IS 
        l_count NUMBER := 0; 
    BEGIN      
        IF pv_i_application_suite IN('SCM', 'FIN','PPM')
          THEN
        SELECT COUNT(*)
           INTO l_count 
...
dbms_output.put_line(l_sql);
    ...
    IS
        ...
        IS
            ...
            and a.table_name not like 'XXMX_PPM%'
            UNION
            SELECT CASE  WHEN DATA_TYPE = 'CLOB' THEN 
                    'dbms_lob.SUBSTR('||COLUMN_NAME||',10000)'
                    ELSE
...
-- ----------------------------------------------------------------------------
    ...
    BEGIN
        IF UPPER(pv_i_table_name) LIKE '%STG' THEN
            v_sql := 'TRUNCATE TABLE XXMX_STG.' || pv_i_table_name; 
            xxmx_utilities_pkg.log_module_message
               (
                    ...
                    ,pt_i_ProcOrFuncName      => 'truncate_table'
                    ,pt_i_Severity            => 'NOTIFICATION'
                    ,pt_i_ProgressIndicator   => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage       => 'SQL= "'||v_sql
                    ,pt_i_OracleError         => NULL
               );
            EXECUTE IMMEDIATE v_sql; 
        ELSIF upper(pv_i_table_name) LIKE '%XFM' AND upper(pv_i_table_name) NOT LIKE 'XXMX_PPM%' THEN
            v_sql :=  'TRUNCATE TABLE XXMX_XFM.' || pv_i_table_name;
            xxmx_utilities_pkg.log_module_message
               (
                     pt_i_ApplicationSuite    => gct_ApplicationSuite
                    ,pt_i_Application         => gct_Application
...
-- ----------------------------------------------------------------------------
   /* PROCEDURE generate_table_data (
        ...
        v_file_col_delimiter varchar2(5);
        v_file_col_enclosed_by varchar2(5);
        l_sql               VARCHAR2(2000);
        ...
        v_phase   varchar2(30) := 'EXTRACT';
        v_PackageName varchar2(30) := 'XXMX_UTILITIES_PKG';
        v_ProcOrFuncName varchar2(30) := 'generate_table_data';
    v_count  number;
BEGIN
    v_message := 'Procedure '||v_PackageName||'.'||v_ProcOrFuncName||' started for table '||pv_i_table_name;
    xxmx_utilities_pkg.log_module_message
             (
              pt_i_ApplicationSuite    => gct_ApplicationSuite
             ,pt_i_Application         => gct_Application
             ...
             ,pt_i_MigrationSetID      => 0
             ,pt_i_Phase               => v_phase
             ,pt_i_PackageName         => v_PackageName
    ...
    DELETE FROM XXMX_DM_STG_XFM_DATA
        WHERE table_name = pv_i_table_name;
    --
    -- Get the delimiter and enclosed by string
    if pv_i_table_name like '%STG' then
        begin
            select CONTROL_FILE_DELIMITER,CONTROL_FILE_OPTIONAL_QUOTES 
                into v_file_col_delimiter,v_file_col_enclosed_by 
                from xxmx_stg_tables 
                where table_name = pv_i_table_name;
            v_message:='Procedure "'||v_PackageName||'.'||'generate_table_data'||'" delimiter='||v_file_col_delimiter||' enclosed='||v_file_col_enclosed_by||' for table '||pv_i_table_name;
        exception
            when no_data_found then
                v_message:='Procedure "'||v_PackageName||'.'||'generate_table_data'||'" No data found in xxmx_stg_tables for table '||pv_i_table_name;
            when others then
                v_message:=substr(SQLERRM,1,500);
        end;
    else
        begin
            select FIELD_DELIMITER,'"' into v_file_col_delimiter,v_file_col_enclosed_by from xxmx_xfm_tables where table_name = pv_i_table_name;
        exception
            when no_data_found then
                v_message:='Procedure "'||v_PackageName||'.'||'generate_table_data'||'" No data found in xxmx_xfm_tables for table '||pv_i_table_name;
            when others then
                v_message:=substr(SQLERRM,1,500);
        end;
    end if;
    --
    v_message := 'delimiter is '||v_file_col_delimiter||' and enclosed by '||v_file_col_enclosed_by;
    xxmx_utilities_pkg.log_module_message
             (
              pt_i_ApplicationSuite    => gct_ApplicationSuite
             ,pt_i_Application         => gct_Application
  ...
  -- This part outputs the HEADER 
    ...
    FOR j IN 1..col_cnt LOOP 
			...
			v_finaltxt := ltrim(v_finaltxt|| v_file_col_delimiter || upper(rec_tab(j).col_name), v_file_col_delimiter );
    ...
    xxmx_utilities_pkg.log_module_message
             ...
             ,pt_i_Severity            => 'NOTIFICATION'               
             ,pt_i_ProgressIndicator   => gvv_ProgressIndicator
             ,pt_i_ModuleMessage       => substr('XXXX='||v_finaltxt,1,4000)
             ,pt_i_OracleError         => NULL
             );          
--
    ...
    xxmx_utilities_pkg.log_module_message
             ...
             ,pt_i_Severity            => 'NOTIFICATION'               
             ,pt_i_ProgressIndicator   => gvv_ProgressIndicator
             ,pt_i_ModuleMessage       => 'Insert into XXMX_DM_STG_XFM_HEADER - '||SQL%rowcount
             ,pt_i_OracleError         => NULL
             );          
  --
  -- This part outputs the DATA
           ...
           -- dbms_output.put_line(v_ret);
            ...
            FOR j IN 1..col_cnt LOOP CASE rec_tab(j).col_type
                ...
                --    v_finaltxt :=  ltrim(v_finaltxt||'|"'||REPLACE(REPLACE(TRIM(v_v_val),'"','""'),CHR(10),' ') ||'"','|');  
                    --v_finaltxt := ltrim(v_finaltxt || '|"' || replace(replace(replace(replace(replace(v_v_val, chr(09), '| '), ' |', '|'), '|', ''),'"', '""'), chr(10), ' ')|| '"', '|');
                    v_finaltxt := ltrim(v_finaltxt || v_file_col_delimiter || v_file_col_enclosed_by || v_v_val || v_file_col_enclosed_by, v_file_col_delimiter );

                WHEN 2 THEN
                    dbms_sql.column_value(c, j, v_n_val);  --Number
                    --v_finaltxt := ltrim(v_finaltxt|| '|"'|| v_n_val|| '"', '|');
                    v_finaltxt := ltrim(v_finaltxt|| v_file_col_delimiter || v_file_col_enclosed_by || v_n_val || v_file_col_enclosed_by, v_file_col_delimiter);

                WHEN 12 THEN
                    dbms_sql.column_value(c, j, v_d_val); --date
                    --v_finaltxt := ltrim(v_finaltxt|| '|"'|| to_char(v_d_val, 'DD/MM/YYYY')|| '"', '|');
                    v_finaltxt := ltrim(v_finaltxt|| v_file_col_delimiter || v_file_col_enclosed_by || to_char(v_d_val, 'DD/MM/YYYY')|| v_file_col_enclosed_by, v_file_col_delimiter);

                WHEN 180 THEN
                    dbms_sql.column_value(c, j, v_v_val); --Timestamp
                    --v_finaltxt := ltrim(v_finaltxt|| '|"'|| trim(v_v_val)|| '"', '|');
                    v_finaltxt := ltrim(v_finaltxt|| v_file_col_delimiter || v_file_col_enclosed_by || trim(v_v_val)|| v_file_col_enclosed_by, v_file_col_delimiter);
    ...
    UPDATE XXMX_DM_STG_XFM_DATA
        ...
        AND    REGEXP_LIKE ( table_data,
                              ...
                              '' );
    --
    select count(*) into v_count from XXMX_DM_STG_XFM_DATA where table_name = pv_i_table_name;
    v_message:='Procedure "'||v_PackageName||'.'||'generate_table_data '||v_count||' rows in XXMX_DM_STG_XFM_DATA updated for table '||pv_i_table_name;
    xxmx_utilities_pkg.log_module_message
             (
              pt_i_ApplicationSuite    => gct_ApplicationSuite
             ,pt_i_Application         => gct_Application
  ...
  PROCEDURE generate_table_data (
    ...
    BEGIN
        ...
        END LOOP;
            ...
            OPEN CUR FOR   'select '||v_column_lst ||' , '||NVL(v_seq_in_fbdi,'NULL')||' from '
            ...
            FORALL i IN 1..g_extract_data.count
               ...
               INSERT INTO XXMX_DM_STG_XFM_DATA values(pv_i_table_name,g_extract_data(i));
               commit;        
...
BEGIN
    ...
    END LOOP;
    --
    INSERT INTO XXMX_DM_STG_XFM_HEADER VALUES (pv_i_table_name,v_finaltxt);
    --
    OPEN CUR FOR   'select '||v_column_lst ||' , '||NVL(v_seq_in_fbdi,'NULL')||' from '
                                 ||v_schema||'.'|| pv_i_table_name;
        LOOP                             
...
-- ----------------------------------------------------------------------------
-- |-----------------------< dba_scheduler_is_running >------------------------|
-- ----------------------------------------------------------------------------
function dba_scheduler_is_running (p_job_name in varchar2) return boolean is
    v_value         number;
begin
    select 1
        into v_value
        from        v$session
        where       status      = 'ACTIVE' 
        and         schemaname  = 'XXMX_CORE'
        and         action      =   p_job_name;
    return true;
exception
    when others then
        return false;
end dba_scheduler_is_running;
-- ----------------------------------------------------------------------------
...
IS
   V_MIGRATION_SET_NAME VARCHAR2(100) := NULL;
   V_CLIENT_CODE        VARCHAR2(100) := 'CITCO';
BEGIN

/* Added Modification on 03Mar22 */    
--    IF pv_i_application_suite = 'HCM' 
--	THEN
--	 V_CLIENT_CODE := 'Test_'||to_char(sysdate,'DDMMYYHH24MI');
--
--	  SELECT UNIQUE MIGRATION_SET_NAME 
--	    INTO V_MIGRATION_SET_NAME
--	    FROM XXMX_PER_ASSIGNMENTS_M_XFM 
--	   WHERE MIGRATION_SET_ID = (SELECT MAX(MIGRATION_SET_ID) FROM XXMX_PER_ASSIGNMENTS_M_XFM); 
--
--	  xxmx_hcm_hdl_file_gen_pkg.gen_main(pt_i_ClientCode => V_CLIENT_CODE
--                                       , pt_i_MigrationSetName => V_MIGRATION_SET_NAME
--                                   );
--	ELSE
      /* End Modification on 03Mar22 */
    xxmx_utilities_pkg.log_module_message
	(
				   ...
				   ,pt_i_ModuleMessage     => 'Calling XXMX_FUSION_LOAD_GEN_PKG.main'
                   ,pt_i_OracleError=> NULL
	);
    --
    dbms_scheduler.create_job (
...
return varchar2 is
    ...
    v_procedure_name varchar(60) := 'call_fusion_load_gen2';
    --
    cursor c_job  (p_job_name in varchar2) is
        select *
...
begin
    --
    v_job_name := pv_i_application_suite||'_'||pt_i_BusinessEntity||'_'||pt_i_sub_entity;
    --
    xxmx_utilities_pkg.log_module_message
	(
					pt_i_ApplicationSuite  => pv_i_application_suite
				   ...
				   ,pt_i_Severity          => 'NOTIFICATION'
				   ,pt_i_PackageName       => 'xxmx_utilities_pkg'
				   ,pt_i_ProcOrFuncName    => v_procedure_name
				   ,pt_i_ProgressIndicator => '0000'
				   ,pt_i_ModuleMessage     => 'Start processing database using job dbms_scheduler. Job Name='||v_job_name
                   ,pt_i_OracleError=> NULL
	);
    -- Check if job already running
    if dba_scheduler_is_running(v_job_name) then
        return 'F'; 
    end if;
    --
    dbms_scheduler.create_job 
    (
	    ...
	    job_action  =>
	    'BEGIN
	       XXMX_FUSION_LOAD_GEN_PKG.main2 (
	       pv_i_application_suite => '''||pv_i_application_suite||'''
	       ,pt_i_BusinessEntity => '''||pt_i_BusinessEntity||'''
	       ,pt_i_sub_entity => '''||pt_i_sub_entity||'''
	       ) ;
	    ...
	    enabled     => TRUE,
	    auto_drop   => TRUE,
	    comments    => 'Run FBDI extract for '||v_job_name
    );
    -- Check that the job has completed before exiting the procedure
    while dba_scheduler_is_running(v_job_name) loop
	...
	(
				   ...
				   ,pt_i_ProcOrFuncName    => v_procedure_name
				   ,pt_i_ProgressIndicator => '0000'
				   ,pt_i_ModuleMessage     => 'Job Completed. Job Name='||v_job_name
                   ,pt_i_OracleError=> NULL
	);
    return 'S';
end call_fusion_load_gen2;
-- ----------------------------------------------------------------------------
-- |-------------------------< get_FBDI_filenames >--------------------------|
-- ----------------------------------------------------------------------------
...
END get_FBDI_filenames;
    ...
    FUNCTION get_primary_ledger_id (p_bu_name VARCHAR2, p_business_entity VARCHAR2)
    RETURN VARCHAR2
    AS

        l_primary_ledger_id VARCHAR2(100):= '-1';
    BEGIN
        IF(p_business_entity = 'AP_INVOICES') THEN
            SELECT LEDGER_ID 
            INTO l_primary_ledger_id
            FROM xxmx_dm_fusion_das das
            where exists(SELECT 1
                         FROM xxmx_ap_invoices_stg stg
                         WHERE das.bu_name= stg.operating_unit
                         )
...
is
    vv_double_quotes_processing                 varchar2(30);
    vv_column_name1                             varchar2(1000);
    vv_column_name2                             varchar2(1000);
    vv_column_name3                             varchar2(1000);
    vv_column_name4                             varchar2(1000);
    vv_column_name5                             varchar2(1000);
    vv_SQLStatement1                            varchar2(32000);
    vv_SQLStatement2                            varchar2(32000);
    vv_SQLStatement3                            varchar2(32000);
    vv_SQLStatement4                            varchar2(32000);
    vv_SQLStatement5                            varchar2(32000);
    vv_stop_processing                          varchar2(5);
    vv_sqlresult_num                            number;
    vv_procedure_name                           varchar2(30) := 'clean_xfm_data';
    ...
    cursor c_table_columns ( p_xfm_table_id in number) is
            ...
            and     nvl(xtc.include_in_outbound_file,'Y') = 'Y'
            ;
begin
    gvv_ProgressIndicator := '0010';
    xxmx_utilities_pkg.log_module_message
                  ...
                  ,pt_i_Severity          => 'NOTIFICATION'
                  ,pt_i_PackageName       => gct_PackageName
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'clean_xfm_data: Started'
                  ,pt_i_OracleError       => NULL
                  );
    -- XFM tables:
    -- The following characters to be handled in a special way
    -- Double quote handling:    Parameter values are REMOVE or KEEP
    -- TAB/Ascii 9 handling:     Remove this char from all text column as when FBDI is created a tab is handled as a separator
    -- Trailing spaces removed
    gvv_ProgressIndicator := '0020';
    vv_double_quotes_processing := XXMX_UTILITIES_PKG.get_single_parameter_value
            (
    ...
    if vv_double_quotes_processing is null then
        vv_double_quotes_processing := 'REMOVE';
    end if;
    xxmx_utilities_pkg.log_module_message
                  (
                   pt_i_ApplicationSuite  => gct_ApplicationSuite
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'vv_double_quotes_processing='||vv_double_quotes_processing
                  ,pt_i_OracleError       => NULL
                  );
    --
    gvv_ProgressIndicator := '0030';
    vv_stop_processing   := NULL;
    select schema_name into v_schema_name from xxmx_xfm_tables where xfm_table_id = pt_i_xfm_table_id;
    vv_SQLStatement1     := 'update '||v_schema_name||'.'||pt_i_xfm_table_name||' set ';
    vv_SQLStatement2     := 'update '||v_schema_name||'.'||pt_i_xfm_table_name||' set ';
    vv_SQLStatement3     := 'update '||v_schema_name||'.'||pt_i_xfm_table_name||' set ';
    vv_SQLStatement4     := 'update '||v_schema_name||'.'||pt_i_xfm_table_name||' set ';
    vv_SQLStatement5     := 'update '||v_schema_name||'.'||pt_i_xfm_table_name||' set ';
    vv_sqlresult_num     := NULL;
    --
    for r_table_column in c_table_columns(pt_i_xfm_table_id)
    loop
        -- tab
        vv_column_name1 := r_table_column.column_name||'=replace('||r_table_column.column_name||',chr(9),'' '')';
        -- double quotes
        if vv_double_quotes_processing = 'KEEP' then
        ...
        -- trim
        vv_column_name3 := r_table_column.column_name||'=trim('||r_table_column.column_name||')';
        -- CR
        vv_column_name4 := r_table_column.column_name||'=replace('||r_table_column.column_name||',chr(13),'' '')';
        -- LF
        vv_column_name5 := r_table_column.column_name||'=replace('||r_table_column.column_name||',chr(10),'' '')';
        --
        if c_table_columns%rowcount>1 then
            vv_column_name1:=','||vv_column_name1;
            vv_column_name2:=','||vv_column_name2;
            vv_column_name3:=','||vv_column_name3;
            vv_column_name4:=','||vv_column_name4;
            vv_column_name5:=','||vv_column_name5;
        end if;
        --
        -- Replace the TAB with space first then if it is at the begining or end of the string they will be removed by the trim
        vv_sqlstatement1 := vv_sqlstatement1||vv_column_name1;
        vv_sqlstatement2 := vv_sqlstatement2||vv_column_name2;
        vv_sqlstatement3 := vv_sqlstatement3||vv_column_name3;
        vv_sqlstatement4 := vv_sqlstatement4||vv_column_name4;
        vv_sqlstatement5 := vv_sqlstatement5||vv_column_name5;
    end loop;
    xxmx_utilities_pkg.log_module_message
                  (
                   pt_i_ApplicationSuite  => gct_ApplicationSuite
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'SQL= '||SUBSTR(vv_SQLStatement1,1,3500)
                  ,pt_i_OracleError       => NULL
                  );
    execute immediate vv_SQLStatement1;
    --
    xxmx_utilities_pkg.log_module_message
                  (
                   pt_i_ApplicationSuite  => gct_ApplicationSuite
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'SQL= '||SUBSTR(vv_SQLStatement2,1,3500)
                  ,pt_i_OracleError       => NULL
                  );
    execute immediate vv_SQLStatement2;
    --
    xxmx_utilities_pkg.log_module_message
                  (
                   pt_i_ApplicationSuite  => gct_ApplicationSuite
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'SQL= '||SUBSTR(vv_SQLStatement3,1,3500)
                  ,pt_i_OracleError       => NULL
                  );
    execute immediate vv_SQLStatement3;
    --
    xxmx_utilities_pkg.log_module_message
                  (
                   pt_i_ApplicationSuite  => gct_ApplicationSuite
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'SQL= '||SUBSTR(vv_SQLStatement4,1,3500)
                  ,pt_i_OracleError       => NULL
                  );
    execute immediate vv_SQLStatement4;
    --
    xxmx_utilities_pkg.log_module_message
                  (
                   pt_i_ApplicationSuite  => gct_ApplicationSuite
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'SQL= '||SUBSTR(vv_SQLStatement5,1,3500)
                  ,pt_i_OracleError       => NULL
                  );
    execute immediate vv_SQLStatement5;
    commit;
    --
    gvv_ProgressIndicator := '0100';
    xxmx_utilities_pkg.log_module_message
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => 'clean_xfm_data: Completed'
                  ,pt_i_OracleError       => NULL
                  );
exception
    when others then
        log_module_message
                (
                  ...
                  ,pt_i_ProcOrFuncName    => vv_procedure_name
                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                  ,pt_i_ModuleMessage     => substr(sqlerrm,1,4000)
                  ,pt_i_OracleError       => NULL
                );
                    Raise;
