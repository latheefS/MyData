--------------------------------------------------------
--  DDL for Package Body XXMX_HCM_HDL_FILE_GEN_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_HCM_HDL_FILE_GEN_PKG" 
AS
   ...
   --**  1.3    10/05/2021    Pallavi Kanajar   Visa  Party Org , People Group
   ...
   --**  1.11   08/03/2023    Vamsi             Changed Assignment Name to Position Name for Business Title
   --**  1.12   09/11/2023    Vamsi             Added Termination Reason Code in Worker Termination
   --**--******************************************************************************
   --**
          ...
          -- WORKER NAME
              ...
              WHERE fs.person_id = per.person_id
               ...
               FROM XXMX_HR_HCM_PERSON_V per,
                    XXMX_HR_HCM_FILE_SET_V1 fs
              WHERE 1=2
                AND fs.person_id = per.person_id
                AND UPPER(fs.action_code) IN('HIRE','ADD_CWK')
               ...
               -- Worker Phone
                ...
                SELECT a.person_number||'-'||a.phone_phone_type||'-'||row_number() over (partition by a.person_number order by a.phone_phone_type)||gv_delim
            ||PHONE_REC
            from
              ( SELECT DISTINCT fs.person_number,per.phone_phone_type,
                  -- fs.person_number||'-'||per.phone_phone_type||'-'||row_number() over (partition by fs.person_number order by per.phone_phone_type)||gv_delim
                    'Worker-'||fs.person_number||gv_delim
					...
					||per.phone_area_code||gv_delim    --AREA_CODE added on 06JUL2022
                    ||per.phone_phone_number||gv_delim
                    ||per.phone_primary_flag phone_rec
               FROM XXMX_HR_HCM_PERSON_V per,
                    XXMX_HR_HCM_FILE_SET_V1 fs
              WHERE fs.person_id = per.person_id
                AND UPPER(fs.action_code) IN('HIRE','ADD_CWK')
                AND per.phone_phone_number IS NOT NULL) a
                order by 1;
              ...
              WHERE fs.person_id = per.person_id
               ...
               CURSOR c_get_per_national_id is
                    ...
                    ||PER.nid_national_identifier_type||gv_delim
              ...
              WHERE fs.person_id = per.person_id
                ...
                --and 1=2 --Added for SIT only - NIDs are masked. Remove this for UAT
                order by 1;
               ...
               SELECT  DISTINCT
                         fs.person_number||gv_delim
                    ||'Worker-'||fs.person_number||gv_delim
                    ||per.effective_start_date||gv_delim  --||per.cit_date_from||gv_delim
                    --||per.cit_date_to||gv_delim
					||to_CHAR(to_date('31-DEC-4712','DD-MON-RRRR'),'RRRR/MM/DD')||gv_delim
                    ||decode(per.cit_legislation_code,'CW','NL','GG','GB','HK','CN','KY','BOC',per.cit_legislation_code)||gv_delim
         ...
         -- Work Relationship
                CURSOR c_per_work_relation IS
                    SELECT  DISTINCT
                         ...
                         ||CASE WHEN UPPER(fs.action_code) = 'ADD_CWK' THEN 'Contingent worker' ELSE w.wrs_worker_type END||gv_delim
                         ||NVL( wrs_legal_employer_name,fs.legal_emp_name)||gv_delim  -- Fix for Legal_employer is null
                         ||fs.action_code||gv_delim
                         ||''||gv_delim
                         ||w.wrs_primary_flag||gv_delim
                         ||W.wrs_original_date_of_hire        -- Added on 22Dec2022
            ...
            -- Worker WorkTerms
                CURSOR c_per_workterms IS
                     SELECT DISTINCT
                         ...
                         ||CASE WHEN UPPER(fs.action_code) = 'ADD_CWK' THEN fs.per_start_date ELSE wa.asg_effective_start_date END||gv_delim   -- Added on 28Dec22
               ...
               -- ASSIGNMENT
                CURSOR c_per_assignment IS
                    SELECT  DISTINCT
                         ...
                         ||'Position '||SUBSTR(wa.asg_position_code,13)||gv_delim -- Assignment Name
                   ...
                   --      ||DECODE(NVL(wa.asg_assignment_status_type_id,'ACTIVE_ASSIGN'),'ACTIVE_ASSIGN',1,2)||gv_delim
                         ...
                         ||''||gv_delim    --||RTRIM(wa.asg_legal_employer_name)||gv_delim  -- TaxReportingUnit
                         ||wa.asg_employment_category||gv_delim  -- Assignment Category
						 ||wa.default_expense_account
                      FROM XXMX_HR_HCM_WORK_ASSIGN_V wa,
                           XXMX_HR_HCM_FILE_SET_V1 fs
                     WHERE fs.person_id          = wa.asg_person_id
                       AND fs.assignment_number  = wa.asg_assignment_number
     ...
     IS
           ...
           -- Worker WorkTerms
               /*CURSOR c_per_workterms IS
				...
				WHERE fs.person_id = wa.asg_person_id
					AND  UPPER(fs.action_code) = UPPER(wa.asg_action_code)
					AND fs.assignment_number  = wa.asg_assignment_number
					AND UPPER(fs.action_code) IN ('CURRENT','ADD_CWK')
                    --AND WA.ASG_SYSTEM_PERSON_TYPE = 'CWK'
					AND wa.asg_effective_start_date IS NOT NULL
				ORDER BY 1;
        ...
        CURSOR c_per_assignment IS
				SELECT  DISTINCT
					...
					--||wa.asg_employee_category||gv_delim  -- WORKER CATEGORY
					||wa.asg_workercategory||gv_delim  -- WORKER CATEGORY		- Kirti
					||DECODE(wa.asg_employment_category,'FTEP','FR','PTEP','PR',wa.asg_employment_category)||gv_delim  -- EMPLOYMENT CATEGORY  - Kirti   --DECODE added on 07Jun2022
					||decode(wa.asg_employment_category,'FTEP','FULL_TIME','PTEP','PART_TIME','')||gv_delim
					||wa.asg_DATE_PROBATION_END||gv_delim
					||wa.asg_NORMAL_HOURS||gv_delim
					||wa.asg_FREQUENCY||gv_delim
					||decode (wa.asg_system_person_type,'EMP','R','T')||gv_delim   -- PermanentTemporary
					||wa.asg_job_code||gv_delim
					||wa.asg_location_code||gv_delim
					--||wa.asg_organization_id||gv_delim         -- DepartmentName    --Commented on 07Jun2022 
					||wa.asg_notice_period||gv_delim
					||wa.asg_notice_period_uom||gv_delim
					--||wa.asg_primary_flag||gv_delim        --Commented on 07Jun2022  
					||wa.asg_hourly_salaried_code||gv_delim
					||wa.asg_position_code||gv_delim
					||wa.asg_grade_code||gv_delim
					||wa.asg_bargaining_unit_code||gv_delim
					...
					||wa.asg_manager_flag||gv_delim
					||DECODE(wa.asg_system_person_type,'EX_EMP','EMP',wa.asg_system_person_type)||gv_delim
					||''||gv_delim     --RTRIM(wa.asg_legal_employer_name)||gv_delim              --Commented on 07Jun2022  <<BLANK>>
					||wa.asg_people_group_name||gv_delim
					||wa.asg_grade_ladder_pgm_name||gv_delim
					||''||gv_delim     --wa.asg_establishment_id||gv_delim       --Commented on 07Jun2022  <<BLANK>>
					--||wa.default_expense_account||gv_delim -- Default Exepnse Account 1.26    --Commented on 07Jun2022 
				...
				SELECT  DISTINCT
					...
					--||wa.asg_employee_category||gv_delim  -- WORKER CATEGORY
					||wa.asg_workercategory||gv_delim  -- WORKER CATEGORY  - Kirti
					||DECODE(wa.asg_employment_category,'FTEP','FR','PTEP','PR',wa.asg_employment_category)||gv_delim  -- EMPLOYMENT CATEGORY   - Kirti  -- Added DECODE on 07Jun2022
					||decode (wa.asg_employment_category,'FTEP','FULL_TIME','PTEP','PART_TIME','')||gv_delim
					||wa.asg_DATE_PROBATION_END||gv_delim
					||wa.asg_NORMAL_HOURS||gv_delim
					||wa.asg_FREQUENCY||gv_delim
					...
					||wa.asg_people_group_name||gv_delim
					||wa.asg_grade_ladder_pgm_name||gv_delim
					||''--trim(wa.asg_establishment_id)
                    ||gv_delim   -- REPORTING ESTABLISHMENT ADDED BY KIRTI
					--||wa.default_expense_account||gv_delim -- Default Exepnse Account 1.26    --Commented on 07Jun2022 
					||wa.asg_position_override_flag||gv_delim-- postion_override_flag					-- Pradeep-Added DFF headers - Change to Attribute fields
					||wa.default_expense_account||gv_delim
                    ||'Global Data Elements'
					||gv_delim
					||wa.ASS_ATTRIBUTE2||gv_delim --head_count_class
					||wa.ASS_ATTRIBUTE3||gv_delim -- payroll_providerid
                    ||wa.ASS_ATTRIBUTE4           -- companyofrecharge
					/*--||wa.ASS_ATTRIBUTE_DATE1||gv_delim
    ...
    --   ActionCode|TerminateWorkRelationshipFlag|ActualTerminationDate|LastWorkingDate
               CURSOR c_per_work_relation IS
                    SELECT  DISTINCT
                         fs.person_number||gv_delim
                         ||'Worker-'||fs.person_number||gv_delim
                         ||fs.term_action||gv_delim
                         ||FS.REASON_CODE||gv_delim    --Added on 09Nov2023 -- 1.12
                         ||'Y'||gv_delim
     ...
     IS
      ...
      CURSOR c_extern_bank_acct IS
       ...
       WHERE b.person_id = fs.person_id
         ...
         order by 1;*/
    SELECT DISTINCT
           '-'||fs.PERSON_NUMBER || '_' || bank_account_number || '_' || branch_name
           ||gv_delim|| bank_name
           ||gv_delim|| branch_name
           ||gv_delim|| bank_account_number
           ||gv_delim|| country_code
     ...
     IS
      ...
      CURSOR c_salary IS
        ...
        WHERE fs.assignment_number  = s.assignment_number
		 ...
		 AND NAME NOT LIKE 'IN%'    --Added on 10Jun2022
          order by 1;
          --
          /************************
          ** Constant Declarations
     ...
     IS
      ...
      CURSOR c_abs_sickness IS
        SELECT DISTINCT
            ...
            gv_delim|| --Assignment Number
            NVL(abs.StartDateDuration,0)||gv_delim||  --StartDateDuration
            NVL(abs.EndDateDuration,0)  --EndDateDuration            
      ...
      /*CURSOR c_talent_profile IS
        ...
        --MERGE|TalentProfile|HRC_SQLLOADER|TalentProfile_14841_PERSON_PROFILE|M Meachin|PERS_14841|A|P|14841|PERSON
            ...
            SELECT distinct
                ...
                Description||gv_delim||
                PROFILECODE||gv_delim||
            ...
            XXMX_HR_HCM_FILE_SET_V1 fs
            where fs.person_number = PRF.person_number
            and fs.action_code = 'HIRE'
            order by 1;
  --
          --Profile Item
    /*    CURSOR c_profile_item IS
            ...
            Select
--profileItemID
                fs.person_number||'_'||CONTENTITEMID||gv_delim||
                PROFILE_ID||gv_delim||
                PROFILECODE||gv_delim||
                CONTENTITEMID||gv_delim||
                CONTENTTYPEID||gv_delim||
                --CONTENTTYPE
                --CountryId
                DATEFROM||gv_delim||
                DATETO ||gv_delim||
                RatingModelId1||gv_delim||
                RatingModelCode1||gv_delim||
                RatingLevelId1||gv_delim||
                RatingLevelCode1||gv_delim||
                SECTIONID
            From XXMX_HR_HCM_TAL_PRF_V PRF,
            XXMX_HR_HCM_FILE_SET_V1 fs
            where fs.person_number = PRF.person_number
