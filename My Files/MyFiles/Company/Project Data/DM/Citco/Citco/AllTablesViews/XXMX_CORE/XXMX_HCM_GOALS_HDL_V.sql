...
'|EBS|G_'||PERSONNUMBER||'_'||row_number() over (partition by PERSONNUMBER order by PERSONNUMBER)
  FROM
...
UNION ALL
SELECT 'MERGE|GoalPlanGoal|GP_'||PERSONNUMBER||'_'||row_number() over (partition by PERSONNUMBER order by PERSONNUMBER)||'|EBS|G_'||PERSONNUMBER||'_'||row_number() over (partition by PERSONNUMBER order by PERSONNUMBER)
||'|Citco Annual Performance Goal Plan|'||WEIGHTING_PERCENT||'|2021/11/01|2022/10/31' METADATA
FROM (SELECT UNIQUE ST.* FROM xxmx_per_goal_stg ST)
;
