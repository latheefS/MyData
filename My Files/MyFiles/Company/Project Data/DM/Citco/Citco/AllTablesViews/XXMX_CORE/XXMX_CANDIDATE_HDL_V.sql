--------------------------------------------------------
--  DDL for View XXMX_CANDIDATE_HDL_V
--------------------------------------------------------
...
where rowid = (select max(ROWID) from XXMX_HCM_IREC_CAN_XFM where candidate_number= r.candidate_number)
UNION ALL
...
where rowid = (select max(ROWID) from XXMX_HCM_IREC_CAN_EMAIL_XFM where candidate_number= r.candidate_number)
UNION ALL
...
where rowid = (select max(ROWID) from XXMX_HCM_IREC_CAN_NAME_XFM where candidate_number= r.candidate_number)
UNION ALL
...
where rowid = (select max(ROWID) from XXMX_HCM_IREC_CAN_ADDRESS_XFM where candidate_number= r.candidate_number)
UNION ALL
...
where rowid = (select max(ROWID) from XXMX_HCM_IREC_CAN_PHONE_XFM where candidate_number= r.candidate_number)
UNION ALL
...
||SOURCE||'|'||source_medium||'|'||TO_CHAR(TO_DATE(availability_date,'RRRR/MM/DD HH24:MI:SS'),'rrrr/mm/dd')||'|'
