--------------------------------------------------------
--  DDL for Package Body XXMX_DYNAMIC_SQL_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "XXMX_CORE"."XXMX_DYNAMIC_SQL_PKG" 
AS
-- ================================================================================
-- | VERSION1 |
-- ================================================================================
...
-- Date       Author            Comment
-- ---------- ----------------- -------------------- ---------------------------
-- 09/08/2022 Michal Arrowsmith Added code toremove TAC chr(9) characters from XFM tables
-- 12/10/2022 Michal Arrowsmith trim_xfm_table_data: For HCM do not check the include_in_outbound_file
-- =============================================================================
     ...
     */
     ...
     BEGIN
          ...
          xxmx_utilities_pkg.log_module_message
               ...
               ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
               ,pt_i_ProgressIndicator => gvv_ProgressIndicator
               ,pt_i_ModuleMessage     => 'PROCEDURE : '||gct_PackageName||'.'||ct_ProcOrFuncName||' ('||pt_i_BusinessEntity||' / '||pt_i_SubEntity||') initiated.'
               ,pt_i_OracleError       => NULL
               );
          --
          ...
          LOOP
               ...
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'Data tables identified '||'for Business Entity "'||pt_i_BusinessEntity||'" and Sub-Entity "'||pt_i_SubEntity||'".'
                    ,pt_i_OracleError       => NULL
                    );
               --
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'STG Table : "'||TablePair_rec.stg_schema_name||'.'||TablePair_rec.stg_table_name||'"'
                    ,pt_i_OracleError       => NULL
                    );
               --
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'XFM Table : "'||TablePair_rec.xfm_schema_name||'.'||TablePair_rec.xfm_table_name||'"'
                    ,pt_i_OracleError       => NULL
                    );
               --
               ...
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'Transferring Client Data from STG to XFM table for '||vv_IDLabel||'.'
                    ,pt_i_OracleError       => NULL
                    );
               --
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => gvn_RowCount||' rows exist in the STG table for '||vv_IDLabel||'.'
                    ,pt_i_OracleError       => NULL
                    );
               --
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'Constructing DELETE statement to '||'remove data previously populated into the '||'XFM table for '||vv_IDLabel||'.'
                    ,pt_i_OracleError       => NULL
                    );
               --
               --
               ...
               gvv_ProgressIndicator := '0050';
               --
               gvv_SQLAction1 := 'DELETE FROM '||gcv_SQLSpace||TablePair_rec.xfm_schema_name||'.'||TablePair_rec.xfm_table_name||' xfm';                       -- table_alias
               --
               --
               -- Construct the WHERE clause for the DELETE statement.
               --
               --
               gvv_SQLWhereClause := 'WHERE  1 = 1 '||'AND    xfm.'||vv_IDCondition;
               --
               gvc_SQLStatement := gvv_SQLAction1||gcv_SQLSpace||gvv_SQLWhereClause;
               --
               xxmx_utilities_pkg.log_module_message
                    (
                     pt_i_ApplicationSuite  => pt_i_ApplicationSuite
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'SQL Statement "'||gvv_SQLAction1||'..." constructed.'
                    ,pt_i_OracleError       => NULL
                    );
               --
               gvv_ProgressIndicator := '0060';
               ...
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => gvn_RowCount||' Rows deleted'
                    ,pt_i_OracleError       => NULL
                    );
               --
               gvv_ProgressIndicator := '0080';
               ...
               xxmx_utilities_pkg.log_module_message
                    ...
                    ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                    ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                    ,pt_i_ModuleMessage     => 'Constructing INSERT AS SELECT statement to '||'select from STG table and insert into XFM table.'
                    ,pt_i_OracleError       => NULL
                    );
               --
               --
               ...
               LOOP
                    gvv_SQLInsertColumnList := gvv_SQLInsertColumnList||','||ColumnMapping_rec.xfm_column_name;
                    --
                    gvv_SQLSelectColumnList := gvv_SQLSelectColumnList||','||'stg.'||ColumnMapping_rec.stg_column_name;
                    --
               END LOOP;
                if gvv_SQLInsertColumnList is null then
                    xxmx_utilities_pkg.log_module_message
                        (
                         pt_i_ApplicationSuite  => pt_i_ApplicationSuite
                        ,pt_i_Application       => pt_i_Application
                        ...
                        ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                        ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                        ,pt_i_ModuleMessage     => 'Unable to produce SQL Statement to copy STG to XFM table. Check tables xxmx_stg_table_columns and xxmx_xfm_table_columns are linked'
                        ,pt_i_OracleError       => NULL
                        );
                    gvv_ApplicationErrorMessage := 
                        SUBSTR('Unexpected Oracle Exception encountered after Progress Indicator "'||gct_PackageName||'.'||ct_ProcOrFuncName||'-'||
                        gvv_ProgressIndicator||'".  Please refer to XXMX_MODULE_MESSAGES table for further details.',1,2048);
                    RAISE_APPLICATION_ERROR(gcn_ApplicationErrorNumber, gvv_ApplicationErrorMessage);
                    --
                else
                    xxmx_utilities_pkg.log_module_message
                        ...
                        ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                        ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                        ,pt_i_ModuleMessage     => SUBSTR('SQL Statement "'||gvv_SQLSelectColumnList||' " constructed.',1,3000)
                        ,pt_i_OracleError       => NULL
                        );
                end if;
               --
...
is
    ...
    CURSOR c_xfm_tables (p_application_suite varchar2,p_application varchar2) is
        ...
        --
        cursor c_table_columns ( p_Applicationsuite in varchar2, p_xfm_table_id in number) is
            select  
                column_name, 
            ...
            and     1 = case 
                when p_Applicationsuite='HCM'           then 1
                when xtc.include_in_outbound_file = 'Y' then 1
                else 0
                end
            ;
        --
        ...
        *************************/
        --
        gct_PackageName             CONSTANT  VARCHAR2(30)                                := 'XXMX_DYNAMIC_SQL_PKG';
        cv_ProcOrFuncName           CONSTANT  VARCHAR2(30)                                 := 'trim_xfm_table_data';
         ...
         --
             ...
             *************************/
             --
              ...
              gvv_SQLStatement3                     VARCHAR2(32000);
    ...
    */
    ...
    loop
            ...
            xxmx_utilities_pkg.log_module_message
                                  ...
                                  ,pt_i_ProcOrFuncName    => cv_ProcOrFuncName
                                  ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                  ,pt_i_ModuleMessage     => 'gvv_sqlstatement='||gvv_sqlstatement
                                  ,pt_i_OracleError       => NULL
                                  );
            ...
            xxmx_utilities_pkg.log_module_message
                     ...
                     ,pt_i_ProcOrFuncName    => cv_ProcOrFuncName
                     ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                     ,pt_i_ModuleMessage     => 'Calling xxmx_utilities_pkg.clean_xfm_data'
                     ,pt_i_OracleError       => NULL
                     );
            xxmx_utilities_pkg.clean_xfm_data(r_xfm_tables.xfm_table_id,r_xfm_tables.xfm_table_name);
            --
    end loop;
exception
    WHEN e_nodata THEN
...
procedure transform_data
     ...
     BEGIN
                    ...
                    */
                    ...
                    LOOP
                         ...
                         xxmx_utilities_pkg.log_module_message
                              ...
                              ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                              ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                              ,pt_i_ModuleMessage     => 'Processing column "'||ColumnToUpdate_rec.xfm_column_name||'".'
                              ,pt_i_OracleError       => NULL
                              );
                         --
                         ...
                         THEN
                              ...
                              xxmx_utilities_pkg.log_module_message
                                   ...
                                   ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                                   ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                   ,pt_i_ModuleMessage     => 'Column '||ColumnToUpdate_rec.xfm_column_name||' has both Data Transformation and Data Enrichment defined.'
                                   ,pt_i_OracleError       => NULL
                                   );
                              --
                              xxmx_utilities_pkg.log_module_message
                                   ...
                                   ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                                   ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                   ,pt_i_ModuleMessage     => 'Data rows with a NULL column value will be assigned the value of "'||ColumnToUpdate_rec.default_value||'" (Data Enrichment).'
                                   ,pt_i_OracleError       => NULL
                                   );
                              --
                              xxmx_utilities_pkg.log_module_message
                                   ...
                                   ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                                   ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                   ,pt_i_ModuleMessage     => 'Data Rows with a Source Value will be transformed based on TRANSFORM_CODE: "'||ColumnToUpdate_rec.transform_code||'" (Data Transformation).'
                                   ,pt_i_OracleError       => NULL
                                   );
                              --
                              ...
                              ELSE
                                   ...
                                   xxmx_utilities_pkg.log_module_message
                                        ...
                                        ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                                        ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                        ,pt_i_ModuleMessage     => 'Data Rows which fail transformation will NOT be assigned default value and will retain their original value.'
                                        ,pt_i_OracleError       => NULL
                                        );
                                   --
                         ...
                         */
                         ...
                         xxmx_utilities_pkg.log_module_message
                            ...
                            ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                            ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                            ,pt_i_ModuleMessage     => 'gvc_SQLStatement='||substr(gvc_SQLStatement,1,3000)
                            ,pt_i_OracleError       => NULL
                            );
                         /*INSERT
                         INTO   isv_temp
                         ...
                         xxmx_utilities_pkg.log_module_message
                                        ...
                                        ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                                        ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                        ,pt_i_ModuleMessage     => 'GVC_SQLSTATEMENT CREATED'
                                        ,pt_i_OracleError       => NULL
                                        );
                         --
        ...
        COMMIT;
        --
        -- Call the procedure to trim all VARCHAR2 fileds in XFM table and handle double quotes 
        --
        gvv_ProgressIndicator:= '0110';
        xxmx_utilities_pkg.log_module_message
               (
               ...
               ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
               ,pt_i_ProgressIndicator => gvv_ProgressIndicator
               ,pt_i_ModuleMessage     => 'Calling trim_xfm_table_data : pt_i_MigrationSetID='||pt_i_MigrationSetID||'/pt_i_FileSetID='||pt_i_FileSetID||'/pt_i_BusinessEntity='||pt_i_BusinessEntity||'/pt_i_sub_entity='||pt_i_SubEntity
               ,pt_i_OracleError       => NULL
               );
        trim_xfm_table_data
        (
                ...
                ,pt_i_FileSetID                  => pt_i_FileSetID
                ,pt_i_BusinessEntity             => pt_i_BusinessEntity
                ,pt_i_sub_entity                 => pt_i_SubEntity
        );
        --
        xxmx_utilities_pkg.log_module_message
               (
     ...
     */
     ...
     BEGIN
          ...
          xxmx_utilities_pkg.log_module_message
               ...
               ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
               ,pt_i_ProgressIndicator => gvv_ProgressIndicator
               ,pt_i_ModuleMessage     => 'pv_i_ExtensionType= '||pv_i_ExtensionType
               ,pt_i_OracleError       => NULL
               );
          gvv_ProgressIndicator := '0030';
          xxmx_utilities_pkg.log_module_message
               ...
               ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
               ,pt_i_ProgressIndicator => gvv_ProgressIndicator
               ,pt_i_ModuleMessage     => 'pt_i_StgPopulationMethod= '||pt_i_StgPopulationMethod
               ,pt_i_OracleError       => NULL
               );
          --
          --
          ...
          xxmx_utilities_pkg.log_module_message
               ...
               ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
               ,pt_i_ProgressIndicator => gvv_ProgressIndicator
               ,pt_i_ModuleMessage     => 'Evaluating '||vv_MessageExtensionType||' Extensions for the "'||pt_i_Phase||'" Phase of Business Entity "'||pt_i_BusinessEntity||'".'
               ,pt_i_OracleError       => NULL
               );
          --
          gvv_ProgressIndicator := '0060';
          xxmx_utilities_pkg.log_module_message
               ...
               ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
               ,pt_i_ProgressIndicator => gvv_ProgressIndicator
               ,pt_i_ModuleMessage     => 'Processing Business Entity '||vv_MessageExtensionType||' Extensions.'
               ,pt_i_OracleError       => NULL
               );
          --
          ...
          LOOP
               ...
               --
               --
               IF   Extensions_cur%NOTFOUND THEN
               ...
               ELSE
                    ...
                    xxmx_utilities_pkg.log_module_message
                         ...
                         ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                         ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                         ,pt_i_ModuleMessage     => 'Verifying Extension Package "'||vt_SchemaName||'.'||vt_PackageName||'".'
                         ,pt_i_OracleError       => NULL
                         );
                    ...
                    --
                    IF   gvt_ObjectStatus = 'NOT EXIST' THEN
                         --
                         xxmx_utilities_pkg.log_module_message
                              (
                               pt_i_ApplicationSuite  => pt_i_ApplicationSuite
                         ...
                         --
                    ELSIF gvt_ObjectStatus = 'INVALID' THEN
                         --
                         xxmx_utilities_pkg.log_module_message
                              (
                               pt_i_ApplicationSuite  => pt_i_ApplicationSuite
                    ...
                    ELSE
                         ...
                         xxmx_utilities_pkg.log_module_message
                              ...
                              ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                              ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                              ,pt_i_ModuleMessage     => 'Extension Procedure "' ||vt_PackageName ||'.' ||vt_ProcedureName ||'" (' ||vt_ExtensionSource ||') is NOT enabled.'
                              ,pt_i_OracleError       => NULL
                              );
                         --
                         vb_CallExtension := FALSE;
                    ...
                    IF   vb_CallExtension THEN
                         xxmx_utilities_pkg.log_module_message
                              ...
                              ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                              ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                              ,pt_i_ModuleMessage     => 'Calling Extension Procedure "'||vt_PackageName||'.'||vt_ProcedureName||'" ('||vt_ExtensionSource||').'
                              ,pt_i_OracleError       => NULL
                              );
                         --
                         gvv_ProgressIndicator := '0120';
                         ...
                         IF   gvv_ReturnStatus <> 'S' THEN
                              xxmx_utilities_pkg.log_module_message
                                   ...
                                   ,pt_i_ProcOrFuncName    => ct_ProcOrFuncName
                                   ,pt_i_ProgressIndicator => gvv_ProgressIndicator
                                   ,pt_i_ModuleMessage     => 'gvt_ReturnMessage='||gvt_ReturnMessage
                                   ,pt_i_OracleError       => NULL
                                   );
                              --
                              IF   vt_ExecuteNextOnError = 'N' THEN
                                   vb_CallNextExtInSequence := FALSE;
                                   xxmx_utilities_pkg.log_module_message
                                        (
                                         pt_i_ApplicationSuite  => pt_i_ApplicationSuite
