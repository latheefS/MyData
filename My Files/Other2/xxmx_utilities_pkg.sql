  ...
  /*
     ...
     */
     --
       ...
       PROCEDURE fusion_Customer_batch(p_file_set_id VARCHAR2);

       PROCEDURE xxmx_write_dbcs (  pt_i_businessentity IN VARCHAR2  ) ;                                    
...
*/
    ...
    BEGIN
        ...
        IF( V_BATCH_LOAD= 'Y') THEN 
            OPEN CUR FOR   'select '||v_column_lst ||' , '||'NVL(batch_name,''NULL'')'||' from '
...
END get_FBDI_filenames;
   ...
   AS                            
      ...
      CURSOR c_columns(p_table_name VARCHAR2)
      IS 
   ...
   Begin
       ...
       LOOP
           ...
           FOR rec_columns IN  c_columns(rec.EXT_TABLE)
           LOOP
               --v_log := 'Column Creation '||rec.EXT_TABLE ||' '||rec_columns.COLUMN_NAME;
               IF( v_column_lst is NULL) THEN 
                   v_column_lst:= rec_columns.COLUMN_NAME;
               ELSE 
                   v_column_lst := v_column_lst ||' , '|| rec_columns.COLUMN_NAME;
        ...
        log_module_message
                ...
                ,pt_i_ProcOrFuncName            => ct_ProcOrFuncName
                ,pt_i_ProgressIndicator         => gvv_ProgressIndicator
                ,pt_i_ModuleMessage             => 'Check if External Table '|| rec.EXT_TABLE ||' has records '
                ,pt_i_OracleError               => NULL
                );
           ...
           WHEN et_exception THEN
               ...
               gvv_ProgressIndicator := '0025';
                log_module_message
                        ...
                        ,pt_i_ProcOrFuncName            => ct_ProcOrFuncName
                        ,pt_i_ProgressIndicator         => gvv_ProgressIndicator
                        ,pt_i_ModuleMessage             => SUBSTR('Unable to read the external file for '|| gvt_ModuleMessage||' '||SQLERRM||' '||SQLCODE,1, 3000) 
                        ,pt_i_OracleError               => NULL
                        );
           WHEN OTHERS THEN
                    RollBack;
   ...
   --End of Added procedure for NON-EBS Clients
    ...
    BEGIN
        ...
        LOOP
            ...
            xxmx_file_zip_utility_pkg.save_zip( g_zipped_blob ,pt_dirname,pt_i_BusinessEntity||'_'||r_zip.batch_name||'_'||SYSDATE||'.zip');
     ...
     IF(v_batch_load= 'Y'and v_SEQ_IN_FBDI_DATA IS NULL) THEN
        ...
        IF( ROUND(ntile_var) > 0) THEN
                ...
                v_sql:='SELECT '''||v_STG_TABLE||''', NTILE('||ntile_var||') OVER( ORDER BY '|| v_common_load_column ||' DESC   ) AS batch_name_1, '
                        ||v_common_load_column|| ',xxmx_loadfile_batchid_s.nextval   FROM ( SELECT DISTINCT '||v_common_load_column||' FROM XXMX_STG.'||v_stg_table ||
                        ' WHERE  migration_set_id = '||lv_migration_set_id ||' )';

                DBMS_OUTPUT.PUT_LINE('TEST3-'||v_sql);
                execute immediate 'INSERT INTO xxmx_dm_file_batch (tablename,Batch_name , batch_column,SequenceBatch)  '||v_sql;
